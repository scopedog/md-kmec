#!/bin/bash
#
# raidkm-test-declustered-reshape-addparity.sh — declustered ADD-PARITY reshape
# (raise m within each group: m -> m+1, g -> g+1) via the general geometry-
# transition engine.  Adds ngroups disks (one new parity column per group), so
# k, ngroups and capacity stay fixed; the group EC tables are rebuilt for the
# new m.  Driven through the rk_dcl_reshape trigger with an explicit new layout
# word (the mdadm --add-parity driver is a follow-up).
...
NGROUPS=$(( (N - SC) / G ))
NEWN=$(( N + NGROUPS ))				# one new parity column per group
NEWG=$(( G + 1 )); NEWM=$(( M + 1 ))
...
echo "=== add-parity: +$NGROUPS disks, m=$M->$NEWM g=$G->$NEWG N=$N->$NEWN (layout 0x$NEWLAYOUT) ==="
...
case "$(cat "$TRIG")" in idle*) rk_pass "add-parity reshape completed";;
...
[ "$PRE" = "$(sudo dd if="$MD" bs=1M count="$PATMB" iflag=direct status=none 2>/dev/null|md5sum|cut -d' ' -f1)" ] &&
	rk_pass "data intact across add-parity" || rk_fail "DATA MISMATCH"
[ "$(cat /sys/block/$MDNAME/size)" = "$OLD_SIZE" ] &&
	rk_pass "capacity unchanged (k fixed)" || rk_fail "capacity changed unexpectedly"
mm=$(rk_scrub); [ "$mm" = 0 ] && rk_pass "scrub clean (new m=$NEWM parity consistent)" || rk_fail "scrub mismatch_cnt=$mm"
# THE decode oracle: fail the NEW m disks -> must reconstruct (proves the new
# parity is genuine, not just scrub-consistent)
...
[ "$deg" = "$NEWM" ] && [ "$PRE" = "$DEG" ] &&
	rk_pass "degraded-decode at new m=$NEWM EC-correct (raised redundancy is real)" ||
...
sudo "$MDADM" --stop "$MD" 2>/dev/null
# zero the (now m+1 / N') superblocks so a following run on the same devices
# does not race udev auto-assembling this array while it recreates
