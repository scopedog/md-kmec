...
mdadm --grow /dev/mdN --spare-columns=s'        # s -> s' (decrease; capacity grows)
```

add-parity/add-data take one new disk per group (`ngroups` disks); spare-count
runs on the fixed pool (`delta_disks == 0`). While the migration walks the
...
Crash recovery is identical (journal v2 records both geometries). Exception: on
a **native-checksum** array these layout-changing reshapes are offline-only for
now — the trigger refuses while the array is open, and mdadm checks first.
...
resize are supported), and every **capacity-shrinking** reshape (pool shrink,
spare-count increase). The group-geometry reshapes — per-group **add-parity**
(`--grow --add-parity`), **add-data** (`--grow --add-data`) and **spare-count
decrease** (`--grow --spare-columns=s'`) — run **online** like pool expansion:
the un-migrated region is served with old-geometry stripes while the journaled
COW migration walks the array. The one exception is a **native-checksum**
array, whose group-geometry reshapes are offline-only for now (the CRC re-key
is not yet validated against concurrent writes; unmount, reshape, remount).
