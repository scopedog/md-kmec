...
write-intent bitmap and online resize. These compose with declustering in
principle but are not yet wired; a non-declustered raidkm array has them.
Raising redundancy or efficiency *within* a group (per-group add-parity /
add-data) and shrinking the pool are v2 reshapes. (Native checksums and online
pool expansion, by contrast, **are** supported — see above.)
