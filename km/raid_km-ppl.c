		       ...
		       struct dma_async_tx_descriptor *tx)
{
	...
	/*
	 * raid_km redesign: arbitrary m closes the write hole by logging ALL
	 * m partial parities (P via XOR/raid6, Q/R/S via ISA-L), not just the
	 * single XOR parity below.
	 */
	if (sh->raid_conf->level == RAID_KM_LEVEL) {
		struct r5conf *conf = sh->raid_conf;

		/*
		 * m==2 RMW fast path: ops_run_prexor6 has already left the two
		 * partial parities in the P and Q dev pages, so just copy them
		 * into the PPL pages.  Chain the copies onto the async pipeline
		 * (off the prexor tx) rather than quiescing per stripe — the
		 * quiesce drained the whole RMW pipeline and dominated the PPL
		 * write penalty.
		 */
		if (conf->m == 2 &&
		    sh->reconstruct_state == reconstruct_state_prexor_drain_run) {
			init_async_submit(&submit, ASYNC_TX_FENCE, tx, NULL, sh,
					  (void *)(srcs + sh->disks + 2));
			tx = async_memcpy(sh->ppl_pages[0], sh->dev[pd_idx].page,
					  0, 0, PAGE_SIZE, &submit);
			init_async_submit(&submit, ASYNC_TX_FENCE, tx, NULL, sh,
					  (void *)(srcs + sh->disks + 2));
			tx = async_memcpy(sh->ppl_pages[1],
					  sh->dev[sh->qd_idx].page, 0, 0,
					  PAGE_SIZE, &submit);
			return tx;
		}

		/* m>2 (ISA-L is synchronous), or m==2 RCW: compute in place */
		if (tx)
			async_tx_quiesce(&tx);
		raidkm_compute_partial_parity(sh);
		return NULL;
	}
	...
	if (count == 1)
		tx = async_memcpy(sh->ppl_pages[0], srcs[0], 0, 0, PAGE_SIZE,
				  &submit);
	else
		tx = async_xor(sh->ppl_pages[0], srcs, 0, count, PAGE_SIZE,
			       &submit);
...
static int ppl_log_stripe(struct ppl_log *log, struct stripe_head *sh)
{
	...
	for (i = 0; i < sh->disks; i++) {
		struct r5dev *dev = &sh->dev[i];
		bool is_parity = (conf->level == RAID_KM_LEVEL)
			? is_parity_disk(sh, i)		/* wrap-aware, any m/layout */
			: (i == sh->pd_idx);

		if (!is_parity && test_bit(R5_Wantwrite, &dev->flags)) {
	...
	if (!test_bit(STRIPE_FULL_WRITE, &sh->state)) {
		/*
		 * raid_km logs all m partial parities per stripe (raid5: 1);
		 * pp_size is the PHYSICAL total so ppl_recover()'s checksum
		 * and sector-advance need no change.
		 */
		int np = conf->m > 0 ? conf->m : 1;
		u32 csum = le32_to_cpu(e->checksum);
		int j;

		le32_add_cpu(&e->pp_size, np << PAGE_SHIFT);
		io->pp_size += np << PAGE_SHIFT;
		for (j = 0; j < np; j++)
			csum = crc32c_le(csum, page_address(sh->ppl_pages[j]),
					 PAGE_SIZE);
		e->checksum = cpu_to_le32(csum);
	}
...
int ppl_write_stripe(struct r5conf *conf, struct stripe_head *sh)
{
	...
	if (io || test_bit(STRIPE_SYNCING, &sh->state) || !sh->ppl_pages[0] ||
...
static void ppl_submit_iounit(struct ppl_io_unit *io)
{
	...
	int np = (conf && conf->m > 0) ? conf->m : 1;	/* PP pages per stripe */
	int i;
	...
	list_for_each_entry(sh, &io->stripe_list, log_list) {
		...
		/* raid_km writes all m partial-parity pages per stripe */
		for (i = 0; i < np; i++) {
			struct page *pp = sh->ppl_pages[i];

			if (!pp)
				break;
			if (!bio_add_page(bio, pp, PAGE_SIZE, 0)) {
				struct bio *prev = bio;
				...
				__bio_add_page(bio, pp, PAGE_SIZE, 0);
 ...
 */
/*
 * raid_km redesign: m-parity PPL recovery.  An entry stores m partial
 * parities per stripe (pp_size is the physical total); recovery rebuilds all
 * m parities as  P_j = PP_j XOR encode_j(modified data)  and writes each to
 * its parity slot pd_idx..pd_idx+m-1 (PARITY_N).  GF addition is XOR for
 * every parity, so the same XOR-in-the-PP step works for P, Q, R, S.
 *
 * On-disk PP layout per entry is block-major: [blk0: pp0..pp(m-1)][blk1: ...].
 */
static int ppl_recover_entry_km(struct ppl_log *log, struct ppl_header_entry *e,
				sector_t ppl_sector)
{
	struct ppl_conf *ppl_conf = log->ppl_conf;
	...
	int strip_sectors, data_disks, i, j, ret = 0;
	...
	for (j = 0; j < m; j++)
		ppage[j] = alloc_page(GFP_KERNEL);
	for (i = 0; i < k; i++)
		dpage[i] = alloc_page(GFP_KERNEL);
	if (!scratch) {
		ret = -ENOMEM;
		goto out_km;
	}
	for (j = 0; j < m; j++)
		if (!ppage[j]) { ret = -ENOMEM; goto out_km; }
	for (i = 0; i < k; i++)
		if (!dpage[i]) { ret = -ENOMEM; goto out_km; }

	r_sector_first = le64_to_cpu(e->data_sector) * (block_size >> 9);

	if ((pp_per >> 9) < conf->chunk_sectors) {
		if (pp_per > 0) {
			data_disks = data_size / pp_per;
			strip_sectors = pp_per >> 9;
		} else {
			data_disks = k;
	...
	for (i = 0; i < strip_sectors; i += (block_size >> 9)) {
		...
		/*
		 * Resolve this block's stripe geometry up front: pd_idx (the
		 * first parity slot) drives the physical<->logical slot mapping
		 * for both the data reads and the parity writes below.  All
		 * data disks of this block share one stripe row, hence one
		 * pd_idx.  The logged parity_disk must match (a layout/seq sanity
		 * check that holds for PARITY_N and rotating alike).
		 */
		parity_sector = raid5_compute_sector(conf, r_sector_first + i,
						     0, &disk, &sh);
		pd = sh.pd_idx;
		BUG_ON(pd != le32_to_cpu(e->parity_disk));

		for (c = 0; c < k; c++)
			memset(page_address(dpage[c]), 0, block_size);

		/* read the modified data into dpage[<logical data index>] so the
		 * encode below sees src[0..k-1] in logical column order */
		for (disk = 0; disk < data_disks; disk++) {
			...
			sector = raid5_compute_sector(conf, r_sector, 0,
						      &dd_idx, NULL);
			/* logical data index of physical slot dd_idx in this row */
			ldx = ((dd_idx - pd + conf->raid_disks) % conf->raid_disks)
			      - m;
			...
			if (!sync_page_io(rdev, sector, block_size,
					  dpage[ldx], REQ_OP_READ, false)) {
				md_error(mddev, rdev);
				ret = -EIO;
				goto out_km;
			}
		...
		/* cod_j = encode_j(modified data in logical column order) */
		for (c = 0; c < k; c++)
			src[c] = page_address(dpage[c]);
		for (j = 0; j < m; j++)
			cod[j] = page_address(ppage[j]);
		raidkm_ppl_encode_modified(conf, src, cod, block_size);

		/* P_j = PP_j XOR encode_j(modified); full-stripe has no PP */
		if (pp_per > 0) {
			for (j = 0; j < m; j++) {
				sector_t pp_sec = ppl_sector -
					log->rdev->data_offset +
					(sector_t)(b * m + j) * (block_size >> 9);

				if (!sync_page_io(log->rdev, pp_sec, block_size,
						  scratch, REQ_OP_READ, false)) {
					md_error(mddev, log->rdev);
					ret = -EIO;
					goto out_km;
				}
				ppl_xor(block_size, ppage[j], scratch);
			}
		}

		/* write each parity index j to its physical slot (pd+j) mod N
		 * (contiguous for PARITY_N, may wrap for rotating) */
		for (j = 0; j < m; j++) {
			int pslot = (pd + j) % conf->raid_disks;
			struct md_rdev *prdev = conf->disks[pslot].rdev;

			if (!prdev || test_bit(Faulty, &prdev->flags))
				continue;	/* missing parity rebuilt later */
			if (!sync_page_io(prdev, parity_sector, block_size,
					  ppage[j], REQ_OP_WRITE, false)) {
				md_error(mddev, prdev);
				ret = -EIO;
				goto out_km;
			}
		}
			     ...
			     sector_t ppl_sector)
{
	...
	if (mddev->level == RAID_KM_LEVEL)
		return ppl_recover_entry_km(log, e, ppl_sector);
...
int ppl_init_log(struct r5conf *conf)
{
	...
	/*
	 * raid_km redesign: PPL is valid for raid5 (single XOR parity) and for
	 * raidkm (level 71, arbitrary m) where we log all m partial parities.
	 */
	if (mddev->level != 5 && mddev->level != RAID_KM_LEVEL) {
