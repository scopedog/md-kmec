...
#include "../isa-l/isa-l_ec.h"

#define UNSUPPORTED_MDDEV_FLAGS	(1L << MD_FAILFAST_SUPPORTED)

/*
 * RHEL 10.2 ABI guard.  raidkm.ko is an out-of-tree module but binds to the
 * *builtin* md_mod, so the layout of struct mddev / struct md_rdev in our md.h
 * must stay byte-identical to the running 6.12.0-211 kernel's.  These
 * compile-time checks lock the fork's md.h to the offsets verified against
 * vmlinux BTF during the RHEL 10.2 port; an accidental md.h edit that drifts
 * the layout fails the build here instead of corrupting memory at runtime.
 *
 * NOTE: this only catches drift in *our* md.h — the compiler sees only the
 * fork's definition, not the running kernel's.  To catch the kernel changing
 * out from under us (e.g. a future 10.2 errata kernel reordering struct mddev),
 * verify these offsets against the target vmlinux's BTF at build time
 * (e.g. `pahole -C mddev`).  Never called; __maybe_unused silences the warning.
 */
static void __maybe_unused raidkm_mddev_abi_checks(void)
{
	...
	BUILD_BUG_ON(offsetof(struct mddev, gendisk)		!= 120);
	BUILD_BUG_ON(offsetof(struct mddev, dm_gendisk)		!= 128);
	BUILD_BUG_ON(offsetof(struct mddev, level)		!= 264);
	BUILD_BUG_ON(offsetof(struct mddev, reshape_position)	!= 352);
	BUILD_BUG_ON(offsetof(struct mddev, sync_io_depth)	!= 488);
	BUILD_BUG_ON(offsetof(struct mddev, normal_io_events)	!= 600);
	BUILD_BUG_ON(offsetof(struct mddev, recovery_active)	!= 608);
	BUILD_BUG_ON(offsetof(struct mddev, cluster_ops)	!= 2040);
	BUILD_BUG_ON(offsetof(struct md_rdev, meta_bdev)	!= 40);
#else
	/* mainline / Debian 6.12 md core (verified via BTF on Debian 13
	 * trixie, 6.12.x-amd64). The kernel-side check (check-mddev-abi.sh,
	 * pahole vs /sys/kernel/btf) is the authoritative guard; these are
	 * a compile-time backstop against fork-side md.h drift. */
	BUILD_BUG_ON(sizeof(struct mddev)			!= 2336);
	BUILD_BUG_ON(offsetof(struct mddev, gendisk)		!= 120);
	BUILD_BUG_ON(offsetof(struct mddev, level)		!= 256);
	BUILD_BUG_ON(offsetof(struct mddev, reshape_position)	!= 344);
	BUILD_BUG_ON(offsetof(struct mddev, recovery_active)	!= 584);
...
static int default_group_thread_cnt = -1;
...
static int default_bio_sort = -1;
module_param(default_bio_sort, int, 0644);
...
static int default_row_rebuild_window = -1;
module_param(default_row_rebuild_window, int, 0644);
 ...
 * preread_active_stripes falls below IO_THRESHOLD, which a saturating load of
 * REQ_SYNC writes (every O_DIRECT write is one) can prevent for as long as it
 * lasts.  Two guards, each with its own switch so either can be the control
 * arm of a measurement:
 */
static bool sync_write_nodelay = true;
module_param(sync_write_nodelay, bool, 0644);
MODULE_PARM_DESC(sync_write_nodelay,
		 "A stripe with a pending REQ_SYNC write takes the preread token instead of being delayed when it lost it to an earlier write on the same stripe (default Y).");

static unsigned int delayed_max_age_ms = 100;
module_param(delayed_max_age_ms, uint, 0644);
MODULE_PARM_DESC(delayed_max_age_ms,
		 "Activate a stripe that has waited this long on delayed_list even while prereads are active (default 100; 0 = never, the stock behaviour).");

static unsigned int overlap_warn_secs = 5;
module_param(overlap_warn_secs, uint, 0644);
MODULE_PARM_DESC(overlap_warn_secs,
		 "Report a writer that has waited this long on R5_Overlap, with the state of the stripe it waits for (default 5; 0 = wait silently).");
...
static bool default_row_write;
module_param(default_row_write, bool, 0644);
MODULE_PARM_DESC(default_row_write,
		 "Initial rk_row_write for new arrays: write a full, aligned row as one request per member instead of through the 4 KiB stripe cache (default N).");

#define RK_ROW_WRITE_DEPTH	64	/* rows in flight through the row path */
#define RK_ROW_WRITE_DEPTH_MAX	1024
unsigned int raidkm_dcl_pop_window = RKDCL_REB_WINDOW;
module_param(raidkm_dcl_pop_window, uint, 0644);
MODULE_PARM_DESC(raidkm_dcl_pop_window,
		 "Declustered population completion window in 4 KiB granules, fixed when an array is loaded (default 16384 = 64 MiB; rounded up to a power of two). A throughput knob, not a correctness bound: population waits rather than overrun it. Small values exist to drive that wait on small test devices.");
...
/*
 * Rows a full-row write owns (raidkm_row_write_prep).  One slot per hash of
 * the row, holding row + 1 while the row is being written, 0 otherwise.  A
 * slot holds exactly one row: a second row that hashes to a busy slot is not
 * locked, its write just goes to the stripe cache, and a lookup compares the
 * row itself, so a collision never makes the stripe cache wait for a row it
 * is not touching.
 *
 * raid5_get_active_stripe() consults the table under the stripe's hash lock
 * (only while row writes are in flight), and the row writer walks the row's
 * stripes under those same locks after setting its slot, so every lookup of
 * the row either happened before the walk -- the writer then finds the stripe
 * in use and gives the row up -- or sees the slot and waits.
 */
#define RAIDKM_ROW_WLOCK_BITS	14
#define RAIDKM_ROW_WLOCK_SLOTS	(1 << RAIDKM_ROW_WLOCK_BITS)
/*
 * Fairness.  A stripe-cache request that waits for a row sets WANTED on the
 * slot; the unlock then leaves the slot at row + 1 | GRACE instead of 0, which
 * no longer reads as locked but refuses the next row write's cmpxchg, until
 * the waiter has its stripe and clears it (raid5_get_active_stripe).  Without
 * it, back-to-back full-row writes could take the row again every time,
 * before the woken waiter looks.
 */
#define RAIDKM_ROW_WLOCK_WANTED	(1ULL << 63)
#define RAIDKM_ROW_WLOCK_GRACE	(1ULL << 62)

static inline u64 raidkm_row_of(struct r5conf *conf, sector_t member_sector)
{
	sector_div(member_sector, conf->chunk_sectors);
	return member_sector;
}

static inline atomic64_t *raidkm_row_wslot(struct r5conf *conf, u64 row)
{
	return &conf->row_wlock[(row * 0x9E3779B97F4A7C15ULL) >>
				(64 - RAIDKM_ROW_WLOCK_BITS)];
}

/* is the row holding stripe @sector (member sector) being row-written? */
static inline bool raidkm_row_wlocked(struct r5conf *conf, sector_t sector,
				      int dcl_group)
{
	u64 row;

	if (likely(!atomic_read(&conf->row_writes)) || dcl_group ||
	    !conf->row_wlock)
		return false;
	row = raidkm_row_of(conf, sector);
	return (atomic64_read(raidkm_row_wslot(conf, row)) &
		~RAIDKM_ROW_WLOCK_WANTED) == row + 1;
}

/* a waiter is about to sleep on the row of @sector: ask for it next */
static inline void raidkm_row_wwant(struct r5conf *conf, sector_t sector)
{
	u64 row = raidkm_row_of(conf, sector);

	atomic64_cmpxchg(raidkm_row_wslot(conf, row), row + 1,
			 (row + 1) | RAIDKM_ROW_WLOCK_WANTED);
}

/* ... and it has its stripe: the row is fair game again */
static inline void raidkm_row_wgot(struct r5conf *conf, sector_t sector)
{
	u64 row = raidkm_row_of(conf, sector);

	atomic64_cmpxchg(raidkm_row_wslot(conf, row),
			 (row + 1) | RAIDKM_ROW_WLOCK_GRACE, 0);
}
...
/* Declustered Phase 3/3b: the spare-column redirect
 * (notes/declustered-population-design.md §1).  While spare assignments
 * {X_i -> spare col j_i} are active, a slot mapped to an X_i is served
 * elsewhere:
 *
 ...
 *
 * MULTI-ASSIGNMENT (3b): spare columns rotate over ALL pool disks, so a
 * redirect can land on another failed-and-assigned disk (rows where j_2's
 * physical disk IS X_1).  Resolution CHAINS until it reaches a live disk or
 * dead-ends (uncovered POPULATING hop -> decode).  Bijectivity makes chains
 * vertex-disjoint and cycle-free from group columns (dcl_resolve in
 * raid_km_dcl.h is the reference; simulator P4 proves the invariants).
 * Only ONE entry is ever POPULATING, so the prefix mark is global.
 *
 ...
 * (retiring assignments for rebalance) happen only under a full quiesce.
 */
static inline int raidkm_dcl_redirect(struct r5conf *conf, int disk,
				      sector_t row, bool for_write)
{
	/* Pairs with the smp_store_release(&conf->nreb, ...) in
	 * raidkm_dcl_arm: entries below the acquired count are fully
	 * published.  (Retire shrinks nreb under a full quiesce, so no
	 * reader is in flight for that direction.) */
	int nreb = smp_load_acquire(&conf->nreb);
	int hops;
	...
	for (hops = 0; hops <= nreb; hops++) {
		...
		if (!to_spare)
			return disk;		/* R (below mark) / decode */
...
/* The GROUP COLUMN whose write-chain traverses disk X in this row, or -1
 * when none does (X holds an unassigned spare column, or a content-free
 * spare-only cycle/self-loop).  Found by the INVERSE walk: X's column; if
 * it is the spare of an active assignment, step to that assignment's disk
 * (its unique predecessor) and repeat.  At most one chain traverses any
 * disk per row (vertex-disjointness), so this is THE candidate — the
 * population pass rebuilds exactly this slot and the write-side chain
 * re-materialises every hop's content at the live endpoint.  This replaces
 * the single-assignment "X at a spare column -> skip" rule: rows where X
 * held ANOTHER assignment's live spare content walk back to that
 * assignment's root. */
static inline int raidkm_dcl_chain_root(struct r5conf *conf, sector_t row,
					int X)
{
	...
	for (hops = 0; hops <= nreb; hops++) {
		u32 lcol = dcl_lcol(conf->dcl, row, X);
		int i, j;

		if (lcol < conf->dcl->ngroups * conf->dcl->g)
			return (int)lcol;	/* group column: root found */
		j = (int)(lcol - conf->dcl->ngroups * conf->dcl->g);
		for (i = 0; i < nreb; i++)
			if (conf->reb[i].spare == j)
				break;
		if (i == nreb)
			return -1;	/* unassigned spare: content-free */
		X = conf->reb[i].disk;	/* unique predecessor */
	}
	return -1;		/* spare-only cycle: content-free */
}

/* The population target slot of THIS stripe, or -1: the slot whose
 * write-chain traverses the POPULATING X (at most one per row) when its
 * group is the one the population pass issued for this row. */
static inline int raidkm_dcl_pop_slot(struct r5conf *conf,
				      struct stripe_head *sh)
{
	/* Single load: the completion path stores reb_pop = -1 from the
	 * sync thread while straggler sync stripes are still handled on
	 * raid5d — a re-load between guard and index would read reb[-1]. */
	int pop = READ_ONCE(conf->reb_pop);
	sector_t row;
	int root;

	if (pop < 0 || pop >= conf->nreb)
		return -1;
	/* POPULATING only: reb_pop also indexes a COPYING entry for whole
	 * copy sessions, and a stripe that runs through here then (a REQUESTED
	 * check/repair, dirty-assemble resync stripes) must NOT be treated as
	 * a population target — it would advance the COPY's mark without
	 * writing R and skip the parity verify on X's chain rows. */
	if (READ_ONCE(conf->reb[pop].state) != RKDCL_ASSIGN_POPULATING)
		return -1;
	row = sh->sector;
	sector_div(row, conf->chunk_sectors);
	root = raidkm_dcl_chain_root(conf, row, conf->reb[pop].disk);
	if (root < 0)
		return -1;			/* content-free row */
...
/*
 * raid_km redesign helpers.  is_raidkm() / is_parity_disk() and the
 * slot-mapping inlines (raidkm_parity_slot/data_slot/data_index/matrix_row)
 * live in raid_km.h so raid_km-ppl.c can share them.
 *
 * for_each_intermediate_parity() — iterate the physical slots of the
 *   parity indices strictly between the first (pd_idx, index 0) and the
 *   last (qd_idx, index m-1).  (slot) receives the physical slot and (j)
 *   the parity index; the caller supplies both as loop variables.  Empty
 *   loop for raid4/5/6 and for raidkm at m=2.  Wrap-aware (via
 *   raidkm_parity_slot) so it is correct under the rotating layout where
 *   the parity block wraps past disk N-1.  Used by sites that already
 *   handle pd_idx and qd_idx explicitly and just need to fan out the same
 *   handling over the m > 2 intermediates.
 */
#define for_each_intermediate_parity(sh, slot, j) \
	for ((j) = 1, (slot) = raidkm_parity_slot((sh), 1); \
	     (j) < raidkm_sh_m(sh) - 1; \
	     (j)++, (slot) = raidkm_parity_slot((sh), (j)))

/*
 * raidkm: are all intermediate parity slots safe for the stripe to
 * be released (clean_event-ready)?  "Safe" means write completed
 * (!LOCKED) and either holds valid parity (UPTODATE) or was
 * discarded.  A failed slot is implicitly safe — we can't be
 * waiting on a write to it.  Returns true for non-raidkm configs
 * and for raidkm at m=2 (no slots between pd_idx and qd_idx).
 */
static bool intermediate_parity_writes_done(struct stripe_head *sh,
					    struct stripe_head_state *s)
{
	...
	for_each_intermediate_parity(sh, p, j) {
		struct r5dev *dev = &sh->dev[p];

		/*
		 * A failed slot is implicitly safe — its write was skipped, so
		 * we can't be waiting on it.  A failed disk is exactly one that
		 * is not Insync; checking failed_num[0..1] (a raid6 two-failure
		 * idiom) misses the 3rd+ failure, so an intermediate parity
		 * slot at failed_num[2] would be treated as a still-pending
		 * write and wedge the stripe.
		 */
	...
	__must_hold(&conf->device_lock)
{
	...
	if (test_bit(STRIPE_HANDLE, &sh->state)) {
		...
		if (test_bit(STRIPE_DELAYED, &sh->state) &&
			...
			sh->delayed_since = jiffies;
			atomic64_inc(&conf->delayed_parked);
					 ...
					 int hash)
{
	...
	while (size) {
		...
		if (!list_empty_careful(list)) {
			...
			/*
			 * Mark each stripe ON_INACTIVE_LIST before the splice
			 * so find_get_stripe()'s lockless rescue path can
			 * tell that sh->lru is now on inactive_list[hash]
			 * (protected by hash_locks[hash]) versus still on
			 * temp_inactive_list (protected by device_lock).
			 * Both list states have count==0 and !STRIPE_HANDLE,
			 * so the bit is the only safe distinguisher.
			 */
			list_for_each_entry(sh, list, lru)
				set_bit(STRIPE_ON_INACTIVE_LIST, &sh->state);
...
static struct stripe_head *get_free_stripe(struct r5conf *conf, int hash)
{
	...
	list_del_init(first);
	clear_bit(STRIPE_ON_INACTIVE_LIST, &sh->state);
	remove_hash(sh);
		...
		sector_t sector, short generation, int hash, int dcl_group)
{
	...
	/*
	 * Fast rescue: count is zero AND STRIPE_ON_INACTIVE_LIST is set,
	 * which means sh->lru is sitting on inactive_list[hash].  That
	 * bucket is protected by hash_locks[hash], which the caller
	 * already holds, so we can unlink without taking device_lock.
	 *
	 * The bit is set in release_inactive_stripe_list() right before
	 * the splice from temp_inactive_list and cleared by both
	 * get_free_stripe() and the unlink below — both under
	 * hash_locks[hash], so the read here is consistent with the
	 * actual list location.
	 *
	 * Earlier (commit 0706ca7, reverted in dc17c72) this fast path
	 * relied only on count==0 + !STRIPE_HANDLE + !STRIPE_EXPANDING,
	 * which doesn't disambiguate inactive_list[hash] from the
	 * temp_inactive_list staging area still owned by device_lock.
	 * That race tripped __list_add_valid_or_report under load.
	 * The flag is what avoids that race.
	 */
	if (test_bit(STRIPE_ON_INACTIVE_LIST, &sh->state) &&
	    !atomic_read(&sh->count)) {
		atomic_inc(&conf->active_stripes);
		if (list_is_singular(conf->inactive_list + hash))
			atomic_inc(&conf->empty_inactive_list_nr);
	...
	/*
	 * Slow path. The reference count is zero which means the stripe
	 * must be on a list (sh->lru) other than inactive_list[hash] —
	 * handle_list, delayed_list, r5c lists, sync_pool, or still on
	 * temp_inactive_list under device_lock.  All of those need
	 * device_lock to safely unlink.
	 */
...
struct stripe_request_ctx {
	 ...
	 * sized by raid5_create_ctx_pool() for the array's largest bio, plus
	 * one for an unaligned one
	 */
		...
		int dcl_group, unsigned int flags)
{
	...
	for (;;) {
		...
		/*
		 * A full-row write owns this stripe's row and is writing its
		 * members underneath the cache: wait for it, as for a quiesce
		 * (raidkm_row_write_prep walks the row under this lock).
		 */
		if (unlikely(raidkm_row_wlocked(conf, sector, dcl_group))) {
			...
			raidkm_row_wwant(conf, sector);
			row_waited = true;
			wait_event_lock_irq(conf->row_write_wait,
					    !raidkm_row_wlocked(conf, sector,
								dcl_group),
					    *(conf->hash_locks + hash));
			continue;
		}
	...
	/* we waited for a row write and hold a stripe of the row now: a row
	 * write finds it in use from here, so the grace can go */
	if (unlikely(row_waited) && sh)
		raidkm_row_wgot(conf, sector);
	spin_unlock_irq(conf->hash_locks + hash);
	return sh;
}
...
{
	int data_disks = sh->disks - raidkm_sh_m(sh);

	BUG_ON(sh->overwrite_disks > data_disks);
	return sh->overwrite_disks == data_disks;
}
...
static bool stripe_can_batch(struct stripe_head *sh)
{
	...
	if (raid5_has_log(conf) || raid5_has_ppl(conf))
		return false;
	/* raid_km redesign: with the phase 4 routing fixes, batching is
	 * correctness-safe for m > 2 — but measurement on md-kmec-rhel10
	 * (brd ramdisks, no GFNI) shows it's 1.8x SLOWER (727 vs 1307 MiB/s
	 * on 7-disk seq write 256K iodepth=8).  Our m > 2 encode path is
	 * synchronous (no async_tx / DMA pipelining benefit), so batching
	 * only adds lock_two_stripes contention and batch_head bookkeeping
	 * with no compute parallelism to offset it.  Revisit if/when an
	 * async EC path lands.
	 *
	 ...
	 * operation, not just while degraded.
	 *
	 * The real-device A/B this comment used to wait for was run on
	 * 2026-09-21 (8+3 over NVMe, k held at 8 so k x chunk stays a power of
	 * two; same rig, same build, CPU platform flipped):
	 *
	 *   GFNI (Ice Lake)    seq write 2651 batched vs 2649 unbatched MiB/s,
	 *                      members 125.6 vs 14.8 KiB -- batching is FREE,
	 *                      and 8+3 matches 8+2 (2655-2738 MiB/s)
	 *   no GFNI (Cascade)  seq write 1416 vs 2050 MiB/s -- batching costs
	 *                      31%, and 8+3 runs at ~54% of 8+2
	 *
	 * So the cost is a property of the scalar EC path, not of batching:
	 * where the encode is fast the request size comes for nothing.  Hence
	 * the knob (sysfs rk_batch_mparity) defaults to Y. */
		...
		struct stripe_head *sh, struct stripe_head *last_sh)
{
	...
	dd_idx = 0;
	while (is_parity_disk(sh, dd_idx))
		dd_idx++;
...
/*
 * Native checksum, demand-paged (P3, redesigned).  The reserved region holds one
 * CRC-32C per 4 KiB block, packed RAIDKM_CSUM_PER_PAGE to a page.  A bounded LRU
...
struct raidkm_csum_cache {
	...
	struct workqueue_struct	*wq;	/* shard workers; max_active = nr_shards */
	struct workqueue_struct	*abio_wq;	/* deferred bypass verifies: their
					 * own queue, so a store flood keeping
					 * every shard worker busy cannot starve
					 * them (each holds a read AND a count
					 * raid5_quiesce waits on) */
...
/* --- addressing within a member's region ------------------------------ */
static unsigned long raidkm_csum_blkidx(struct r5conf *conf, sector_t sector)
{
	return sector >> RAID5_STRIPE_SHIFT(conf);
}
static unsigned long raidkm_csum_key(int member, unsigned long blk)
{
	return ((unsigned long)member << 40) | (blk / RAIDKM_CSUM_PER_PAGE);
}
static unsigned int raidkm_csum_slot(unsigned long blk)
{
...
static bool raidkm_csum_page_read(struct r5conf *conf, int m,
				  unsigned long pgidx, struct page *page)
{
	struct md_rdev *rdev = conf->disks[m].rdev;
	sector_t rstart, rsec, off = pgidx * (PAGE_SIZE >> 9);
	bool ok;

	if (!rdev || test_bit(Faulty, &rdev->flags))
		return false;
	/* pin across the (sleeping) region I/O: raid5_remove_disk frees the
	 * rdev once nr_pending drops (same pattern as raid5_read_one_chunk) */
	atomic_inc(&rdev->nr_pending);
	if (!raidkm_csum_region(conf, rdev, &rstart, &rsec) ||
	    off + (PAGE_SIZE >> 9) > rsec) {		/* past region end */
		rdev_dec_pending(rdev, conf->mddev);
		return false;
	}
	ok = sync_page_io(rdev, (rstart - rdev->data_offset) + off,
				   ...
				   struct raidkm_csum_page *pg)
{
	...
	/* pin across the (sleeping) region write -- see raidkm_csum_page_read */
	atomic_inc(&rdev->nr_pending);
	if (!raidkm_csum_region(conf, rdev, &rstart, &rsec) ||
	    off + (PAGE_SIZE >> 9) > rsec) {		/* past region end */
		rdev_dec_pending(rdev, conf->mddev);
...
static void raidkm_csum_page_free(struct raidkm_csum_page *pg)
{
	__free_page(pg->page);
	kfree(pg);
}
...
static bool raidkm_csum_get_slot(struct raidkm_csum_page *pg,
				 unsigned long blk, u32 *crc)
{
	__le32 *crcs;
	u32 c;

	if (test_bit(RAIDKM_CSUM_P_REBUILD, &pg->flags))
		return false;
	crcs = kmap_local_page(pg->page);
	c = le32_to_cpu(crcs[raidkm_csum_slot(blk)]);
	kunmap_local(crcs);
	if (!c)
		return false;				/* never written */
	*crc = (c == RAIDKM_CSUM_ZERO_MARK) ? 0 : c;
	return true;
}

static void raidkm_csum_set_slot(struct raidkm_csum_page *pg,
				 unsigned long blk, u32 crc)
{
	__le32 *crcs = kmap_local_page(pg->page);

	crcs[raidkm_csum_slot(blk)] =
		cpu_to_le32(crc ? crc : RAIDKM_CSUM_ZERO_MARK);
	kunmap_local(crcs);
	clear_bit(RAIDKM_CSUM_P_REBUILD, &pg->flags);
						...
						unsigned long key)
{
	...
	if (conf->csum_disk) {
		void *crcs = page_address(pg->page);
		struct raidkm_csum_trailer *tr = crcs + RAIDKM_CSUM_BODY;

		if (!raidkm_csum_page_read(conf, key >> 40,
					   key & ((1UL << 40) - 1), pg->page)) {
			/* Transient region-I/O failure: do NOT publish a clean
			 * zero page -- it would silently shadow the real
			 * on-region tags, and once a store dirtied it, the
			 * flush would write zeros over up to 1021 good CRCs.
			 * Fail the lookup (verify/store skip this time); the
			 * next fault retries. */
			raidkm_csum_page_free(pg);
			return NULL;
		}
		if (le32_to_cpu(tr->gen) == 0) {
			clear_page(crcs);			/* never written */
		} else if (le32_to_cpu(tr->self_crc) !=
			   crc32c_le(0, crcs, RAIDKM_CSUM_BODY)) {
			pr_warn_ratelimited("md/raid:%s: native csum region page rotted (disk %lu page %lu); slots absent, will re-derive\n",
				mdname(conf->mddev), key >> 40,
				key & ((1UL << 40) - 1));
			clear_page(crcs);
			set_bit(RAIDKM_CSUM_P_REBUILD, &pg->flags);
		}
	} else {
		clear_page(page_address(pg->page));	/* in-core only */
	}
...
static void raidkm_csum_verify_stripe(struct raidkm_csum_shard *s, struct stripe_head *sh)
{
	...
	for (i = 0; i < sh->disks; i++) {
		...
		if (!test_bit(R5_UPTODATE, &dev->flags))
			/* The endio raises the gate BEFORE R5_UPTODATE (so
			 * analyse never sees an unverified UPTODATE block);
			 * we can observe that window via an early queue.  Do
			 * NOT clear the gate -- that endio queues the stripe
			 * again once R5_UPTODATE is visible, and this dev is
		...
		if (rederive) {
			...
			if (got != want) {
				...
				if (!is_parity_disk(sh, i))
					set_bit(R5_IntegrityHeal, &dev->flags);
			}
		}
		/* Lower the gate LAST.  R5_CsumPending is the ONLY fence that
		 * keeps analyse_stripe/handle_stripe (reachable concurrently
		 * via raid5_sync_request's and retry_aligned_read's direct
		 * handle_stripe calls) from consuming this block -- Wantfill
		 * to the reader, or as a reconstruction source -- and the
		 * expected-CRC lookup above may BLOCK on a region fault.
		 * Clearing before the verdict (the old test_and_clear at the
		 * top) opened a multi-ms window serving unverified data.
		 * Same-word bitops order this after the verdict flags. */
		clear_bit(R5_CsumPending, &dev->flags);
	}
	mutex_unlock(&s->lock);
}
	...
	for (;;) {
		...
		xa_for_each(&s->pending, idx, e) {
			...
			if (!--budget)
				 ...
				 * not starve the queued verifies below (each
 ...
 * first so queued stores land), before the conf's rdevs are freed. */
static void raidkm_csum_writeback(struct r5conf *conf)
{
	struct raidkm_csum_cache *c = conf->csum;
	struct raidkm_csum_page *pg;
	unsigned long key;
...
static struct raidkm_csum_cache *raidkm_csum_cache_alloc(struct r5conf *conf)
{
	...
	if (!c->wq)
		goto fail;
	c->abio_wq = alloc_workqueue("raidkm_csum_ab_%s",
...
/* Free the cache + workqueue + every resident page.  Called at free_conf. */
static void raidkm_csum_cache_free(struct r5conf *conf)
{
	struct raidkm_csum_cache *c = conf->csum;
	struct raidkm_csum_page *pg;
	unsigned long key;
	...
	if (!c)
		return;
	if (c->abio_wq)
		destroy_workqueue(c->abio_wq);		/* drains bypass verifies */
	if (c->wq)
...
static void ops_run_io(struct stripe_head *sh, struct stripe_head_state *s)
{
	...
	/*
	 * Deferred, stripe-sector-ordered submission (sysfs rk_bio_sort):
	 *
	 *   RK_BIO_SORT_WRITES  upstream's rule — writes only, so a foreground
	 *                       read never waits for a flush.  Measured on
	 *                       null_blk: a rebuild's spare writes go from 7.3
	 *                       to 74.9 KiB and the rebuild runs 13% faster,
	 *                       but healthy sequential writes lose 35%, since
	 *                       they were already merging at 127 KiB and only
	 *                       pay the extra latency.
	 *   RK_BIO_SORT_SYNC    order the resync/recovery stream ONLY, reads
	 *                       included: that stream has no foreground caller
	 *                       waiting on it, a whole window is in flight, and
	 *                       its order is what decides the request size the
	 *                       drive sees while rebuilding.  Foreground I/O
	 *                       keeps immediate submission.
	 */
	sort_mode = READ_ONCE(conf->batch_bio_dispatch);
	if (!conf->group_cnt)
		sort_mode = RK_BIO_SORT_OFF;
	else if (sort_mode == RK_BIO_SORT_SYNC &&
		 !test_bit(STRIPE_SYNCING, &head_sh->state))
		sort_mode = RK_BIO_SORT_OFF;
	should_defer = sort_mode != RK_BIO_SORT_OFF;
	defer_reads = sort_mode == RK_BIO_SORT_SYNC;
...
again:
		...
		if (rdev) {
			...
			if (should_defer && (op_is_write(op) || defer_reads))
				...
				int srctype)
{
	...
	/*
	 * raid_km redesign: emit the sources in LOGICAL order — data index d
	 * at srcs[d], parity index p at srcs[k+p] — reading each from its
	 * physical slot via raidkm_data_slot()/raidkm_parity_slot().  Every
	 * m>2 consumer (isal_gen_syndrome_m, isal_update_syndrome_m,
	 * ops_run_check_pq) walks srcs[] in logical order, so this one mapping
	 * makes them all layout-agnostic; for PARITY_N the slot equals the
	 * index and this reduces to the original tail-parity placement.  Skip
	 * the raid6 helpers (raid6_d0/raid6_idx_to_slot) which only know the
	 * two-parity rotation.  Returns k (data count).
	 */
	if (is_raidkm(conf)) {
		...
		for (i = 0; i < disks; i++)
			srcs[i] = NULL;
		for (d = 0; d < k; d++) {
			int slot = raidkm_data_slot(sh, d);
			struct r5dev *dev = &sh->dev[slot];
			if ((srctype == SYNDROME_SRC_ALL) ||
		...
		for (p = 0; p < m; p++) {
			int slot = raidkm_parity_slot(sh, p);
			srcs[k + p] = sh->dev[slot].page;
			offs[k + p] = sh->dev[slot].offset;
		}
		return k;
	}
...
/*
 * raid_km redesign: generic Reed-Solomon reconstruction for raidkm (any m).
 * Reconstructs every slot flagged R5_Wantcompute from the k surviving
 * uptodate slots via gf_invert_matrix — the in-kernel twin of the userspace
 * EC verifier's decode.  Handles 1..m missing slots in one pass and respects
 * the fixed PARITY_N layout, unlike ops_run_compute6_{1,2} (P+Q only, <=2
 * targets, rotating-layout slot math).
 *
 * Synchronous, like ops_run_reconstruct6's m > 2 path; returns NULL.
 * ec_encode_data_* / *_gfni self-wrap kernel_fpu — do NOT wrap here.
 */
/*
 * Per-cpu scratch for the synchronous decode in ops_run_compute_km, kept off
 * the kernel stack: at RAIDKM_MAX_STRIPE_DISKS=32 these arrays total ~3 KiB,
 * which pushed the stack frame past the 2 KiB warning limit.  Allocated once
 * per cpu in alloc_scratch_buffer(); safe to share per-cpu because
 * ops_run_compute_km runs under conf->percpu->lock (no concurrent use per cpu).
 */
struct raidkm_decode_scratch {
	unsigned char b[RAIDKM_MAX_STRIPE_DISKS * RAIDKM_MAX_STRIPE_DISKS];
		   ...
		   unsigned long *ops_request)
{
	...
	for (i = 0; i < disks; i++)
		if (test_bit(R5_Wantcompute, &sh->dev[i].flags)) {
			if (nmiss < m)
				miss[nmiss] = i;
			nmiss++;
		}
	if (WARN_ON_ONCE(nmiss == 0 || nmiss > m ||
			 k > RAIDKM_MAX_STRIPE_DISKS))
		goto complete;

	/* k surviving uptodate slots to decode from.  fetch_block only
	 * requests this compute once at least k survivors are UPTODATE. */
	for (i = 0; i < disks && nsrc < k; i++)
		if (!test_bit(R5_Wantcompute, &sh->dev[i].flags) &&
		    test_bit(R5_UPTODATE, &sh->dev[i].flags))
			srcsl[nsrc++] = i;
	if (nsrc < k) {
		/*
		 * A decode source vanished between fetch_block requesting this
		 * compute and it running here — typically a surviving disk
		 * failed mid-flight (the array gained a failure), so a slot that
		 * was an UPTODATE survivor is no longer usable.  This is an
		 * expected transient across a failure transition, not a bug.
		 * Abandon the compute WITHOUT marking the targets uptodate
		 * (claiming success on un-computed blocks would corrupt them),
		 * drop the compute request, and let the stripe be re-handled:
		 * md has already flagged the newly-failed member, so the next
		 * pass reconstructs from a valid survivor set.  The write path's
		 * schedule_reconstruction defer keeps the write held off until
		 * the failed data slots are genuinely reconstructed.
		 */
		for (i = 0; i < disks; i++)
	...
	/* decode matrix = generator rows of the k survivors, then invert.
	 * The matrix is indexed by LOGICAL row (raidkm_matrix_row maps each
	 * physical survivor slot to its data/parity generator row), so the
	 * inverse recovers the k logical data values regardless of layout. */
	for (i = 0; i < k; i++) {
		int row = raidkm_matrix_row(sh, srcsl[i]);
		unsigned char *amat = raidkm_a_matrix(sh);
		for (j = 0; j < k; j++)
			b[i * k + j] = amat[row * k + j];
		src[i] = page_address(sh->dev[srcsl[i]].page) +
			 sh->dev[srcsl[i]].offset;
	}
	if (WARN_ON_ONCE(gf_invert_matrix(b, inv, k) < 0))
		goto complete;	/* singular: survivor set not independent */

	/* reconstruct each missing DATA slot one row at a time: the decode
	 * coefficients for logical data index d are inv[d].  One row per call
	 * keeps the table buffer a fixed k*32 bytes regardless of m. */
	for (p = 0; p < nmiss; p++) {
		int s = miss[p];
		int d;
		unsigned char *d1[1];

		if (is_parity_disk(sh, s)) {
			have_parity_miss = true;
			continue;
		}
		d = raidkm_data_index(sh, s);
		for (j = 0; j < k; j++)
			row1[j] = inv[d * k + j];
		d1[0] = page_address(sh->dev[s].page) + sh->dev[s].offset;
		if (isal_have_avx512_gfni()) {
			ec_init_tables_gfni(k, 1, row1, dtbls1);
			ec_encode_data_avx512_gfni(bytes, k, 1, dtbls1, src, d1);
		} else if (isal_have_gfni()) {
			ec_init_tables_gfni(k, 1, row1, dtbls1);
			ec_encode_data_avx2_gfni(bytes, k, 1, dtbls1, src, d1);
		} else {
			ec_init_tables_base(k, 1, row1, dtbls1);
			ec_encode_data_base(bytes, k, 1, dtbls1, src, d1);
		}
	}
	...
	if (have_parity_miss) {
		...
		for (p = 0; p < nmiss; p++) {
			int s = miss[p];
			int pj;			/* parity index of slot s */
			size_t toff;
			unsigned char *d1[1];

			if (!is_parity_disk(sh, s))
				continue;
			pj = (s - sh->pd_idx + disks) % disks;
			toff = (size_t)pj * k * 32;
			d1[0] = page_address(sh->dev[s].page) + sh->dev[s].offset;
			if (isal_have_avx512_gfni())
				ec_encode_data_avx512_gfni(bytes, k, 1,
					raidkm_g_gfni(sh) + toff, data, d1);
			else if (isal_have_gfni())
				ec_encode_data_avx2_gfni(bytes, k, 1,
					raidkm_g_gfni(sh) + toff, data, d1);
			else
				ec_encode_data_base(bytes, k, 1,
					raidkm_g_base(sh) + toff, data, d1);
		}
	}

	/* Mark all reconstructed slots uptodate.  ops_complete_compute clears
	 * R5_Wantcompute on target/target2 (and BUG_ONs if it's already gone),
	 * so leave those two be and clear the flag on any extra slots here. */
	for (i = 0; i < disks; i++) {
		if (!test_bit(R5_Wantcompute, &sh->dev[i].flags))
			continue;
		set_bit(R5_UPTODATE, &sh->dev[i].flags);
		if (i != sh->ops.target && i != sh->ops.target2)
			clear_bit(R5_Wantcompute, &sh->dev[i].flags);
	}
...
/* Forward declarations: the ISA-L helpers live next to
 * ops_run_reconstruct6 (where they're primarily used) but
 * ops_run_prexor6 needs the update helper too. */
static void isal_update_syndrome_m(struct stripe_head *sh,
		...
		struct dma_async_tx_descriptor *tx)
{
	...
	if (is_raidkm(conf) && conf->m > 2) {
		/*
		 * raid_km redesign: m > 2 prexor uses ec_encode_data_update
		 * to fold the OLD data contribution out of parity in place.
		 * Synchronous — but if any predecessor tx (e.g. a compute
		 * op) is still in flight, the .page contents we read could
		 * be stale.  Quiesce first.  Returns NULL so the caller's
		 * biodrain doesn't try to chain off a stale tx.
		 */
		if (tx)
			async_tx_quiesce(&tx);
		isal_update_syndrome_m(sh, blocks, offs, count,
	...
	/*
	 * Stock raid6 P+Q path — also used by raidkm at m == 2.  raidkm's
	 * set_syndrome_sources lays data at blocks[i] (coeff 2^i) with P at
	 * blocks[k], Q at blocks[k+1] in PARITY_N order, which is byte-for-
	 * byte identical to ISA-L's m=2 encode (proven: same GF(2^8)/0x11d,
	 * gen=2, P=XOR, Q=Sum 2^i.D_i).  So the m=2 grow-to-m=3 prefix
	 * property is preserved even though the fast SIMD engine computes it.
	 */
...
static void ops_complete_reconstruct(void *stripe_head_ref)
{
	...
	for (i = disks; i--; ) {
		...
		if (dev->written || is_parity_disk(sh, i)) {
...
/*
 * raid_km redesign: synchronous EC encode for m > 2.  Bypasses the
 * async_tx framework because raid6_call.gen_syndrome only knows P+Q
 * math; we go directly to ISA-L's ec_encode_data_*_gfni using the
 * conf->ec_g_tbls built at create-time.
 *
 * Layout contract from set_syndrome_sources (m > 2 path):
 *   blocks[0..k-1]   = data source pages
 *   blocks[k..k+m-1] = parity destination pages
 *
 * NULL entries in the data range stand in for the "empty zero page"
 * (matches what async_pq's do_sync_gen_syndrome does — see
 * lib/raid6/async_pq.c).  bytes is the per-disk transfer size,
 * always RAID5_STRIPE_SIZE.
 *
 * No async_tx descriptor is returned.  Caller is expected to invoke
				...
				int k, size_t bytes)
{
	...
	for (i = 0; i < k; i++)
		src[i] = blocks[i]
			? (unsigned char *)page_address(blocks[i]) + offs[i]
			: (unsigned char *)raid6_get_zero_page(); /* RHEL 10.2 port fix #2 */
	for (i = 0; i < m; i++)
		dst[i] = (unsigned char *)page_address(blocks[k + i]) + offs[k + i];

	/* ec_encode_data_avx{2,512}_gfni self-wrap kernel_fpu_begin/end
	 * (see isa-l/gfni_glue.c).  ec_encode_data_base is plain C.  Do
	 * NOT wrap again here — nested kernel_fpu_begin corrupts the
	 * preempt count and panics on the first encode. */
	if (isal_have_avx512_gfni())
		ec_encode_data_avx512_gfni(bytes, k, m, raidkm_g_gfni(sh),
					   src, dst);
	else if (isal_have_gfni())
		ec_encode_data_avx2_gfni(bytes, k, m, raidkm_g_gfni(sh),
					 src, dst);
	else
		ec_encode_data_base(bytes, k, m, raidkm_g_base(sh),
				    src, dst);
}

/*
 * raid_km redesign: incremental parity update for m > 2 RMW.
 *
 * Used twice in the RMW dance:
 *   - Prexor pass (srctype = SYNDROME_SRC_WANT_DRAIN): the data slots
 *     about to be overwritten still hold the OLD payload on-disk and
 *     in sh->dev[i].page.  Folding that contribution into the existing
 *     parity buffers XOR-cancels it (in GF(2), add == sub).
 *   - Post-biodrain reconstruct pass (srctype = SYNDROME_SRC_WRITTEN):
 *     the same slots now hold the NEW payload (biodrain copied bio
 *     data into .page).  Folding that contribution in re-adds it.
 *   The net effect across both passes is exactly the parity delta —
 *   parity_after = parity_before XOR g[r][i] * (new XOR old).
 *
 * Layout contract (same as isal_gen_syndrome_m): set_syndrome_sources
 * for m > 2 puts data sources at blocks[0..k-1] (NULL for inactive
 * slots) and parity destinations at blocks[k..k+m-1].
 *
 * ec_encode_data_update_*_gfni self-wrap kernel_fpu — do NOT wrap.
 */
static void isal_update_syndrome_m(struct stripe_head *sh,
				   struct page **blocks, unsigned int *offs,
				   int k, size_t bytes)
{
	unsigned char *coding[RAIDKM_MAX_M];
	int m = raidkm_sh_m(sh);
	int i;

	if (WARN_ON_ONCE(m > RAIDKM_MAX_M))
		return;

	for (i = 0; i < m; i++)
		coding[i] = (unsigned char *)page_address(blocks[k + i]) +
			    offs[k + i];
	...
	for (i = 0; i < k; i++) {
		...
		if (!blocks[i])
			continue;
		src = (unsigned char *)page_address(blocks[i]) + offs[i];

		if (isal_have_avx512_gfni())
			ec_encode_data_update_avx512_gfni(bytes, k, m, i,
							  raidkm_g_gfni(sh),
							  src, coding);
		else if (isal_have_gfni())
			ec_encode_data_update_avx2_gfni(bytes, k, m, i,
							raidkm_g_gfni(sh),
							src, coding);
		else
			ec_encode_data_update_base(bytes, k, m, i,
						   raidkm_g_base(sh),
						   src, coding);
	}
}
...
/*
 * raid_km redesign: compute all m partial parities for a PPL-logged
 * partial-stripe write into sh->ppl_pages[0..m-1].  Synchronous (called
 * from ops_run_partial_parity after the caller has quiesced any tx).
 *
 * Closing the write hole for arbitrary m means logging ALL m partial
 * parities, not just the single XOR parity raid5 PPL logs.
 *
 * RMW (reconstruct_state_prexor_drain_run): ops_run_prexor6 has already
 *   folded the OLD data being overwritten out of the parity dev pages, so
 *   each parity page already holds parity-over-unmodified-data == the
 *   partial parity.  Just copy them (m==2 P+Q via async_gen_syndrome's
 *   PQ_XOR_DST, m>2 via isal_update_syndrome_m — both leave PP in place).
 * RCW (reconstruct_state_drain_run): the not-overwritten data slots were
 *   read and are R5_UPTODATE; the slots being written are not UPTODATE yet
 *   (biodrain runs after this op).  Encode the UPTODATE data slots, in
 *   their natural disk positions so the GF coefficients match, into the m
 *   PP pages.
 *
 * PPL requires PAGE_SIZE == 4096 == DEFAULT_STRIPE_SIZE, so dev/ppl pages
 * are full pages at offset 0 and RAID5_STRIPE_SIZE == PAGE_SIZE.
 */
void raidkm_compute_partial_parity(struct stripe_head *sh)
{
	...
	if (sh->reconstruct_state == reconstruct_state_prexor_drain_run) {
		/* parity index j lives at physical slot raidkm_parity_slot() */
		for (j = 0; j < m; j++)
			memcpy(page_address(sh->ppl_pages[j]),
			       page_address(sh->dev[raidkm_parity_slot(sh, j)].page),
			       bytes);
		return;
	}
	...
	if (m == 2) {
		/*
		 * P+Q over the UPTODATE (not-overwritten) data via raid6_call;
		 * zero-page fill for non-contributing slots preserves the 2^i
		 * coefficient by logical position.  gen_syndrome overwrites the
		 * dst.  Feed data in LOGICAL order (index i), reading each from
		 * its physical slot, so the positional coefficient is correct
		 * under either layout.
		 */
		void *ptrs[RAIDKM_MAX_STRIPE_DISKS];

		for (i = 0; i < k; i++) {
			int slot = raidkm_data_slot(sh, i);
			ptrs[i] = test_bit(R5_UPTODATE, &sh->dev[slot].flags)
				? page_address(sh->dev[slot].page)
				: (void *)raid6_get_zero_page(); /* RHEL 10.2 port fix #2 */
		}
		ptrs[k]     = page_address(sh->ppl_pages[0]);
		ptrs[k + 1] = page_address(sh->ppl_pages[1]);
		raid6_call.gen_syndrome(k + 2, bytes, ptrs);
		return;
	}

	/*
	 * m > 2: accumulate each UPTODATE data slot into the zeroed PP pages
	 * with ISA-L ec_encode_data_update (applies the per-column coeff).
	 * The column index is the LOGICAL data index i; read the page from
	 * its physical slot.
	 */
	{
		unsigned char *cod[RAIDKM_MAX_M];

		for (j = 0; j < m; j++) {
			cod[j] = page_address(sh->ppl_pages[j]);
			memset(cod[j], 0, bytes);
		}
		for (i = 0; i < k; i++) {
			int slot = raidkm_data_slot(sh, i);
			unsigned char *src;

			if (!test_bit(R5_UPTODATE, &sh->dev[slot].flags))
				continue;
			src = page_address(sh->dev[slot].page);
			if (isal_have_avx512_gfni())
				ec_encode_data_update_avx512_gfni(bytes, k, m, i,
						raidkm_g_gfni(sh), src, cod);
			else if (isal_have_gfni())
				ec_encode_data_update_avx2_gfni(bytes, k, m, i,
						raidkm_g_gfni(sh), src, cod);
			else
				ec_encode_data_update_base(bytes, k, m, i,
						raidkm_g_base(sh), src, cod);
		}
	}
}

/*
 * raid_km redesign: PPL recovery encode.  src[0..k-1] point to the modified
 * data read back from disk (the raid6_empty_zero_page address for positions
 * not modified by the logged write); cod[0..m-1] receive encode_j(modified
 * data).  The caller XORs in the stored partial parities afterwards
 * (P_j = PP_j ^ encode_j(modified) — GF add is XOR for every parity).
 *
 * Runs once during assembly, so the plain-C base path is fine (no GFNI).
 */
void raidkm_ppl_encode_modified(struct r5conf *conf, void **src,
				void **cod, size_t bytes)
{
	int m = conf->m;
	int k = conf->raid_disks - conf->max_degraded;
	...
	if (m == 2) {
		...
		/*
		 * PPL recovery runs at assembly against the current geometry
		 * (no reshape region distinction here), so use the current
		 * base tables directly.
		 */
		ec_encode_data_base(bytes, k, m, conf->ec_g_tbls_base,
				    (unsigned char **)src, (unsigned char **)cod);
	}
}
		     ...
		     struct dma_async_tx_descriptor *tx)
{
	...
	if (i >= sh->disks) {
		...
		for_each_intermediate_parity(sh, p, j)
			set_bit(R5_Discard, &sh->dev[p].flags);
...
again:
	...
	if (is_raidkm(sh->raid_conf) && sh->raid_conf->m > 2) {
		/*
		 * raid_km redesign: bypass async_gen_syndrome (raid6_call
		 * P+Q only) for m > 2.  Two dispatch shapes:
		 *   SYNDROME_SRC_ALL    : full-stripe write / RCW — encode
		 *                         all parity from scratch.
		 *   SYNDROME_SRC_WRITTEN: post-biodrain RMW pass — fold the
		 *                         NEW data contribution into the
		 *                         parity that ops_run_prexor6
		 *                         already had the OLD contribution
		 *                         folded out of.
		 *
		 * tx may carry an in-flight biodrain (async_copy_data) that
		 * hasn't finished writing the NEW data into .page yet.
		 * Quiesce before reading, otherwise we'd encode stale bytes
		 * and the parity would not match a fresh full-stripe encode.
		 */
		if (tx)
			async_tx_quiesce(&tx);
		if (synflags == SYNDROME_SRC_WRITTEN)
			isal_update_syndrome_m(sh, blocks, offs,
					       count,
					       RAID5_STRIPE_SIZE(sh->raid_conf));
		else
...
static void ops_run_check_pq(struct stripe_head *sh, struct raid5_percpu *percpu, int checkp)
{
	...
	if (is_raidkm(conf) && conf->m > 2) {
		/*
		 * raid_km redesign: m > 2 scrub.  set_syndrome_sources put the
		 * data pages at srcs[0..k-1] and the on-disk parity at
		 * srcs[k..k+m-1].  Re-encode each parity row from the data into
		 * percpu->spare_page (one row at a time — the per-row table is
		 * already at conf->ec_g_tbls[p*k*32]) and compare to what's on
		 * disk.  In repair mode (not MD_RECOVERY_CHECK) fix any mismatch
		 * in place; handle_parity_checks6 then marks the corrected slots
		 * R5_Wantwrite.  zero_sum_result carries a per-parity mismatch
		 * bitmask (bit p == parity slot k+p).
		 *
		 * Synchronous, like ops_run_reconstruct6's m > 2 path.
		 * ec_encode_data_*_gfni self-wrap kernel_fpu — do NOT wrap.
		 */
		...
		if (!WARN_ON_ONCE(k > RAIDKM_MAX_STRIPE_DISKS || m > RAIDKM_MAX_M)) {
			...
			for (p = 0; p < m; p++) {
				...
				if (isal_have_avx512_gfni())
					ec_encode_data_avx512_gfni(bytes, k, 1,
						raidkm_g_gfni(sh) + toff, data, dst);
				else if (isal_have_gfni())
					ec_encode_data_avx2_gfni(bytes, k, 1,
						raidkm_g_gfni(sh) + toff, data, dst);
				else
...
static void raid_run_ops(struct stripe_head *sh, unsigned long ops_request)
{
	...
	struct dma_async_tx_descriptor *tx = NULL;
	struct r5conf *conf = sh->raid_conf;
	int level = conf->effective_level;
	struct raid5_percpu *percpu;
...
static void free_stripe(struct kmem_cache *sc, struct stripe_head *sh)
{
	...
	{
		int _j;
		for (_j = 0; _j < RAIDKM_MAX_M; _j++)
			if (sh->ppl_pages[_j])
				__free_page(sh->ppl_pages[_j]);
	}
	...
	int disks, struct r5conf *conf)
{
	...
	if (sh) {
		...
		if (raid5_has_ppl(conf)) {
			/* one partial-parity page per parity (m); raid5 has m==1 */
			int np = conf->m > 0 ? conf->m : 1;
			int j;
			for (j = 0; j < np; j++) {
				sh->ppl_pages[j] = alloc_page(gfp);
				if (!sh->ppl_pages[j]) {
					free_stripe(sc, sh);
					return NULL;
				}
...
static void raid5_end_read_request(struct bio * bi)
{
	...
	if (!bi->bi_status) {
		/*
		 * Native checksum: decide the verify BEFORE R5_UPTODATE becomes
		 * visible.  analyse_stripe gates R5_Wantfill on !R5_CsumPending,
		 * so if an unverified block were published UPTODATE first, a
		 * concurrently running handle_stripe (raid5_sync_request and
		 * retry_aligned_read call it directly) could hand the bytes to
		 * the reader -- or feed them to a reconstruction -- in the
		 * window before the gate went up.  Raising the gate first
		 * closes that: both bits live in dev->flags, so same-word
		 * coherence orders the two set_bit()s for every observer.
		 *
		 * The verify itself: lock-free expected-CRC lookup
		 * (raidkm_csum_peek: pending xarray, then RCU page-slot read) +
		 * crc32c in place -- nothing sleeps, legal in completion
		 * context (dm-integrity 'I' mode verifies in end_io the same
		 * way).  A resident tag that matches needs NO gate and no
		 * hops: the hot path.  Only a non-resident page (the fetch
		 * could block) or an inline mismatch defers to the shard
		 * worker, which re-checks under the shard lock (a fast-path
		 * mismatch may be a benign race) before a confirmed mismatch
		 * feeds the R5_ReadError/R5_IntegrityHeal heal.  Data AND
		 * parity are covered; a SYNCING stripe always defers (the
		 * worker owns the rederive-on-resync refresh).  R5_InJournal
		 * prexor reads land in orig_page, not dev->page, so they are
		 * excluded rather than verified against the wrong buffer.
		 * The worker is queued AFTER R5_UPTODATE below.
		 */
		if (conf->csum &&
		 ...
		 */
		/*
		 * Native checksum: the gate was raised before R5_UPTODATE
		 * above; queue the deferred verify only now, AFTER R5_UPTODATE
		 * is visible -- the worker skips (without lowering the gate) a
		 * gated dev that is not yet UPTODATE, relying on this queue to
		 * re-run it.
		 */
		if (conf->csum &&
		    test_bit(R5_CsumPending, &sh->dev[i].flags))
...
static void raid5_error(struct mddev *mddev, struct md_rdev *rdev)
{
	 ...
	 * failure with the primary alive is dropped there).  3b: park
	 * whenever the assignment table has room; arming waits for any
	 * running population (-EBUSY, retried every raid5d pass) so
	 * sequential failures populate one after the other. */
	if (conf->dcl && conf->dcl_auto &&
	    !test_bit(MD_BROKEN, &conf->mddev->flags) &&
	    conf->nreb < (int)conf->dcl->s && conf->reb_want < 0 &&
	    rdev->raid_disk >= 0)
			      ...
			      struct stripe_head *sh)
{
	...
	int raid_disks = previous ? conf->previous_raid_disks
				  : conf->raid_disks;
	/*
	 * data_disks is this geometry's parity count subtracted from its disk
	 * count.  An add-parity reshape changes the parity count, so the
	 * previous geometry uses prev_m (== m for grow-data and every non-raidkm
	 * level, where max_degraded == m too).
	 */
	int data_disks = raid_disks - (previous ? conf->prev_m : conf->m);
	...
	*dd_idx = sector_div(stripe, data_disks);
	 ...
	 * Switch on effective_level so raid_km (level 71) takes the
	 * level-6 path without needing a separate case.
	 *
	 * raid_km redesign: for m > 2 we use a fixed ALGORITHM_PARITY_N
	 * style layout — data at disk indices [0, raid_disks - m),
	 * parity at [raid_disks - m, raid_disks).  pd_idx and qd_idx
	 * encode the first and last parity disks respectively;
	 * intermediate parity indices are pd_idx+1 .. qd_idx-1.  This
	 * dodges all the rotating-layout complexity in raid5/raid6.
	 */
	pd_idx = qd_idx = -1;
	if (is_raidkm(conf)) {
		 ...
		 * index d lands at slot (pd_idx + m + d) mod N.
		 *
		 *   PARITY_N : pd_idx == data_disks (fixed) -> parity at the
		 *              tail [k, N), data slot == data index.
		 *   rotating : pd_idx rotates one disk leftward per stripe
		 *              (generalized left-symmetric), spreading parity
		 *              and reads across all members.
		 *
		 * The qd_idx / dd_idx formulas below are identical for both;
		 * only the pd_idx choice differs, so the rest of the personality
		 * is layout-agnostic via raidkm_data_slot()/raidkm_parity_slot().
		 */
		if (conf->rotating)
			pd_idx = raid_disks - 1 - sector_div(stripe2, raid_disks);
		else
			pd_idx = data_disks;
		qd_idx = (pd_idx + m - 1) % raid_disks;
		*dd_idx = (pd_idx + m + *dd_idx) % raid_disks;
		goto out_pq;
	}
	switch(conf->effective_level) {
	case 4:
		pd_idx = data_disks;
		break;
	case 5:
...
sector_t raid5_compute_blocknr(struct stripe_head *sh, int i, int previous)
{
	...
	/* previous geometry uses prev_m (see raid5_compute_sector); the inverse
	 * map must agree with the forward map's data-disk count. */
	int data_disks = raid_disks - (previous ? conf->prev_m : conf->m);
	...
	if (i == sh->pd_idx)
		return 0;
	/*
	 * raid_km redesign: invert the slot mapping.  rel is the slot's
	 * position relative to pd_idx (mod N): the first m positions are
	 * parity (return 0), the rest are data carrying logical index
	 * rel - m.  This single modular form covers PARITY_N (pd_idx == k,
	 * no wrap) and rotating (pd_idx varies, parity block may wrap).
	 */
	if (is_raidkm(conf)) {
			 ...
			 int rcw, int expand)
{
	...
	if (rcw) {
		...
		if (s->locked + raidkm_sh_m(sh) == disks)
	...
	if (level == 6) {
		int p, j;
		/* raidkm m > 2: lock the intermediate parity slots too.
		 * Empty loop for stock raid6 (qd_idx == pd_idx + 1). */
		for_each_intermediate_parity(sh, p, j) {
			set_bit(R5_LOCKED, &sh->dev[p].flags);
			clear_bit(R5_UPTODATE, &sh->dev[p].flags);
			s->locked++;
		}
	...
	if (raid5_has_ppl(sh->raid_conf) && sh->ppl_pages[0] &&
				...
				int dd_idx, int forwrite)
{
	...
	/*
	 * A row rebuild owns this stripe's whole chunk: it is about to write
	 * the rebuilding member's copy of it from decoded data, and a write
	 * landing in between would either be lost under that copy or race it.
	 * Bounce it the way an overlapping write is bounced — the caller waits
	 * on R5_Overlap and retries, and the rebuild wakes it when the chunk
	 * is done.  Reads are unaffected: they reconstruct from the survivors
	 * exactly as they do while the member is missing.
	 *
	 * With native checksum on, a READ is not passive: its member reads
	 * come back R5_CsumPending, the shard worker verifies them, and a
	 * mismatch turns into a heal REWRITE (R5_IntegrityHeal) that
	 * handle_stripe issues internally — never through this function, so
	 * the write bounce above would not catch it.  That rewrite would land
	 * in the middle of the rebuild's survivor reads.  So bounce reads too
	 * while a rebuild owns the chunk: they wait one chunk and retry.
	 */
	if ((forwrite || conf->csum) && test_bit(STRIPE_ROW_SYNC, &sh->state))
		return true;
			     ...
			     int dd_idx, int forwrite, int previous)
{
	...
	*bip = bi;
	...
	if (conf->mddev->bitmap && firstwrite) {
		...
		spin_unlock_irq(&sh->stripe_lock);
		raidkm_bitmap_startwrite(conf->mddev, sh->sector,
					RAID5_STRIPE_SECTORS(conf));
		spin_lock_irq(&sh->stripe_lock);
		     ...
		     struct stripe_head_state *s, int disks)
{
	...
	for (i = disks; i--; ) {
		...
		if (bitmap_end && conf->mddev->bitmap)
			raidkm_bitmap_endwrite(conf->mddev,
		   ...
		   struct stripe_head_state *s)
{
	...
	/*
	 * Declustered population: the address being populated cannot be
	 * reconstructed — more members failed here than parity covers, on top
	 * of the one being rebuilt.  This completion never reaches
	 * raidkm_dcl_pop_done(), so the mark can never pass it: left alone,
	 * population would wait for it forever (raidkm_dcl_pop_admit) or, as it
	 * once did, re-scan forever.  Feeding the mark instead would publish a
	 * spare block nobody wrote.  So stop: pause population (raid5d will not
	 * re-request it) and end this pass.  The next pass that starts — an
	 * explicit repair, or the resume at assemble — IS the retry and lifts
	 * the pause (raidkm_dcl_populate_request); if the address still fails,
	 * this pauses it again.  Reads of it keep failing as they did before
	 * population started.
	 */
	if (conf->dcl && READ_ONCE(conf->reb_pop) >= 0 &&
	    test_bit(MD_RECOVERY_REQUESTED, &conf->mddev->recovery) &&
	    !test_bit(MD_RECOVERY_RECOVER, &conf->mddev->recovery) &&
	    raidkm_dcl_pop_slot(conf, sh) >= 0) {
		WRITE_ONCE(conf->reb_fail_sector, sh->sector);	/* sysfs only */
		WRITE_ONCE(conf->reb_pop_stuck, true);
		pr_err_ratelimited("md/raid:%s: declustered: population cannot reconstruct sector %llu — more members failed there than parity covers, so that data is lost; population paused (a repair retries it, which helps only if the failure was transient)\n",
				   mdname(conf->mddev),
				   (unsigned long long)sh->sector);
		abort = 1;	/* ok = 0: INTR ends the pass */
	}
			   ...
			   int disk_idx, int disks)
{
	...
	/* If we want to read from a failed device, then we need to actually
	 * read every other device.  raidkm m > 2 can have up to RAIDKM_MAX_M
	 * failed slots, and analyse_stripe records them high-slot-first — so a
	 * failed DATA slot being read can sit at failed_num[2] behind a failed
	 * parity slot.  Check all failed slots, not just the first two, or the
	 * survivors never get scheduled for read and the stripe deadlocks. */
	for (i = 0; i < s->failed && i < RAIDKM_MAX_M; i++)
		if (sh->dev[s->failed_num[i]].toread)
			return 1;
	...
	if (is_raidkm(sh->raid_conf) && s->failed >= 2) {
		/*
		 * Degraded write on a raidkm array with >= 2 failures.  To
		 * recompute parity we need every data slot's current value, so
		 * any failed DATA slot that isn't being fully overwritten must
		 * be reconstructed, which in turn needs k surviving slots read
		 * as decode sources.  The raid6 fallback below is built for P+Q
		 * (it only inspects failed_num[0..1] and feeds a P/Q compute),
		 * which can't express raidkm's generic decode — so it fails to
		 * schedule the survivor reads (and, for > 2 failures, to flag
		 * the 3rd+ failed data slot) and the writer wedges or, when the
		 * decode runs short of sources, silently corrupts the stripe.
		 * Handle >= 2 failures generically here for every raidkm m
		 * (including m == 2), ahead of the PREREAD_ACTIVE gate so the
		 * sources are actually fetched.  1 failure stays on the stock
		 * raid6 compute path.
		 */
		bool overwriting = dev->towrite &&
				   test_bit(R5_OVERWRITE, &dev->flags);
		bool need_data_reconstruct = false;
		int j;

		/*
		 * Only force survivor reads when a failed DATA slot actually
		 * has to be rebuilt — i.e. it is not being fully overwritten,
		 * so its old contents are still needed for the parity encode.
		 * If only parity slots failed (or every failed data slot is
		 * fully overwritten) there is nothing to reconstruct: the
		 * reconstruct-write path encodes fresh parity from the new data
		 * and writes the surviving parity.  Forcing a read here in that
		 * case would lock a surviving parity slot, block
		 * schedule_reconstruction (which needs s->locked == 0), and
		 * spin the stripe forever.
		 */
		for (j = 0; j < s->failed && j < RAIDKM_MAX_M; j++) {
			int fn = s->failed_num[j];

			if (!is_parity_disk(sh, fn) &&
			    !(sh->dev[fn].towrite &&
			      test_bit(R5_OVERWRITE, &sh->dev[fn].flags))) {
				need_data_reconstruct = true;
				break;
			}
		}
		if (!need_data_reconstruct)
			return 0;

		/* A surviving source we don't have: read its current (pre-write)
		 * value.  Read it even when this slot is itself being fully
		 * overwritten — the failed data slot is decoded from k surviving
		 * slots, and if too few parity survive (small m, or many failed
		 * parity) the overwritten data survivors are needed to reach k.
		 * Their OLD value is the decode source; the biodrain that
		 * installs the NEW data runs only after the decode completes
		 * (held off by the defer in handle_stripe_dirtying), so reading
		 * here is safe. */
		if (test_bit(R5_Insync, &dev->flags))
			return 1;
		/* a failed DATA slot whose old contents we still need (it is
		 * not being fully overwritten): reconstruct it — fetch_block
		 * decodes it once the survivors are in */
		if (!test_bit(R5_Insync, &dev->flags) &&
		    !is_parity_disk(sh, disk_idx) && !overwriting)
			return 1;
		/* failed parity, or a slot being fully overwritten: nothing */
		return 0;
	}
		       ...
		       int disk_idx, int disks)
{
	...
	if (need_this_block(sh, s, disk_idx, disks)) {
		...
		if (is_raidkm(sh->raid_conf)) {
			/*
			 * raid_km redesign (Phase 6): generic RS decode.  If the
			 * needed block is on a failed disk and all surviving
			 * slots are now uptodate, reconstruct every missing slot
			 * in one ops_run_compute_km pass.  Otherwise read this
			 * block if it is an available (Insync) survivor; if it's
			 * a failed block whose survivors aren't all in yet, wait
			 * — the survivor reads are scheduled by other fetch_block
			 * calls (need_this_block returns 1 for them once we want
			 * to read from a failed device).
			 */
			if (!test_bit(R5_Insync, &dev->flags) && s->failed &&
			    s->failed <= raidkm_sh_m(sh)) {
				int k = disks - raidkm_sh_m(sh);
				int t = -1, t2 = -1, cnt = 0, j;
				int avail = 0;

				/* Generic RS decode needs k independent survivors to
				 * invert the generator matrix.  Count the actual decode
				 * sources — slots that are UPTODATE and not themselves
				 * awaiting compute (s->uptodate can't be used: it is
				 * inflated by blocks this same handle_stripe pass has
				 * just flagged Wantcompute or queued to write).  If
				 * fewer than k are in yet, fall through and read the
				 * survivors; a later pass computes once enough arrive. */
				for (j = 0; j < disks; j++)
				...
				if (avail >= k) {
					...
					if (cnt) {
						pr_debug("Computing(km) stripe %llu: %d missing data block(s)\n",
							 (unsigned long long)sh->sector, cnt);
				...
				/* not enough survivors yet, or only parity missing
				 * here — fall through to read this survivor */
			}
			if (test_bit(R5_Insync, &dev->flags)) {
				set_bit(R5_LOCKED, &dev->flags);
		...
		if ((s->uptodate == disks - 1) &&
			...
			BUG_ON(other < 0);
			/* Upstream 52e4324935be ("md/raid5: skip 2-failure compute
			 * when other disk is R5_LOCKED"): with skip_copy on a
...
returnbi:
				...
				if (conf->mddev->bitmap)
					raidkm_bitmap_endwrite(
						conf->mddev, sh->sector,
						RAID5_STRIPE_SECTORS(conf),
	...
	if (!discard_pending &&
		...
		/* raidkm m > 2: also clear the intermediate parity slots,
		 * otherwise R5_Discard stays set and the next stripe_alloc
		 * misroutes.  Empty loop for stock raid5/6. */
		for_each_intermediate_parity(sh, p, j) {
			clear_bit(R5_Discard, &sh->dev[p].flags);
			clear_bit(R5_UPTODATE, &sh->dev[p].flags);
		}
		if (sh->qd_idx >= 0) {
...
/*
 * Does this stripe carry a write whose submitter asked not to be plugged?
 * Only called once a pass has decided to delay the stripe.
 */
static bool raidkm_stripe_has_sync_write(struct stripe_head *sh, int disks)
{
	bool found = false;
	int i;

	spin_lock_irq(&sh->stripe_lock);
	for (i = 0; i < disks && !found; i++) {
		struct bio *bi;

		/* a bio spanning several devs sits on several chains with one
		 * bi_next: walk this dev's part only, as every other reader
		 * of towrite does */
		for (bi = sh->dev[i].towrite;
		     bi && bi->bi_iter.bi_sector <
			   sh->dev[i].sector + RAID5_STRIPE_SECTORS(sh->raid_conf);
		     bi = r5_next_bio(sh->raid_conf, bi, sh->dev[i].sector))
				  ...
				  int disks)
{
	...
	/*
	 * This pass decided to delay: it has prereads to do and no preread
	 * token.  make_stripe_request() gives a stripe that token when a
	 * REQ_SYNC write is added, so that write skips the plugging delay --
	 * but it is one token per stripe, taken with test_and_set, and it is
	 * spent when the FIRST write is issued.  A second write that landed on
	 * the stripe in between took nothing, and would be parked here on
	 * delayed_list, which only drains once no preread is active anywhere:
	 * not while the load that caused this lasts.  So give a stripe that
	 * still carries a REQ_SYNC write the token it should have had, and let
	 * it be handled again rather than parked -- the next pass has the token
	 * and issues the reads.  Checked only on this path, so a workload
	 * without REQ_SYNC writes never pays for the scan.
	 */
	if (READ_ONCE(sync_write_nodelay) &&
	    test_bit(STRIPE_DELAYED, &sh->state) &&
	    !test_bit(STRIPE_PREREAD_ACTIVE, &sh->state) &&
	    raidkm_stripe_has_sync_write(sh, disks) &&
	    !test_and_set_bit(STRIPE_PREREAD_ACTIVE, &sh->state)) {
		atomic_inc(&conf->preread_active_stripes);
		atomic64_inc(&conf->sync_token_regrant);
		clear_bit(STRIPE_DELAYED, &sh->state);
		set_bit(STRIPE_HANDLE, &sh->state);
	}
	 ...
	 */
	/*
	 * raidkm degraded write with >= 2 failures: a failed DATA slot that
	 * isn't being fully overwritten must be reconstructed (from the
	 * surviving slots) before the parity re-encode, because the encode
	 * needs its current value.  The synchronous ops_run_compute_km decode
	 * reads its sources from the UPTODATE surviving slots — but
	 * schedule_reconstruction clears R5_UPTODATE on every parity slot
	 * (including the surviving parity the decode relies on).  If we let the
	 * write start in the same pass the decode bails with nsrc < k and
	 * silently corrupts the stripe.  Hold the write off until every such
	 * failed data slot is UPTODATE (reconstructed).
	 *
	 * The gate matches need_this_block(): only there do we proactively
	 * schedule the survivor reads that feed the decode.  At 1 failure
	 * reconstruction rides the stock raid6 compute path (survivor reads are
	 * PREREAD-gated), and deferring here would wait on reads that never get
	 * scheduled — a deadlock.
	 */
	if (is_raidkm(conf) && s->failed >= 2) {
		int kk = disks - raidkm_sh_m(sh), di;

		for (di = 0; di < kk; di++) {
			struct r5dev *d = &sh->dev[raidkm_data_slot(sh, di)];
			bool ow = d->towrite &&
				  test_bit(R5_OVERWRITE, &d->flags);

			if (!test_bit(R5_Insync, &d->flags) &&
			    !test_bit(R5_UPTODATE, &d->flags) && !ow)
				return 0;
		}
	}
				  ...
				  int disks)
{
	...
	/*
	 * raidkm recovery / hot-replace (ALL m, including m == 2): reconstruct a
	 * rebuilding slot with the layout-aware EC path.  This MUST cover m == 2
	 * too — falling through to the stock raid6 recovery below mis-rebuilds the
	 * parity slots of a rotating-layout array (stock geometry != raidkm's
	 * rotation), silently corrupting parity on exactly the stripes where the
	 * rebuilt disk is a P/Q slot.  The m == 2 *scrub* still uses the stock
	 * path further down (it is correct there); only recovery is pulled up.
	 */
	if (is_raidkm(conf)) {
		...
		/*
		 * Recovery / hot-replace detection.  A slot is being *rebuilt*
		 * (not merely failed) when its member is present and not faulty
		 * but not yet in-sync — i.e. a spare md is syncing into the slot.
		...
		if (recovering || pop >= 0) {
			 ...
			 * Data slots are decoded by the fill path
			 * (gf_invert_matrix) and are UPTODATE by now; parity
			 * slots are re-encoded here from the complete data.  The
			 * surviving (in-sync) slots are left untouched — they
			 * already hold correct data on disk.  ops_run_io writes
			 * the recovering members (and skips any still-faulty
			 * slot) once we mark them R5_Wantwrite.
			 */
			...
			if (s->failed > raidkm_sh_m(sh)) {
				set_bit(STRIPE_INSYNC, &sh->state);
				return;
			}
			/* every data slot must be present to re-encode parity;
			 * wait for the fill path to finish decoding a recovering
			 * data slot if it hasn't yet.  Data lives at logical
			 * order; map index -> physical slot. */
			for (i = 0; i < kk; i++)
				if (test_bit(R5_UPTODATE,
					     &sh->dev[raidkm_data_slot(sh, i)].flags))
					ndata++;
			if (ndata < kk)
				return;
			...
			for (i = 0; i < disks; i++) {
				...
				if (is_parity_disk(sh, i) && !test_bit(R5_UPTODATE,
							 &sh->dev[i].flags)) {
					/* recovering parity: re-encode this slot's
					 * parity row (its index relative to pd_idx)
					 * from the data into the slot's page. */
					int pj = (i - sh->pd_idx + disks) % disks;
					size_t toff = (size_t)pj * kk * 32;
					unsigned char *dst[1] = {
						page_address(sh->dev[i].page) +
						sh->dev[i].offset };

					if (isal_have_avx512_gfni())
						ec_encode_data_avx512_gfni(bytes, kk, 1,
							raidkm_g_gfni(sh) + toff,
							data, dst);
					else if (isal_have_gfni())
						ec_encode_data_avx2_gfni(bytes, kk, 1,
							raidkm_g_gfni(sh) + toff,
							data, dst);
					else
						ec_encode_data_base(bytes, kk, 1,
							raidkm_g_base(sh) + toff,
							data, dst);
					set_bit(R5_UPTODATE, &sh->dev[i].flags);
				}
				if (!test_bit(R5_UPTODATE, &sh->dev[i].flags))
					continue;	/* data slot not decodable */
	...
	if (is_raidkm(conf) && conf->m > 2) {
		/*
		 * raid_km redesign: m > 2 scrub.  The verify/repair compute is
		 * a synchronous ISA-L re-encode in ops_run_check_pq, which sets
		 * zero_sum_result to a per-parity mismatch bitmask (bit p ==
		 * parity slot k+p) and, when repairing, fixes the mismatched
		 * parity pages in place.
		 *
		 * Degraded scrub: the fill path (need_this_block reads every
		 * block during sync; fetch_block reconstructs missing DATA via
		 * the Phase 6 decode) has already made all k data slots UPTODATE
		 * by the time we get here (locked == 0, compute done).  So we can
		 * still verify — and repair — the surviving parity while
		 * degraded; ops_run_check_pq skips parity slots whose disk is
		 * failed (their page was never read), and the failed slots simply
		 * aren't written back.
		 */
		switch (sh->check_state) {
		case check_state_idle: {
			...
			/* too many disks gone to reconstruct the data — give up */
			if (s->failed > raidkm_sh_m(sh)) {
				set_bit(STRIPE_INSYNC, &sh->state);
				break;
			}
			/* re-encoding parity needs every data slot present; if the
			 * decode hasn't finished yet, wait for the fill path. */
			for (i = 0; i < kk; i++)
				if (test_bit(R5_UPTODATE,
					     &sh->dev[raidkm_data_slot(sh, i)].flags))
					ndata++;
		...
		case check_state_run_pq:
			break;	/* wait for ops_run_check_pq to complete */
		case check_state_check_result:
			sh->check_state = check_state_idle;
			if (test_bit(STRIPE_INSYNC, &sh->state))
				break;
			...
			if (test_bit(MD_RECOVERY_CHECK, &conf->mddev->recovery)) {
				...
				/* ops_run_check_pq already corrected the mismatched
				 * parity pages; schedule their write-back.  Bit p of
				 * zero_sum_result is parity index p -> physical slot
				 * raidkm_parity_slot(). */
				for (p = 0; p < raidkm_sh_m(sh); p++) {
					struct r5dev *d =
						&sh->dev[raidkm_parity_slot(sh, p)];
...
static void handle_stripe_expansion(struct r5conf *conf, struct stripe_head *sh)
{
	...
	for (i = 0; i < sh->disks; i++)
		if (!is_parity_disk(sh, i)) {
			...
			/* destination is ready once every DATA slot has been
			 * copied in; skip all parity slots (m for raidkm — the
			 * old pd_idx/qd_idx test missed the m>2 intermediates). */
			for (j = 0; j < conf->raid_disks; j++)
				if (!is_parity_disk(sh2, j) &&
...
static void analyse_stripe(struct stripe_head *sh, struct stripe_head_state *s)
{
	...
	for (i=disks; i--; ) {
		...
		if (test_bit(R5_UPTODATE, &dev->flags) && dev->toread &&
		    !test_bit(STRIPE_BIOFILL_RUN, &sh->state) &&
		    !test_bit(R5_CsumPending, &dev->flags))
			set_bit(R5_Wantfill, &dev->flags);

		/* native csum: a block read from disk but not yet verified holds
		 * the whole stripe -- handle_stripe queues the verify workqueue and
		 * returns until it clears, so we only ever act on verified blocks. */
		if (test_bit(R5_CsumPending, &dev->flags))
			s->csum_pending++;
		...
		if (test_bit(R5_Wantcompute, &dev->flags)) {
			s->compute++;
			/* raidkm m > 2 can reconstruct up to m blocks at once;
			 * raid4/5/6 cap at max_degraded (1/1/2). */
...
static void handle_stripe(struct stripe_head *sh)
{
	...
	/*
	 * Native checksum: if any block was read from disk but not yet verified,
	 * hold the WHOLE stripe.  Queue the csum workqueue (it takes a ref, fetches
	 * the tag -- which may block -- verifies, then re-handles the stripe) and
	 * return without processing.  We never return or reconstruct from an
	 * unverified block, matching load-all's "handle_stripe sees only verified
	 * blocks" guarantee.  STRIPE_HANDLE is intentionally NOT set here -- the
	 * worker sets it after verifying so the stripe re-enters handle_stripe.
	 */
	if (s.csum_pending) {
		raidkm_csum_queue_verify(conf, sh);
		goto finish;
	...
	if (is_raidkm(conf)) {
		/*
		 * raidkm can fail > 2 slots; the raid6 failed_num[0..1] idiom
		 * misses pd/qd when they are the 3rd+ failure (analyse_stripe
		 * records failures high-slot-first, and pd_idx is the lowest
		 * parity slot — so when all parity fails pd lands at
		 * failed_num[2] and is wrongly seen as still-writable, wedging
		 * the clean_event gate below).  A slot is failed iff it is not
		 * Insync.  raidkm always has a qd_idx, so no effective_level
		 * shortcut for q.
		 */
		...
		s.q_failed = (s.failed >= 1 && s.failed_num[0] == sh->qd_idx)
			...
			|| conf->effective_level < 6;
	}

	if (s.written &&
	    intermediate_parity_writes_done(sh, &s) &&
	    (s.p_failed || ((test_bit(R5_Insync, &pdev->flags)
	 ...
	 */
	if (s.failed <= raidkm_sh_m(sh) && !conf->mddev->ro)
...
static void raid5_activate_delayed(struct r5conf *conf)
	__must_hold(&conf->device_lock)
{
	bool drain = atomic_read(&conf->preread_active_stripes) < IO_THRESHOLD;
	unsigned int max_age = READ_ONCE(delayed_max_age_ms);

	/*
	 * The list drains when no preread is active anywhere -- which a
	 * saturating load of REQ_SYNC writes can prevent for as long as it
	 * lasts, and then whatever is parked here starves (measured on a
	 * rebuilding 8+2 under 4 KiB O_DIRECT random writes: submitters blocked
	 * behind a parked write for minutes, released the moment the load
	 * stopped).  So an entry is also activated once it has waited
	 * delayed_max_age_ms: long enough to keep the plugging the delay
	 * exists for, short enough that nothing waits on the load to end.  The
	 * list is FIFO, so the scan stops at the first young entry.  A backstop,
	 * not a deadline -- this runs from raid5d, so an array that goes idle
	 * with something parked waits for whatever next wakes it; the guard that
	 * keeps a sync write off the list in the first place is the fix.
	 */
	while (!list_empty(&conf->delayed_list)) {
		...
		if (!drain) {
			if (!max_age ||
			    time_before(jiffies, sh->delayed_since +
						 msecs_to_jiffies(max_age)))
				break;
			atomic64_inc(&conf->delayed_aged);
		}
		list_del_init(l);
		clear_bit(STRIPE_DELAYED, &sh->state);
...
static bool raidkm_csum_abio_defer(struct r5conf *conf, struct bio *raid_bio)
{
	...
	queue_work(conf->csum->abio_wq, &ab->work);
	return true;
}
...
/*
 * --- native checksum in the row layer ---------------------------------
 *
 * The row layer works outside the stripe cache, so it has to do for itself
 * the two things the stripe path does through ops_run_io() and
 * raidkm_csum_verify_stripe(): CRC-check every block it reads out of a
 * member, and store a CRC for every block it writes.  Without that, turning
 * the row layer on would quietly retire the integrity guarantee on exactly
 * the paths that need it most — a degraded read and a rebuild are where a
 * silently rotted survivor does its damage.
 *
 * Both row paths run in sleepable context — the row-read workqueue and the
 * sync thread — so they call the blocking lookup directly and need none of
 * the -EAGAIN/defer dance raidkm_csum_verify_abio() does from endio.
 *
 * CRCs are keyed by PHYSICAL member.  On a declustered array that is the
 * same disk ops_run_io() keyed the stored CRC with: the row layer runs only
 * while every spare assignment is steady (raidkm_row_dcl_steady()), and in
 * the NONE and POPULATED states raidkm_dcl_redirect() does not look at
 * for_write, so the read map the row layer maps with and the write map the
 * CRC was stored under name the same disk.
 *
 * Spans handed to these helpers are whole CRC blocks by construction: a
 * degraded read rounds its folio span out with raidkm_row_csum_align(), a
 * rebuild covers exactly one chunk (a multiple of the stripe size).
 */

/* the row's folio span must cover whole CRC blocks as well as whole pages */
static inline unsigned int raidkm_row_csum_align(struct r5conf *conf)
{
	if (!conf->csum)
		return RAIDKM_PAGE_SECTORS;
	return max_t(unsigned int, RAIDKM_PAGE_SECTORS,
		     RAID5_STRIPE_SECTORS(conf));
}

static inline u32 raidkm_row_block_crc(struct r5conf *conf, const void *buf)
{
	return raidkm_csum_fold(crc32c_le(0, buf, RAID5_STRIPE_SIZE(conf)));
}

/*
 * Verify [asect, asect + len) of physical member `member`, whose content is
 * already in `buf`.  0 = clean, or nothing stored to check it against (an
 * absent slot means never written); -EBADMSG = a stored CRC disagrees.
 *
 * As on the aligned-read bypass, a mismatch here is NOT trusted and nothing
 * is logged: the caller drops the row to the stripe cache, which re-reads,
 * rechecks under the stripe lock and drives the heal machinery.  That both
 * absorbs a benign race with a write and keeps one warning path for rot.
 */
static int raidkm_row_csum_check(struct r5conf *conf, int member,
				 sector_t asect, const void *buf,
				 unsigned int len)
{
	unsigned long blk = raidkm_csum_blkidx(conf, asect);
	unsigned int bsize = RAID5_STRIPE_SIZE(conf), off;
	struct raidkm_csum_shard *s = NULL;
	int ret = 0;

	if (!conf->csum || raidkm_csum_rederive(conf))
		return 0;		/* resync window: stored CRCs are stale */

	for (off = 0; off < len; off += bsize, blk++) {
		struct raidkm_csum_shard *ns =
			raidkm_csum_shard_for(conf->csum, blk);
		u32 want;

		/* consecutive blocks share a shard until the region page
		 * changes, so this takes the mutex once per region page */
		if (ns != s) {
			if (s)
				mutex_unlock(&s->lock);
			s = ns;
		...
		if (raidkm_csum_expected(s, member, blk, &want) &&
		    want != raidkm_row_block_crc(conf, buf + off)) {
			ret = -EBADMSG;
			break;
		}
	}
...
/*
 * Store the CRCs for a span the row layer is about to write, the way
 * ops_run_io() does at write issue: before the bio goes out, so the cache is
 * authoritative the moment the data can be read back.  Non-faulting (resident
 * slot or the shard's pending xarray), so it is safe anywhere.
 */
static void raidkm_row_csum_store(struct r5conf *conf, int member,
				  sector_t asect, const void *buf,
				  unsigned int len)
{
	unsigned long blk = raidkm_csum_blkidx(conf, asect);
	...
	if (!conf->csum)
		return;
	for (off = 0; off < len; off += bsize, blk++)
		raidkm_csum_store(conf, member, blk,
				  raidkm_row_block_crc(conf, buf + off));
}
 ...
 * declustered layouts in a steady spare state are served here, native
 * checksum included (verified above); anything else (reshape, a live
 * population/copy-back session, a write journal or PPL) falls back to the
 * stripe cache.
 */
...
/* row-wide read: one chunk of the span and where its bytes come from */
struct raidkm_row_chunk {
	struct bio		*bio;
	int			src;	/* index into src[], or -1: the decode */
};
...
static int raidkm_row_decode(struct raidkm_row_read *rr)
{
	...
	if (rr->target >= k) {
		/*
		 * Rebuilding a PARITY column (row rebuild only; a degraded
		 * read never targets one).  The sources are then the k data
		 * columns in order, so there is nothing to invert: the
		 * generator's own row for this parity index encodes it.
		 */
		for (i = 0; i < k; i++)
			if (WARN_ON_ONCE(rr->src[i].row != i)) {
				err = -EINVAL;
				goto out;
			}
		for (j = 0; j < k; j++)
			row1[j] = a[rr->target * k + j];
		goto tables;
	}
	for (i = 0; i < k; i++)
		for (j = 0; j < k; j++)
...
/*
 * Check the survivors we just read against their stored CRCs, before we spend
 * a decode on them.  `conf` is a parameter and not `rr->conf`: the rebuild
 * reuses a per-worker scratch `rr` (raidkm_row_rebuild_band()) that carries
 * only the fields the decode needs, so rr->conf is NULL there.
 */
static int raidkm_row_csum_srcs(struct r5conf *conf,
...
/* row-wide read: copy each chunk from its source, or from the decode */
static void raidkm_row_wide_complete(struct raidkm_row_read *rr)
{
	struct r5conf *conf = rr->conf;
	sector_t cmask = conf->chunk_sectors - 1;
	int c;

	atomic64_inc(&conf->row_dread_wide);
	for (c = 0; c < rr->nchunks; c++) {
		...
		bio_for_each_segment(bv, bio, it) {
			memcpy_to_page(bv.bv_page, bv.bv_offset, p + off,
				       bv.bv_len);
			off += bv.bv_len;
		}
		bio_endio(bio);
...
static void raidkm_row_read_work(struct work_struct *work)
{
	...
	if (rr->status || atomic_read(&rr->bucket->inflight) ||
		...
		goto raced;
	if (raidkm_row_csum_srcs(conf, rr))
		goto csum_bad;
	if (raidkm_row_decode(rr))
		goto raced;
...
csum_bad:
	/*
	 * A write that landed while we were verifying rewrote both the blocks
	 * and their CRCs, so re-read the row generation before calling this
	 * rot: a moved generation is the ordinary race this layer already
	 * expects.  Either way the stripe cache re-reads and rechecks — on a
	 * real mismatch it is the path that heals — this only decides which
	 * counter the row is charged to.
	 */
	if (atomic_read(&rr->bucket->gen) != rr->gen0 ||
	    atomic_read(&rr->bucket->inflight))
		goto raced;
	atomic64_inc(&conf->row_dread_csum_bad);
	goto stripe;
raced:
	atomic64_inc(&conf->row_dread_raced);
stripe:
	/* the stripe cache serves it; retry_aligned_read drops
	 * active_aligned_reads, one per chunk bio */
	if (rr->chunks) {
		for (i = 0; i < rr->nchunks; i++)
			add_bio_to_retry(rr->chunks[i].bio, conf);
	} else {
...
/* issue @rr's k source reads over [asect, asect + len) */
static void raidkm_row_read_submit(struct mddev *mddev,
				   struct raidkm_row_read *rr)
{
	...
	for (i = 0; i < rr->k; i++) {
		...
		bi->bi_iter.bi_sector = rr->asect + rdev->data_offset;
		bio_add_folio_nofail(bi, rr->src[i].folio, rr->len, 0);
...
/* take an active_aligned_reads reference, waiting out a quiesce, as
 * raid5_read_one_chunk does */
static void raidkm_row_aligned_get(struct r5conf *conf)
{
	bool did_inc = false;
...
/*
 * Generator row i (data index i, or k + parity index) -> the PHYSICAL disk
 * that holds it for this row.  Classic: the row's parity starts at slot pd
 * and data index d sits at slot pd+m+d.  Declustered: parity sits at the
 * group tail, so the in-group slot IS i, and the disk is that slot's mapped
 * pool disk after the READ-side redirect chain (same map the aligned-read
 * bypass and the stripe path's endio lookups use).  The index, not the rdev,
 * because it is also the key native checksum stores CRCs under.
 */
static int raidkm_row_member_idx(struct r5conf *conf, sector_t logical,
				 sector_t msect, int pd, int k, int m, int i)
{
	int n = conf->raid_disks;

	if (conf->dcl)
		return raidkm_dcl_read_pdisk(conf, logical, msect, i);
	return i < k ? (pd + m + i) % n : (pd + i - k) % n;
}
...
static int raidkm_row_dread_one(struct mddev *mddev, struct bio *raid_bio)
{
	...
	unsigned int len, order, align;
	struct md_rdev *tgt;
	int dd_idx, pd, target, group = 0, i, nsrc = 0, seq, tgt_idx;
	...
	/* with native csum on, the span must also cover whole CRC blocks, so
	 * that everything read and everything decoded can be checked */
	align = raidkm_row_csum_align(conf);
	a0 = msect & ~((sector_t)align - 1);
	a1 = round_up(msect + bio_sectors(raid_bio), (sector_t)align);
	len = (a1 - a0) << 9;
	if (target >= k)
		return 0;
	...
	tgt_idx = raidkm_row_member_idx(conf, logical, msect, pd, k, m, target);
	tgt = conf->disks[tgt_idx].rdev;
	if (read_seqcount_retry(&conf->gen_lock, seq))
		return 0;
	...
	for (i = 0; i < k + m && nsrc < k; i++) {
		int idx = raidkm_row_member_idx(conf, logical, msect,
						pd, k, m, i);
		struct md_rdev *rdev = conf->disks[idx].rdev;
	...
	raidkm_row_read_submit(mddev, rr);
	return 1;
...
/*
 * Serve a degraded read that spans several chunks of ONE row (declustered: of
 * one group of a row) and misses exactly one of them.  Decoding the missing
 * chunk reads the row's k survivors, and those already hold every healthy
 * data chunk of the row at the same member sectors -- so bypassing the healthy
 * chunks as well, as the per-chunk path does, reads them twice: 1.7x survivor
 * reads at 8+2, which on bandwidth-bound NVMe cost 37% of degraded read
 * throughput (stock md reads each survivor block once).  Read the row once
 * instead, and copy each chunk from its source or from the decode.
 *
 * Two missing chunks in the span would need two decodes; such a bio keeps the
 * per-chunk path.  The span is split per chunk before any I/O, so each piece
 * keeps the one-chunk contract the fallback relies on (retry_aligned_read
 * maps a single chunk) and holds its own active_aligned_reads reference.
 * 0 = not served, bio untouched.
 */
#define RAIDKM_ROW_WIDE_MAX	64	/* chunks per span; a longer bio re-enters */

static int raidkm_row_dread_wide(struct mddev *mddev, struct bio *raid_bio)
{
	struct r5conf *conf = mddev->private;
	unsigned int cs = conf->chunk_sectors, shift = ilog2(cs);
	int n = conf->raid_disks, m = conf->m, k = n - m;
	sector_t start = raid_bio->bi_iter.bi_sector;
	sector_t l0 = (start >> shift) << shift;
	sector_t end, row, rowm = 0, a0 = 0, a1 = 0;
	int dix[RAIDKM_ROW_WIDE_MAX];
	struct raidkm_row_read *rr;
	struct raidkm_row_bucket *b;
	unsigned int align, len, order;
	int nc, c, i, seq, pd = 0, group = 0, target = -1, tc = -1, nsrc = 0;

	if (conf->dcl) {
		k = conf->dcl->k;
		m = conf->dcl->m;
	}
	if (conf->prev_chunk_sectors != cs)
		return 0;
	/* a row's (group's) k data chunks are logically contiguous */
	row = start >> shift;
	end = l0 + ((sector_t)(k - sector_div(row, k)) << shift);
	end = min(end, bio_end_sector(raid_bio));
	nc = min_t(sector_t, DIV_ROUND_UP_SECTOR_T(end - l0, cs),
		   RAIDKM_ROW_WIDE_MAX);
	if (nc < 2)
		return 0;
	end = min(end, l0 + ((sector_t)nc << shift));
	...
	align = raidkm_row_csum_align(conf);
	for (c = 0; c < nc; c++) {
		sector_t l = l0 + ((sector_t)c << shift), ms;
		int dd_idx, cpd, grp = 0, d, idx;

		ms = raid5_compute_sector(conf, l, 0, &dd_idx, NULL);
		if (conf->dcl) {
			cpd = k;
			d = dd_idx;
			grp = raidkm_dcl_group_of(conf, l, 0);
		} else {
			row = ms >> shift;
			cpd = conf->rotating ? n - 1 - (int)sector_div(row, n) : k;
			d = (dd_idx - cpd - m + 2 * n) % n;
		}
		if (c == 0) {
			rowm = ms;
			pd = cpd;
			group = grp;
			a0 = rowm & ~((sector_t)align - 1);
			a1 = round_up(rowm + cs, (sector_t)align);
		} else if (ms != rowm || cpd != pd || grp != group) {
			return 0;	/* not one code word after all */
		}
		if (d >= k)
			return 0;
		dix[c] = d;
		idx = raidkm_row_member_idx(conf, l, ms, pd, k, m, d);
		if (raidkm_row_member_ok(conf->disks[idx].rdev, a0, a1))
			continue;
		if (target >= 0)
			return 0;	/* two decodes: per-chunk path */
		target = d;
		tc = c;
	}
	if (target < 0 || read_seqcount_retry(&conf->gen_lock, seq))
		return 0;
	...
	rr->k = k;
	rr->m = m;
	rr->chunks = kmalloc_array(nc, sizeof(*rr->chunks), GFP_NOIO);
	if (!rr->chunks)
		goto decline;
	for (i = 0; i < k + m && nsrc < k; i++) {
		int idx = raidkm_row_member_idx(conf, l0, rowm, pd, k, m, i);
		struct md_rdev *rdev = conf->disks[idx].rdev;
	...
	if (nsrc < k)
		goto decline;
	/* every healthy chunk of the span must be served by one of the sources */
	for (c = 0; c < nc; c++) {
		rr->chunks[c].bio = NULL;
		rr->chunks[c].src = -1;
		if (c == tc)
			continue;
		for (i = 0; i < k; i++)
			if (rr->src[i].row == dix[c])
				rr->chunks[c].src = i;
		if (rr->chunks[c].src < 0)
			goto decline;
	}

	b = raidkm_row_bucket(conf, rowm, group);
	rr->gen0 = atomic_read(&b->gen);
	if (atomic_read(&b->inflight))
		goto decline;		/* hot row: leave it to the stripe cache */
	...
	/* one reference per chunk bio: each completes, or is retried, alone */
	atomic_add(nc - 1, &conf->active_aligned_reads);

	/* committed: the tail past the span re-enters, the span splits per chunk */
	if (bio_end_sector(raid_bio) > end) {
		struct bio *split = bio_split(raid_bio, end - start, GFP_NOIO,
					      &conf->bio_split);
	...
	for (c = 0; c < nc; c++) {
		struct bio *cb = raid_bio;

		if (c < nc - 1) {
			sector_t cend = l0 + ((sector_t)(c + 1) << shift);

			cb = bio_split(raid_bio,
				       cend - raid_bio->bi_iter.bi_sector,
				       GFP_NOIO, &conf->bio_split);
			bio_chain(cb, raid_bio);
		}
		md_account_bio(mddev, &cb);
		rr->chunks[c].bio = cb;
	}
...
/*
 * Row layer, rebuild.  Recovering a member through the stripe cache reads
 * every survivor 4 KiB at a time and writes the spare 4 KiB at a time, in
 * whatever order the workers finish; on flash with a large indirection unit
 * both ends are below the unit.  Here one chunk — which is exactly one row —
 * is rebuilt as k chunk-sized survivor reads, one decode, and one chunk-sized
 * write to the member being rebuilt.
 *
 * The row's stripes are pinned and marked STRIPE_ROW_SYNC first, so a
 * foreground write to the row bounces on R5_Overlap and retries once the
 * chunk is done (stripe_bio_overlaps); a row with anything already in flight
 * is left to the stripe cache instead of waited for, so this never blocks
 * foreground I/O — it just declines.  Reads are untouched: they reconstruct
 * from the survivors exactly as they do while the member is missing.
 */
/*
 * Per-worker scratch, allocated once per band.  Allocating the k + 1
 * chunk-sized buffers per ROW instead costs more than the I/O does: at 128 KiB
 * they are order-5 pages, and a rebuild runs on a machine whose memory is busy
 * (on these rigs, with the ramdisk under the array), so every row paid
 * compaction.  Reusing one set per worker took the rebuild from 405 to the
...
static void raidkm_row_sync_endio(struct bio *bi)
{
	struct raidkm_row_sync *rs = bi->bi_private;

	if (bi->bi_status)
		rs->status = bi->bi_status;
	bio_put(bi);
	if (atomic_dec_and_test(&rs->pending))
		complete(&rs->done);
}

/*
 * Claim one stripe of a row for a row rebuild: true = it was idle and is now
 * ours (STRIPE_ROW_SYNC set), false = something is using it and it was left
 * alone.  On false the caller
 * drops its reference itself and must NOT hand the stripe to
 * raidkm_row_rebuild_release(), which clears STRIPE_ROW_SYNC on whatever it
 * is given: had the stripe been busy because another claimer marked it, that
 * would clear THEIR claim and let a write into a chunk still being rebuilt.
 */
static bool raidkm_row_claim_stripe(struct r5conf *conf,
				    struct stripe_head *sh)
{
	bool busy = false;
	int i;
	...
	spin_lock_irq(&sh->stripe_lock);
	if (sh->batch_head ||
	    test_bit(STRIPE_ACTIVE, &sh->state) ||
	    test_bit(STRIPE_HANDLE, &sh->state) ||
	    test_bit(STRIPE_SYNCING, &sh->state) ||
	    test_bit(STRIPE_ROW_SYNC, &sh->state) ||
	    test_bit(STRIPE_DELAYED, &sh->state))
		busy = true;
	/*
	 * A stripe waiting on a native-csum verify is not idle even though
	 * nothing is locked: the shard worker can still turn a mismatch into a
	 * heal rewrite, which handle_stripe issues internally and the ROW_SYNC
	 * write bounce therefore never sees.  Leave those rows to the stripe
	 * cache.  Reading csum_list outside its qlock is deliberately
	 * one-sided: a queue we miss is a verify that has not run yet, and the
	 * R5_CsumPending test below still catches it (the worker clears that
	 * flag only after the verdict).
	 */
	if (conf->csum && !list_empty(&sh->csum_list))
		busy = true;
	for (i = 0; !busy && i < sh->disks; i++)
		if (sh->dev[i].toread || sh->dev[i].towrite ||
			...
			busy = true;
	if (!busy)
		set_bit(STRIPE_ROW_SYNC, &sh->state);
	spin_unlock_irq(&sh->stripe_lock);
	return !busy;
}

/* release the row's stripes and let any bounced writer through */
static void raidkm_row_rebuild_release(struct r5conf *conf,
				       struct stripe_head **shs, int nsh)
{
	int j, i;

	for (j = 0; j < nsh; j++) {
		struct stripe_head *sh = shs[j];

		if (!sh)
			continue;
		/*
		 * Under stripe_lock, because stripe_bio_overlaps() tests the
		 * bit under it: otherwise a writer can see ROW_SYNC set, we
		 * can clear it and find no R5_Overlap to wake, and the writer
		 * then sets R5_Overlap and waits for a wakeup that no longer
		 * has anyone to deliver it — an idle stripe is never handled,
		 * so nothing else would clear the bit.
		 */
		spin_lock_irq(&sh->stripe_lock);
		if (test_and_clear_bit(STRIPE_ROW_SYNC, &sh->state))
			for (i = 0; i < sh->disks; i++)
				if (test_and_clear_bit(R5_Overlap,
						       &sh->dev[i].flags))
					wake_up_bit(&sh->dev[i].flags,
						    R5_Overlap);
		spin_unlock_irq(&sh->stripe_lock);
		raid5_release_stripe(sh);
	}
	/*
	 * A bounced read does not wait on R5_Overlap: retry_aligned_read()
	 * parks the bio in conf->retry_read_aligned and raid5d breaks out of
	 * its loop, so the wake_up_bit above reaches nobody and the bio waits
	 * for whatever happens to poke raid5d next.  Poke it here — only
	 * reads on a native-csum array bounce at all (stripe_bio_overlaps),
	 * and this costs nothing when none did.
	 */
	md_wakeup_thread(conf->mddev->thread);
}

/*
 * Rebuild one chunk-aligned row onto @target.  1 = done (the caller may
 * claim conf->chunk_sectors of progress), 0 = declined, do it the old way,
 * -EBUSY = the row could not be claimed right now (a stripe in use, or none
 * free to take): nothing was read or written, the same call may succeed a
 * moment later, and the CALLER counts the decline if it gives up.
 */
static int raidkm_row_rebuild_chunk(struct mddev *mddev, sector_t sector_nr,
				    struct md_rdev *target, int target_slot,
				    struct raidkm_row_rb_ctx *ctx)
{
	struct r5conf *conf = mddev->private;
	int n = conf->raid_disks, m = conf->m, k = n - m;
	unsigned int chunk_sectors = conf->chunk_sectors;
	unsigned int len = chunk_sectors << 9;
	unsigned int nsh = chunk_sectors / RAID5_STRIPE_SECTORS(conf);
	struct stripe_head **shs = ctx->shs;
	struct raidkm_row_read *rr = ctx->rr;
	struct raidkm_row_sync rs;
	struct blk_plug plug;
	sector_t row = sector_nr;
	bool pinned_target = false, csum_rejected = false;
	int pd, tgt_row, i, j, nsrc = 0, ret = 0;

	if (chunk_sectors % RAID5_STRIPE_SECTORS(conf) || nsh == 0)
		return 0;
	memset(shs, 0, nsh * sizeof(*shs));

	/* which generator row does the member being rebuilt hold for this
	 * row?  Data index d sits at slot pd+m+d, parity index p at pd+p. */
	sector_div(row, chunk_sectors);
	pd = conf->rotating ? n - 1 - (int)sector_div(row, n) : k;
	tgt_row = (target_slot - pd - m + 2 * n) % n;
	if (tgt_row >= k)		/* a parity slot: rows k.. in generator
					 * order, sources are the k data slots */
		tgt_row = k + ((target_slot - pd + n) % n);
	if (tgt_row >= k + m)
		return 0;

	/* NB: rr->conf and rr->raid_bio stay NULL here — this scratch rr only
	 * carries what the decode and the CRC check need.  Anything reached
	 * from this path must take conf as an argument. */
	rr->k = k;
	rr->m = m;
	rr->target = tgt_row;
	rr->asect = sector_nr;
	...
	/* sources: the k lowest generator rows that are not the target and
	 * whose member is usable.  A parity target needs the k data rows, in
	 * order, which is what this produces (rows 0..k-1 come first). */
	for (i = 0; i < k + m && nsrc < k; i++) {
		...
		if (!rdev || test_bit(Faulty, &rdev->flags) ||
		    !test_bit(In_sync, &rdev->flags) ||
		    rdev_has_badblock(rdev, sector_nr, chunk_sectors))
			continue;
		atomic_inc(&rdev->nr_pending);
		rr->src[nsrc].rdev = rdev;
		rr->src[nsrc].row = i;
		rr->src[nsrc].member = slot;
		nsrc++;
	}
	if (nsrc < k)
		goto out_sources;
	if (rdev_has_badblock(target, sector_nr, chunk_sectors))
		goto out_sources;
	/*
	 * Pin the member being rebuilt for the duration of the write.  It is
	 * by definition NOT In_sync, and raid5_remove_disk() only refuses a
	 * removal for In_sync or pinned members — without this a concurrent
	 * --remove, or a fault plus remove_and_add_spares(), frees the rdev
	 * and closes its bdev while our chunk write is in flight.
	 */
	atomic_inc(&target->nr_pending);
	pinned_target = true;

	/* own the row: every stripe must be idle, or we leave the row alone */
	for (j = 0; j < nsh; j++) {
		struct stripe_head *sh;

		sh = raid5_get_active_stripe(conf, NULL,
					     sector_nr + j * RAID5_STRIPE_SECTORS(conf),
					     0, R5_GAS_NOBLOCK |
					     R5_GAS_NOQUIESCE);
		if (!sh) {
			ret = -EBUSY;
		...
		if (!raidkm_row_claim_stripe(conf, sh)) {
			/* not ours: never hand it to the release */
			raid5_release_stripe(sh);
			ret = -EBUSY;
			goto out_stripes;
		}
	...
	blk_finish_plug(&plug);
	wait_for_completion_io(&rs.done);
	if (rs.status)
		goto out_stripes;
	/*
	 * Native csum: rebuilding from a silently rotted survivor writes the
	 * rot onto the spare and makes it permanent, so check every survivor
	 * before decoding from it.  A row that fails goes to the stripe cache,
	 * whose verify re-reads it, warns and drives the heal.
	 */
	if (raidkm_row_csum_srcs(conf, rr))
		goto out_csum;
	if (raidkm_row_decode(rr))
		goto out_stripes;
	/* publish the CRCs for the chunk, as ops_run_io() does at write issue.
...
out_csum:
	atomic64_inc(&conf->row_rebuild_csum_bad);
	csum_rejected = true;		/* counted here, not as a plain decline */
out_stripes:
	raidkm_row_rebuild_release(conf, shs, nsh);
out_sources:
	for (i = 0; i < nsrc; i++)
		rdev_dec_pending(rr->src[i].rdev, mddev);
	if (pinned_target)
		rdev_dec_pending(target, mddev);
	if (!ret && !csum_rejected)
		atomic64_inc(&conf->row_rebuild_declined);
	return ret;
}

/*
 * Declustered population, one chunk-aligned row (sysfs rk_dcl_row_rebuild).
 *
 * The stripe path writes the spare column in RAID5_STRIPE_SIZE pieces: on
 * real NVMe that measured 6.2 KiB average against the classic row rebuild's
 * 128.0 KiB (notes/md-kmec/row-x-declustered-audit-plan-2026-09-19.md §H4).
 * This is the classic writer's mechanics — own the row, k chunk-sized
 * survivor reads, verify, one decode, one chunk-sized write — over the
 * DECLUSTERED map:
 *
 *   root   = the group column whose write-chain traverses X in this row
 *            (raidkm_dcl_chain_root; -1 = content-free, nothing to rebuild)
 *   tslot  = root % g IS the generator row: slots 0..k-1 are data indices,
 *            k..g-1 parity indices, and all the rotation lives in dcl_disk()
 *   target = the WRITE-side redirect of root's disk, i.e. the spare column
 *            this row's content belongs on
 *
 * The engine OWNS the map here rather than reading it: the mark it resolves
 * against is the one this pass advances, and the caller publishes a row's
 * granules only after the chunk write has completed.  That is why population
 * can be driven this way while a concurrent degraded row READ still has to
 * wait for assignments to be steady (raidkm_row_dcl_steady, H1).
 *
 * 1 = done (the caller may claim the whole row), RK_DCL_ROW_EMPTY = no chain
 * traverses X in this row, so there is nothing to rebuild and the caller may
 * claim it just the same, 0 = declined (leave the row to the stripe path),
 * -EBUSY = the row could not be claimed right now: nothing was read or written
 * and the same call may succeed a moment later.
 */
#define RK_DCL_ROW_EMPTY	2
static int raidkm_dcl_rebuild_chunk(struct mddev *mddev, sector_t sector_nr,
				    int X, struct raidkm_row_rb_ctx *ctx)
{
	struct r5conf *conf = mddev->private;
	unsigned int chunk_sectors = conf->chunk_sectors;
	unsigned int len = chunk_sectors << 9;
	...
	sector_t row = sector_nr;
	bool pinned_target = false, csum_rejected = false;
	int k, g, group, root, tslot, tdisk;
	int i, j, nsrc = 0, ret = 0;
	...
	k = (int)ge->k;
	g = (int)ge->g;
	if (rr->k != k)			/* buffers built for another geometry */
		return 0;
	root = raidkm_dcl_chain_root(conf, row, X);
	if (root < 0)
		return RK_DCL_ROW_EMPTY;	/* no chain traverses X here:
						 * nothing to rebuild, and the
						 * caller still feeds the mark */
	group = root / g;
	tslot = root % g;

	tdisk = raidkm_dcl_redirect(conf, dcl_disk(ge, row, (u32)root), row, true);
	if (tdisk < 0 || tdisk >= conf->raid_disks || tdisk == X)
		return 0;		/* no live endpoint to write: decode row */
	target = conf->disks[tdisk].rdev;
	if (!target || test_bit(Faulty, &target->flags) ||
	    conf->disks[tdisk].replacement)
		return 0;

	/* NB: rr->conf and rr->raid_bio stay NULL here — this scratch rr only
	 * carries what the decode and the CRC check need. */
	rr->m = (int)ge->m;
	rr->target = tslot;
	rr->asect = sector_nr;
	rr->len = len;
	...
	/*
	 * Sources: the group's other columns, in slot order, so the k data
	 * rows come first — which is what raidkm_row_decode() requires when
	 * the target is a PARITY column (it encodes from the k data rows and
	 * has nothing to invert).  A column whose read-side redirect dead-ends
	 * on X, or lands on a member that is gone, is simply not a source:
	 * g - 1 candidates for k needed leaves m - 1 of them spare.
	 */
	...
	/* own the row: every stripe of THIS group must be idle */
	for (j = 0; j < nsh; j++) {
		struct stripe_head *sh;

		sh = raid5_get_active_stripe(conf, NULL,
					     sector_nr + j * RAID5_STRIPE_SECTORS(conf),
					     group, R5_GAS_NOBLOCK |
					     R5_GAS_NOQUIESCE);
		if (!sh) {
			ret = -EBUSY;
			goto out_stripes;
		...
		if (!raidkm_row_claim_stripe(conf, sh)) {
			/* not ours: never hand it to the release (it would
			 * clear another claimer's STRIPE_ROW_SYNC) */
			raid5_release_stripe(sh);
			ret = -EBUSY;
			goto out_stripes;
		}
	...
	if (rs.status)
		goto out_stripes;
	/* a silently rotted survivor would be made permanent on the spare */
	if (raidkm_row_csum_srcs(conf, rr))
		goto out_csum;
	if (raidkm_row_decode(rr))
		goto out_stripes;
	/* CRCs are keyed by the PHYSICAL disk the content lands on, which for
	 * a redirected column is the spare — the same key the stripe path's
	 * population write would have stored under. */
	raidkm_row_csum_store(conf, tdisk, sector_nr, rr->out_addr, len);

	/* one chunk-sized write to the spare column */
	{
		struct bio *bi = bio_alloc_bioset(target->bdev,
						  raidkm_row_buf_nr_vecs(&ctx->buf[k]),
						  REQ_OP_WRITE | REQ_SYNC,
 ...
 */
/* how far a pacing deadline may trail "now" before it is pulled up */
#define RK_ROW_PACE_CREDIT	(HZ / 10)
			       ...
			       unsigned long started)
{
	...
	if (kb_per_sec <= 0 || !did) {
		WRITE_ONCE(conf->row_rb_pace_owed, false);
		return;
	}
	 ...
	 */
	/*
	 * A deadline that has fallen behind is only pulled up to within a
	 * bounded credit of "now", not to "now".  Every sleep overshoots a
	 * little (it is in jiffies), so the next band starts past the deadline;
	 * snapping the deadline forward each time threw that overshoot away
	 * instead of letting the next band repay it, and the rebuild ran at 144
	 * MB/s under a 200 MB/s ceiling (HRT, X4000) while a 50 MB/s one, with
	 * a fifth as many sleeps, reached 47.  The bound keeps an array that
	 * was idle or slow from bursting far past the ceiling afterwards.
	 */
	due = conf->row_rb_pace_due;
	if (time_before(due + RK_ROW_PACE_CREDIT, started))
		due = started - RK_ROW_PACE_CREDIT;
	...
	conf->row_rb_pace_due = due;
	WRITE_ONCE(conf->row_rb_pace_owed, true);
}

/*
 * Pay the pacing debt the previous step ran up -- at the START of the next
 * step, not at the end of that one.  A row step reports its progress with
 * md_done_sync() before raid5_sync_request() returns, and md adds the step to
 * recovery_active only after it has returned, so between the two the counter
 * is NEGATIVE.  status_resync() reads that as "more in flight than the cursor
 * has covered" and prints the start-of-sync position, and a pacer that sleeps
 * right there spends nearly all of its time in that state: /proc/mdstat read
 * position 1 in 14 of 16 and 16 of 16 samples on HRT's paced runs.  Here the
 * previous step is fully accounted, so the position is the true cursor.
 */
static void raidkm_row_rb_pace_wait(struct mddev *mddev)
{
	struct r5conf *conf = mddev->private;
	unsigned long due = conf->row_rb_pace_due;

	if (!READ_ONCE(conf->row_rb_pace_owed))
		return;
	WRITE_ONCE(conf->row_rb_pace_owed, false);
	/* the debt is at most one band's budget: a deadline left behind by a
	 * pass that ended, or by the knob being turned off, must not be read as
	 * "sleep until jiffies wraps back round to it" */
	if (time_after(due, jiffies + 5 * HZ))
		return;
 ...
 * the same shape the declustered copy-back band uses.  Rows are independent:
 * each owns its own stripes, buffers and member I/O.
 *
 * Progress is claimed for the longest PREFIX of rows that succeeded, so a row
 * that declined (busy, badblock, allocation failure) simply ends the band and
 ...
 */
/* rows in flight.  16, not 8: on HRT's X4000 it rebuilt a third faster under
 * a sequential read (899 / 912 against 655-709 MB/s) at no cost to the read,
...
/*
 * How far the engine may rebuild ahead of md's cursor, in chunks: the default
 * for sysfs rk_row_rebuild_window, 8 MiB at a 128 KiB chunk.  It bounds work
 * that an interrupted pass throws away and the burst md's throttle lets
 * through, not memory -- a chunk holds buffers only while a worker has it, so
 * what is in flight stays at the worker count whatever the window.  0 selects
 * the synchronous bands above.
 */
...
#define RK_ROW_WIN_BUSY_USEC	250

struct raidkm_row_rb_band {
	struct mddev		*mddev;
	struct md_rdev		*target;
	int			target_slot;
	/* declustered population: the target is per ROW (this row's spare
	 * column), so the band carries the populating disk instead and the
	 * writer resolves the rest -- see raidkm_dcl_rebuild_chunk() */
	bool			dcl;
	int			dcl_X;
	sector_t		start;
	unsigned int		nrows;
...
struct raidkm_row_rb_worker {
	struct work_struct		work;
	struct raidkm_row_rb_band	*band;	/* a synchronous band, or ... */
	struct raidkm_row_rb_set	*set;	/* ... the look-ahead window */
	struct raidkm_row_rb_ctx	ctx;
};

static void raidkm_row_rebuild_worker(struct work_struct *work)
{
	struct raidkm_row_rb_worker *w =
		container_of(work, struct raidkm_row_rb_worker, work);
	struct raidkm_row_rb_band *b = w->band;
	...
	while ((i = (unsigned int)atomic_fetch_inc(&b->next)) < b->nrows &&
		...
		sector_t sect = b->start + (sector_t)i * conf->chunk_sectors;
		int fb, ret;

		ret = b->dcl ?
			raidkm_dcl_rebuild_chunk(b->mddev, sect, b->dcl_X,
						 &w->ctx) :
			raidkm_row_rebuild_chunk(b->mddev, sect, b->target,
						 b->target_slot, &w->ctx);
		if (ret > 0) {
			b->ok[i] = 1;
			continue;
...
/* what a window slot says about its chunk; 0 = not rebuilt (yet) */
#define RK_WIN_DONE	1	/* on the member, md may claim it */
#define RK_WIN_BAD	2	/* declined: the stripe cache takes it */
...
struct raidkm_row_rb_set {
	...
	/*
	 * The look-ahead window (raidkm_row_rebuild_window).  Everything but
	 * next, limit, stop and st[] belongs to md's sync thread, and those
	 * four are all the workers share with it.  Chunk numbers, not sectors.
	 */
	bool				active;
	bool				stop;	/* drain: take no more chunks */
	struct mddev			*mddev;
	struct md_rdev			*target;
	int				target_slot;
	...
	atomic64_t			limit;	/* workers stop short of this */
	atomic64_t			retry;	/* a declined head to try again,
						 * or -1 */
	atomic_t			running; /* workers inside their loop */
	u8				*st;	/* ring, chunk % win */
	wait_queue_head_t		wq;	/* the sync thread, on the head */
};

static inline unsigned int rk_win_slot(const struct raidkm_row_rb_set *set,
				       s64 chunk)
{
	u32 slot;

	div_u64_rem((u64)chunk, set->win, &slot);
	return slot;
}

/*
 * Stop the look-ahead window and wait for its workers.  Whatever it rebuilt
 * beyond md's cursor is simply rebuilt again: those chunks sit above the
 * member's recovery_offset, which trails what raid5_sync_request() returned,
 * and they are counted as unclaimed.  md's sync thread, or conf teardown.
 */
static void raidkm_row_win_drain(struct r5conf *conf,
				 struct raidkm_row_rb_set *set)
{
	...
	if (!set || !set->active)
		return;
	WRITE_ONCE(set->stop, true);
	for (i = 0; i < set->nwk; i++)
		cancel_work_sync(&set->wk[i].work);
	next = atomic64_read(&set->next);
	for (c = set->head; c < next; c++)
		lost += smp_load_acquire(&set->st[rk_win_slot(set, c)]) ==
			RK_WIN_DONE;
	if (lost)
		atomic64_add(lost, &conf->row_rebuild_unclaimed);
	memset(set->st, 0, RK_ROW_REBUILD_WINDOW_MAX);
	atomic64_set(&set->retry, -1);
	set->active = false;
	WRITE_ONCE(set->stop, false);
}
...
static void raidkm_row_rb_set_free(struct r5conf *conf)
{
	...
	/* its workers use these buffers until the window is drained */
							 ...
							 int *page_bufs_out)
{
	...
	set->st = kzalloc(RK_ROW_REBUILD_WINDOW_MAX, GFP_NOIO);
	if (!set->wk || !set->bufs || !set->st)
		goto fail;
	init_waitqueue_head(&set->wq);
	for (i = 0; i < nwk; i++) {
							 ...
							 int k, int workers)
{
	 ...
	 * chunk), so the workers together can own the whole stripe cache and
	 * at more rows than that every claim past the cache fails, is retried
	 * and ends up declined to the stripe path, which needs stripe heads
	 * too.  So never ask for more than the cache holds -- 8 rows at a 256
	 * cache, which is where the default stood before it became 16 -- and
	 * say what would lift the limit.  (Not less than that: md grows the
	 * cache on demand when writers wait for a stripe head, so a fuller
	 * reserve only costs a read-loaded rebuild rows it could have had.)
	 */
	{
		unsigned int nsh = max_t(unsigned int, 1, conf->chunk_sectors /
					 RAID5_STRIPE_SECTORS(conf));
		unsigned int cache = max(conf->min_nr_stripes,
					 conf->max_nr_stripes);
		unsigned int cap = max_t(unsigned int, 1, cache / nsh);

		if (nwk > cap) {
			if (!set || set->nwk != cap)
				pr_info_ratelimited("md/raid:%s: row rebuild limited to %u rows in flight (of %u asked for) by stripe_cache_size=%u; %u would lift it\n",
					mdname(conf->mddev), cap, nwk, cache,
					nsh * nwk);
			nwk = cap;
		}
	}
...
static sector_t raidkm_row_rebuild_band(struct mddev *mddev, sector_t sector_nr,
					sector_t max_sector,
					struct md_rdev *target, int target_slot)
{
	struct r5conf *conf = mddev->private;
	...
	band.mddev = mddev;
	band.target = target;
	band.target_slot = target_slot;
	band.dcl = false;		/* a stack band: say so explicitly */
	band.dcl_X = -1;
	band.start = sector_nr;
	band.nrows = nrows;
	band.ok = ok;
...
/* the population window's backpressure gate, defined with the rest of the
 * population pass further down */
static bool raidkm_dcl_pop_admit(struct r5conf *conf, sector_t first,
				 sector_t last);

/*
 * The same band, driving a DECLUSTERED population: rows are handed to the
 * workers, each resolves its own target (this row's spare column) and writes
 * it whole, and the sync step claims the finished PREFIX.  Without this the
 * population would run one row at a time on md's sync thread -- measured on
 * real NVMe (12 x 4 GiB, 8+2 over 12 with 2 spare columns, chunk 128K):
 * 128 KiB writes but 59 MiB/s, against the stripe path's 6.2 KiB at 466.
 * The writes are the point; the spindles are what the band puts back.
 *
 * Returns sectors claimed, all of which the caller must feed to the prefix
 * mark; 0 means the stripe path should take this address.
 */
static sector_t raidkm_dcl_rebuild_band(struct mddev *mddev, sector_t sector_nr,
					sector_t max_sector, int X)
{
	struct r5conf *conf = mddev->private;
	struct dcl_geom *ge = conf->dcl;
	struct raidkm_row_rb_band band;
	...
	/*
	 * Backpressure: the band's whole range at once, exactly as the
	 * content-free branch admits a row's granules as a range.  A band
	 * starts at md's cursor, which is the mark, so pop_room() admits it
	 * whatever the window is (firstg <= base) -- the window bounds what
	 * is UNFINISHED below an address, and a band finishes before it
	 * reports.
	 */
	if (!raidkm_dcl_pop_admit(conf, sector_nr,
				  sector_nr + (sector_t)nrows *
				  conf->chunk_sectors - 1))
		return 0;
	...
	for (i = done; i < nrows; i++)
		unclaimed += ok[i];
	if (unclaimed)		/* rebuilt past a declined row: redone later */
		atomic64_add(unclaimed, &conf->row_rebuild_unclaimed);
	return (sector_t)done * conf->chunk_sectors;
}

/*
 * The look-ahead window: the engine owns the pass (rk_row_rebuild_window).
 *
 * A band is synchronous -- md's sync thread hands out its rows, waits for the
 * slowest, reports the prefix and only then asks for the next band, so the
 * workers stand idle for every round trip through md_do_sync() and whatever
 * a band rebuilt past a declined row is thrown away.  Here the workers keep
 * going instead: they take chunks in order up to `limit`, a window ahead of
 * md's cursor, and raid5_sync_request() returns the contiguous finished prefix
 * at the cursor, waiting for the head chunk only.
 *
 *   - md's cursor still moves in whole chunks and never past one that is not
 *     on the member, so recovery_offset trails the prefix and what the window
 *     holds beyond it can be lost at any time: an interrupted pass drains it
 *     (the final raid5_sync_request() call) and rebuilds those chunks again.
 *     Until md reaches them they stay right the way a band's early rows do:
 *     a write to the row goes to the member under recovery at every offset
 *     (ops_run_io() tests only Faulty), and the row claim keeps such a write
 *     out of the chunk while it is being rebuilt.
 *   - md's throttle keeps working by backpressure: when md sleeps the window
 *     fills and the workers stop, a window's worth past the rate asked for.
 *   - A declined chunk no longer ends anything.  When md's cursor reaches it
 *     the stripe cache rebuilds that one chunk (the sync window below stops
 *     at the chunk boundary) while the workers carry on beyond it, and the
 *     chunks they finished meanwhile are claimed, not rebuilt twice.
 *
 * The sync thread never sends a chunk to the stripe cache that a worker has
 * or may yet take: it waits out a head in flight, and takes an undispatched
 * head away from the workers (`next`) before giving it up.  A sync request on
 * a stripe a worker has claimed is a combination a band never produced.
 */
static void raidkm_row_rebuild_win_worker(struct work_struct *work)
{
	struct raidkm_row_rb_worker *w =
		container_of(work, struct raidkm_row_rb_worker, work);
	struct raidkm_row_rb_set *set = w->set;
	struct mddev *mddev = set->mddev;
	struct r5conf *conf = mddev->private;

	atomic_inc(&set->running);
	for (;;) {
		s64 c = atomic64_read(&set->next);
		int ok, tries;

		/* a quiesce waits for our stripes; an interrupted pass, or a
		 * window being drained, would only throw the chunk away */
		if (READ_ONCE(set->stop) || READ_ONCE(conf->quiesce) ||
		    test_bit(MD_RECOVERY_INTR, &mddev->recovery))
			break;
		/* the head md is waiting on comes before anything new */
		if (atomic64_read(&set->retry) >= 0)
			c = atomic64_xchg(&set->retry, -1);
		else
			c = -1;
		if (c < 0) {
			c = atomic64_read(&set->next);
			if (c >= atomic64_read(&set->limit)) {
				/*
				 * The window is full: go home -- but say so
				 * FIRST and look again, because md's sync
				 * thread raises `limit` (or posts a `retry`)
				 * and THEN reads `running` to decide whether
				 * anyone needs waking (raidkm_row_win_kick).
				 * One of us always sees the other's store;
				 * without the second look a worker could leave
				 * just as room appeared, unseen, and the head
				 * would sit until the wait's timeout -- 50 ms,
				 * a quarter of a short pass on a fast rig.
				 */
				atomic_dec(&set->running);
				smp_mb__after_atomic();	/* pairs with the kick's */
				if (READ_ONCE(set->stop) ||
				    (atomic64_read(&set->next) >=
				     atomic64_read(&set->limit) &&
				     atomic64_read(&set->retry) < 0))
					return;
				atomic_inc(&set->running);
				continue;
			}
			if (!atomic64_try_cmpxchg(&set->next, &c, c + 1))
				continue;
		}
		/*
		 * A busy row costs nothing to decline, and that is the
		 * trouble: when stripes run short for a moment every worker
		 * that meets one moves straight on to the next chunk and
		 * declines that too, a run of chunks is written off within
		 * microseconds, and md's cursor then has them redone one at a
		 * time.  The bands never did this -- a declined row ended the
		 * band, everyone stood back, and the next band took the row
		 * again at full width.  (8+2 NVMe, 16 workers under a
		 * sequential read: 287 MiB/s against the bands' 312 before
		 * this, with 270-590 chunks a pass redone at the head.)  So
		 * the worker that met the row stands back itself, briefly,
		...
		for (tries = 0; ; tries++) {
			...
			if (ok != -EBUSY)
				break;
			if (tries >= RK_ROW_WIN_BUSY_TRIES ||
			    READ_ONCE(set->stop) || READ_ONCE(conf->quiesce) ||
			    test_bit(MD_RECOVERY_INTR, &mddev->recovery)) {
				atomic64_inc(&conf->row_rebuild_declined);
				break;
			}
			atomic64_inc(&conf->row_rebuild_retries);
			usleep_range(RK_ROW_WIN_BUSY_USEC,
				     2 * RK_ROW_WIN_BUSY_USEC);
		}
		/*
		 * Release: md's sync thread reports this chunk as rebuilt on
		 * the strength of this store, and everything the chunk did --
		 * the member write, the published CRCs, the stripes released
		 * -- has to be visible to it first.
		 */
		smp_store_release(&set->st[rk_win_slot(set, c)],
				  ok > 0 ? RK_WIN_DONE : RK_WIN_BAD);
		wake_up(&set->wq);
	}
	atomic_dec(&set->running);
}
...
/*
 * Wake the workers that went home when the window filled.  One that is still
 * in its loop reads `limit` and `retry` for itself before every chunk, so
 * while all of them are -- the steady state whenever the member is the limit,
 * which is where a sync step returns a chunk or two at a time -- there is
 * nothing to queue, and md's sync thread does not pay for nwk queue_work()
 * calls per step.  Callers store `limit` or `retry` BEFORE calling this, and
 * a worker announces that it is leaving before it looks at them a last time
 * (raidkm_row_rebuild_win_worker), so a worker on its way out is never
 * missed.  (A worker sent home by a quiesce or an interrupt leaves without
 * that handshake; the head wait's timeout is for those.)
 */
static void raidkm_row_win_kick(struct raidkm_row_rb_set *set)
{
	unsigned int i;

	smp_mb();	/* our caller's store, then `running`: see the worker */
	if (atomic_read(&set->running) >= (int)set->nwk)
		return;
	if (atomic64_read(&set->retry) < 0 &&
	    atomic64_read(&set->next) >= atomic64_read(&set->limit))
		return;
	for (i = 0; i < set->nwk; i++)
		queue_work(raidkm_row_wq, &set->wk[i].work);
}

/* rebuild through the window; returns the sectors of progress to claim
 * (0 = the chunk at sector_nr goes to the stripe cache) */
static sector_t raidkm_row_rebuild_window(struct mddev *mddev,
					  sector_t sector_nr,
					  sector_t max_sector,
					  struct md_rdev *target,
					  ...
					  unsigned int win)
{
	...
	head = (s64)tmp;
	/* sync_max bounds what may be rebuilt, not just what is reported */
	tmp = min(max_sector, READ_ONCE(mddev->resync_max));
	sector_div(tmp, conf->chunk_sectors);
	end_c = (s64)tmp;
	if (head >= end_c) {
		/*
		 * md may not reach this chunk after all -- sync_max can be
		 * lowered under a running pass, and md_do_sync() tested it
		 * before calling in.  The stripe cache takes the chunk from
		 * here, so the window must not be left holding it: that is the
		 * one combination this engine promises never to produce.
		 */
	...
	/*
	 * md's cursor is the authority.  It leaves the window when a bitmap
	 * skips ahead or a pass restarts; the member or the knob can change
	 * under a pass as well.  All rare, and all answered the same way.
	 */
	if (set && set->active &&
	...
	if (!set->active) {
		...
		atomic64_set(&set->retry, -1);
		atomic_set(&set->running, 0);
		for (i = 0; i < set->nwk; i++) {
			set->wk[i].set = set;
			INIT_WORK(&set->wk[i].work,
				  raidkm_row_rebuild_win_worker);
		}
		set->active = true;
	}
	atomic64_set(&set->limit, min_t(s64, head + win, end_c));
	raidkm_row_win_kick(set);

	slot = rk_win_slot(set, head);
	t0 = ktime_get();
again:
	while (!(st = smp_load_acquire(&set->st[slot]))) {
		s64 nx = head;

		/*
		 * Nobody has the head and, quiesced or interrupted, nobody is
		 * going to take it: keep it from the workers for good -- out
		 * of `next`, or back out of `retry` -- and let the stripe
		 * cache have it, which knows how to wait out a quiesce and is
		 * where md looks for an interrupt.  A head that IS in flight
		 * is waited for -- one chunk's I/O.
		 */
		if ((READ_ONCE(conf->quiesce) ||
		     test_bit(MD_RECOVERY_INTR, &mddev->recovery)) &&
		    (atomic64_try_cmpxchg(&set->next, &nx, head + 1) ||
		     atomic64_cmpxchg(&set->retry, head, -1) == head)) {
			st = RK_WIN_BAD;
			break;
		}
		/* the timeout re-kicks workers a passing quiesce sent home */
		if (!wait_event_timeout(set->wq,
					smp_load_acquire(&set->st[slot]),
					HZ / 20))
			atomic64_inc(&conf->row_rebuild_head_timeouts);
		raidkm_row_win_kick(set);
	}
	/*
	 * Declined while it was still ahead of md's cursor, which is early:
	 * the row was busy THEN.  Try it once more now that md is here, which
	 * is the chance a band gave every declined row by starting the next
	 * band on it.  Most of what declines under a read load passes this
	 * time, and only what declines twice costs 32 stripe heads.
	 */
	if (st == RK_WIN_BAD && !retried && !READ_ONCE(conf->quiesce) &&
	    !test_bit(MD_RECOVERY_INTR, &mddev->recovery)) {
		retried = true;
		atomic64_inc(&conf->row_rebuild_retries);
		WRITE_ONCE(set->st[slot], 0);
		/* the clear, THEN the post -- see the barrier below */
		smp_wmb();
		atomic64_set(&set->retry, head);
		raidkm_row_win_kick(set);
		goto again;
	}
	atomic64_add(ktime_to_ns(ktime_sub(ktime_get(), t0)),
		     &conf->row_rebuild_head_wait_ns);

	if (st == RK_WIN_DONE)
		while (head + n < atomic64_read(&set->limit) &&
		       smp_load_acquire(&set->st[rk_win_slot(set, head + n)]) ==
		       RK_WIN_DONE)
			n++;
	/*
	 * Hand the slots back BEFORE raising the limit: chunk c + win reuses
	 * the slot of chunk c, and a worker may take it the moment the limit
	 * allows.  A declined head is consumed too -- the stripe cache takes
	 * that chunk from here, and md's next call on the chunk grid is the
	 * one after it.
	 */
	for (i = 0; i < max(n, 1U); i++)
		WRITE_ONCE(set->st[rk_win_slot(set, head + i)], 0);
	set->head = head + max(n, 1U);
	/*
	 * ... and the clears have to be VISIBLE before the limit that lets a
	 * worker reuse those slots, not merely written first.  A worker in its
	 * loop reads `limit` itself, with nothing to pair with; if it saw the
	 * new limit while a clear was still in this CPU's store buffer, it
	 * could take chunk head + win, spend a chunk's I/O on it, publish
	 * RK_WIN_DONE for the slot -- and then our clear would land on top and
	 * lose it.  Nothing re-dispatches that chunk (`next` is past it), so
	 * the next sync step would wait on a head no one is coming back for.
	 * Free on x86; the arm64 build is why it is written down.
	 */
	smp_wmb();
	atomic64_set(&set->limit, min_t(s64, set->head + win, end_c));
	raidkm_row_win_kick(set);

	if (!n)
		return 0;
	atomic64_add(n, &conf->row_rebuild_done);
	return (sector_t)n * conf->chunk_sectors;
}

/*
 * Row layer, full-row write.
 *
 * A write that covers a whole row (k chunks at one member offset: 1 MiB at
 * 8+2 with 128 KiB chunks) already reaches the members as one request per
 * member through the stripe cache, and its parity is already one syndrome call
 * per row.  What it costs there is CPU: 32 stripe heads per row, each taking
 * and dropping its locks for every one of the k data pages added and every
 * member bio issued and completed (on a CPU-bound 8+2 rig, ~37% of the write
 * path's busy CPU was that lock traffic, parity ~15%).  Here the row is written
 * without any of it: parity is computed straight from the caller's pages, and
 * each member gets one bio -- a clone of the caller's bio for a data chunk, a
 * chunk-sized buffer for a parity chunk.
 *
 * Correctness against the stripe cache comes from a row lock
 * (raidkm_row_write_lock): the row's slot in conf->row_wlock is set, every
 * stripe of the row already in the cache must be idle, and those have their
 * cached pages dropped, which the write makes stale.  Until the member writes
 * are done, any stripe-cache access to the row -- read, write, sync, discard --
 * waits in raid5_get_active_stripe().  No stripe head is taken, so the row
 * path does not need room in the stripe cache.  Row readers
 * (degraded read) see the write through the row's write generation, as they
 * see a stripe-cache write.  Nothing waits for the row path: a bio whose
 * rows cannot all be claimed at once, or with no context free, goes to the
 * stripe cache whole (raidkm_row_write_bio).
 *
 * Kept narrow on purpose -- everything else goes to the stripe cache: a
 * healthy array only (no degraded, recovering or replacement member), no
...
struct raidkm_row_wctx {
	struct list_head	list;		/* conf->row_wctx_free while idle */
	struct work_struct	submit;		/* issues the member writes */
	struct work_struct	work;		/* runs when they are done */
	struct r5conf		*conf;
	struct bio		*raid_bio;
	struct raidkm_row_bucket *bucket;
	sector_t		msect;		/* member sector of the row */
	unsigned int		base;		/* bytes into raid_bio: the row */
	blk_opf_t		opf;		/* for the member writes */
	atomic_t		pending;
	/* fixed when built; a context of another geometry is rebuilt */
	int			k, m;
	unsigned int		chunk_sectors;
	atomic64_t		*wslot;		/* the row's lock slot, NULL = not locked */
	u64			row;
	bool			bitmap;		/* counted in the write-intent bitmap */
	struct raidkm_row_buf	*pbuf;		/* m parity chunks */
...
static void raidkm_row_write_done(struct work_struct *work);
static void raidkm_row_write_submit(struct work_struct *work);

static void raidkm_row_wctx_free(struct raidkm_row_wctx *w)
{
	int i;
...
static struct raidkm_row_wctx *raidkm_row_wctx_alloc(struct r5conf *conf,
						     int k, int m)
{
	const gfp_t gfp = GFP_NOIO | __GFP_NOWARN;
	unsigned int len = conf->chunk_sectors << 9;
	struct raidkm_row_wctx *w;
	int i;

	w = kzalloc(struct_size(w, mw, k + m), gfp);
	if (!w)
		return NULL;
	w->conf = conf;
	w->k = k;
	w->m = m;
	w->chunk_sectors = conf->chunk_sectors;
	INIT_WORK(&w->submit, raidkm_row_write_submit);
	INIT_WORK(&w->work, raidkm_row_write_done);
	w->pbuf = kcalloc(m, sizeof(*w->pbuf), gfp);
	...
	w->dst = w->src + k;
	for (i = 0; i < m; i++)
		if (raidkm_row_buf_alloc(&w->pbuf[i], len))
			goto fail;
	return w;
fail:
	raidkm_row_wctx_free(w);
	return NULL;
}

/* an idle context for a k + m row, or NULL: rk_row_write_depth are in use */
static struct raidkm_row_wctx *raidkm_row_wctx_get(struct r5conf *conf,
						   int k, int m)
{
	struct raidkm_row_wctx *w;

	spin_lock(&conf->row_wctx_lock);
	w = list_first_entry_or_null(&conf->row_wctx_free,
				     struct raidkm_row_wctx, list);
	if (w) {
		list_del(&w->list);
	} else if (conf->row_wctx_nr < READ_ONCE(conf->row_write_depth)) {
		conf->row_wctx_nr++;
	} else {
		spin_unlock(&conf->row_wctx_lock);
		return NULL;
	}
	spin_unlock(&conf->row_wctx_lock);
	...
	if (w && w->k == k && w->m == m &&
	    w->chunk_sectors == conf->chunk_sectors)
		return w;
	/* none idle (a slot was reserved above), or built for a geometry a
	 * reshape has since changed: build one; this happens at most
	 * rk_row_write_depth times per geometry */
	raidkm_row_wctx_free(w);
	w = raidkm_row_wctx_alloc(conf, k, m);
	if (!w) {
		spin_lock(&conf->row_wctx_lock);
		conf->row_wctx_nr--;
		spin_unlock(&conf->row_wctx_lock);
...
static void raidkm_row_wctx_put(struct r5conf *conf, struct raidkm_row_wctx *w)
{
	spin_lock(&conf->row_wctx_lock);
	if (!READ_ONCE(conf->row_write) ||
	    conf->row_wctx_nr > READ_ONCE(conf->row_write_depth)) {
		conf->row_wctx_nr--;
		spin_unlock(&conf->row_wctx_lock);
		raidkm_row_wctx_free(w);
		return;
	}
	list_add(&w->list, &conf->row_wctx_free);
	spin_unlock(&conf->row_wctx_lock);
}

/* free the idle contexts; ones in flight are freed as they come back if the
 * knob is off (raidkm_row_wctx_put), and at the latest by free_conf() */
static void raidkm_row_wctx_free_idle(struct r5conf *conf)
{
	struct raidkm_row_wctx *w, *tmp;
	LIST_HEAD(idle);

	spin_lock(&conf->row_wctx_lock);
	list_for_each_entry_safe(w, tmp, &conf->row_wctx_free, list) {
		list_move(&w->list, &idle);
		conf->row_wctx_nr--;
	}
	spin_unlock(&conf->row_wctx_lock);
	list_for_each_entry_safe(w, tmp, &idle, list) {
		list_del(&w->list);
		raidkm_row_wctx_free(w);
	}
}
...
/*
 * m parity chunks from k data chunks, @len bytes of each: @src holds the k
 * data pointers followed by the m parity pointers (@dst == src + k) -- the
 * stripe path's own encoders, so the two paths cannot disagree.  At m == 2
 * that is raid6's P+Q (async_gen_syndrome's routine), whose Q weights data
 * index d by 2^d; above it, ISA-L with the tables raidkm_build_ec() made for
 * this geometry (conf->ec_g_tbls_*, as isal_gen_syndrome_m uses).  @len is a
 * multiple of the sector size, which covers the widest raid6 unroll.
 */
static void raidkm_row_encode(struct r5conf *conf, unsigned int len, int k,
			      int m, unsigned char **src, unsigned char **dst)
{
	if (m == 2 && k >= 2) {		/* raid6 wants at least 4 disks */
		raid6_call.gen_syndrome(k + 2, len, (void **)src);
		return;
	}
	/* ec_encode_data_*_gfni bracket kernel_fpu themselves */
	if (isal_have_avx512_gfni())
		ec_encode_data_avx512_gfni(len, k, m, conf->ec_g_tbls_gfni,
					   src, dst);
	else if (isal_have_gfni())
		ec_encode_data_avx2_gfni(len, k, m, conf->ec_g_tbls_gfni,
					 src, dst);
	else
		ec_encode_data_base(len, k, m, conf->ec_g_tbls_base, src, dst);
}

/* may @bi go to the row path at all?  (Per row, the members decide too.) */
static bool raidkm_row_write_ok(struct mddev *mddev, struct r5conf *conf,
				struct bio *bi)
{
	return READ_ONCE(conf->row_write) &&
...
/*
 * The caller's pages for [base, base + bytes) of @bio: every piece a whole
 * number of sectors, so no encode run is shorter than one.
 */
static bool raidkm_row_write_bvecs_ok(struct bio *bio, unsigned int base,
				      unsigned int bytes)
{
	struct bvec_iter it = bio->bi_iter;

	bio_advance_iter(bio, &it, base);
	while (bytes) {
		struct bio_vec bv = mp_bvec_iter_bvec(bio->bi_io_vec, it);
		unsigned int l = min(bv.bv_len, bytes);

		if ((bv.bv_offset | l) & (SECTOR_SIZE - 1))
			return false;
		bio_advance_iter_single(bio, &it, l);
		bytes -= l;
	}
	return true;
}
...
/*
 * Give the row back to the stripe cache: whoever waits for it in
 * raid5_get_active_stripe() looks again.  A bounced aligned read parked for
 * raid5d (retry_aligned_read asks without blocking) is retried when raid5d
 * next runs, so poke it -- only then.
 */
static void raidkm_row_write_unlock(struct r5conf *conf,
				    struct raidkm_row_wctx *w)
{
	u64 locked = w->row + 1;
	...
	if (!w->wslot)
		return;
	/* a stripe-cache request waits for this row: keep the row for it */
	if (atomic64_cmpxchg(w->wslot, locked, 0) != locked)
		atomic64_set(w->wslot, locked | RAIDKM_ROW_WLOCK_GRACE);
	w->wslot = NULL;
	if (wq_has_sleeper(&conf->row_write_wait))	/* orders the slot */
		wake_up(&conf->row_write_wait);
	if (READ_ONCE(conf->retry_read_aligned) ||
	    READ_ONCE(conf->retry_read_aligned_list))
		md_wakeup_thread(conf->mddev->thread);
}

/*
 * Own the row: set its slot, then walk its stripes under their hash locks.
 * A stripe that is in use means the stripe cache is working on the row:
 * give it up (false).  An idle one keeps its R5_UPTODATE pages as a cache --
 * parity left by a read-modify-write, typically -- which the write is about
 * to make stale: drop them, or the next read-modify-write of that stripe
 * computes its parity from the old parity.  Nothing can take an idle stripe
 * while we hold its hash lock, and after the walk nothing can take one at
 * all until the slot clears (raid5_get_active_stripe).
 */
static bool raidkm_row_write_lock(struct r5conf *conf,
				  struct raidkm_row_wctx *w)
{
	atomic64_t *slot = raidkm_row_wslot(conf, w->row);
	/* now, not when the context was built: stripe_size can change */
	unsigned int nsh = conf->chunk_sectors / RAID5_STRIPE_SECTORS(conf);
	sector_t first = w->msect;
	bool busy = false;
	int j, i;

	if (atomic64_cmpxchg(slot, 0, w->row + 1) != 0)
		return false;		/* this row, or one sharing its slot */
	w->wslot = slot;
	smp_mb__after_atomic();		/* the slot, then the walk */

	for (j = 0; j < nsh && !busy; j++) {
		sector_t sect = first + j * RAID5_STRIPE_SECTORS(conf);
		int h = stripe_hash_locks_hash(conf, sect, 0);
		struct stripe_head *sh;

		spin_lock_irq(conf->hash_locks + h);
		sh = __find_stripe(conf, sect, conf->generation, 0);
		/* in use: referenced, or queued for handling on a list with
		 * a zero count */
		if (sh && (atomic_read(&sh->count) || sh->batch_head ||
			   test_bit(STRIPE_HANDLE, &sh->state) ||
			   test_bit(STRIPE_DELAYED, &sh->state) ||
			   test_bit(STRIPE_BIT_DELAY, &sh->state) ||
			   test_bit(STRIPE_ROW_SYNC, &sh->state)))
			...
			for (i = 0; i < sh->disks; i++)
				clear_bit(R5_UPTODATE, &sh->dev[i].flags);
		spin_unlock_irq(conf->hash_locks + h);
	}
	if (busy)
		raidkm_row_write_unlock(conf, w);
	return !busy;
}

/*
 * A row write is over (or never started): let a sync pass that waits for row
 * writes (raid5_sync_request) or a quiesce (raid5_quiesce) go ahead.  The last
 * thing a row write does with conf -- the quiesce is what lets the array be
 * stopped and conf freed.
 */
static void raidkm_row_write_unfence(struct r5conf *conf)
{
	if (atomic_dec_and_test(&conf->row_writes)) {
		if (wq_has_sleeper(&conf->row_write_wait))
			wake_up(&conf->row_write_wait);
		wake_up(&conf->wait_for_quiescent);
	}
}

static void raidkm_row_write_endio(struct bio *bi)
{
	struct raidkm_row_wmember *mw = bi->bi_private;
	struct raidkm_row_wctx *w = mw->w;

	if (bi->bi_status)
		WRITE_ONCE(mw->failed, 1);
	bio_put(bi);
	if (atomic_dec_and_test(&w->pending))
		queue_work(raidkm_row_wq, &w->work);
}

/*
 * Every member write of the row is done.  In process context: the release
 * takes stripe_lock with spin_lock_irq, and a failed member may be given a
 * bad block or failed (both can sleep).
 */
static void raidkm_row_write_done(struct work_struct *work)
{
	struct raidkm_row_wctx *w = container_of(work, struct raidkm_row_wctx,
						 work);
	struct r5conf *conf = w->conf;
	struct mddev *mddev = conf->mddev;
	struct bio *raid_bio = w->raid_bio;
	int i, nfail = 0;

	for (i = 0; i < w->k + w->m; i++) {
		struct md_rdev *rdev = w->mw[i].rdev;

		if (READ_ONCE(w->mw[i].failed)) {
			/*
			 * What raid5_end_write_request() and handle_stripe()
			 * do for a stripe: remember the error, ask for a
			 * replacement, and record the chunk as a bad block on
			 * the member -- or, if it cannot take one, fail it.
			 * Either way the row then reads back through parity.
			 */
			nfail++;
			atomic64_inc(&conf->row_write_errors);
			set_bit(WriteErrorSeen, &rdev->flags);
			if (!test_and_set_bit(WantReplacement, &rdev->flags))
				set_bit(MD_RECOVERY_NEEDED, &mddev->recovery);
			if (!rdev_set_badblocks(rdev, w->msect,
						w->chunk_sectors, 0))
				md_error(mddev, rdev);
		}
		rdev_dec_pending(rdev, mddev);
	...
	/* the members hold the new row: row readers may use it again ... */
	atomic_dec(&w->bucket->inflight);
	/* ... and the stripe cache may have it back */
	raidkm_row_write_unlock(conf, w);
	if (nfail > w->m)
		raid_bio->bi_status = BLK_STS_IOERR;	/* beyond the parity */
	/* a failed member leaves the array degraded, and md then keeps the
	 * bit set: the region is resynced when the member comes back */
	if (w->bitmap)
		raidkm_bitmap_endwrite(mddev, w->msect, w->chunk_sectors,
				       !nfail);
	/*
	 * A failed member is now a bad block or a failed disk -- in memory.
	 * As the stripe path holds a stripe's bios while MD_SB_CHANGE_PENDING
	 * is set, the caller's write is not acknowledged before that is on
	 * disk: after a crash the member must not come back looking in sync
	 * with the old contents of a chunk the caller was told is written.
	 */
	if (nfail) {
	...
	/*
	 * The context goes back to the pool (this work item may be queued
	 * again from there, and then runs once this returns), and only then
	 * does the in-flight count drop: until it does, a quiesce -- and with
	 * it a stop of the array, which frees conf and the pool -- waits.
	 * After it, only mddev and the caller's bio are touched.
	 */
	raidkm_row_wctx_put(conf, w);
	raidkm_row_write_unfence(conf);
	md_write_end(mddev);
	bio_endio(raid_bio);
}

/* issue the member writes of a claimed row: one bio per member */
static void raidkm_row_write_submit(struct work_struct *work)
{
	struct raidkm_row_wctx *w = container_of(work, struct raidkm_row_wctx,
						 submit);
	struct mddev *mddev = w->conf->mddev;
	struct bio *raid_bio = w->raid_bio;
	unsigned int cs = w->chunk_sectors, len = cs << 9;
	/*
	 * Locals: a member that completes inside submit_bio_noacct() (a
	 * bio-based member does) can finish the row and free @w before the
	 * loop looks at it again -- nothing below reads @w after the last
	 * submit.
	 */
	const int k = w->k, n = w->k + w->m;
	struct blk_plug plug;
	int i;

	/* the bitmap bits for this row are on disk before its data is */
	if (w->bitmap)
		mddev->bitmap_ops->unplug(mddev, true);
	atomic_set(&w->pending, n);
	blk_start_plug(&plug);
	for (i = 0; i < n; i++) {
		struct md_rdev *rdev = w->mw[i].rdev;
		struct bio *bi;
		...
		if (i < k) {
			/* a data chunk: the caller's own pages */
			bi = bio_alloc_clone(rdev->bdev, raid_bio, GFP_NOIO,
					     &mddev->bio_set);
			bio_trim(bi, (w->base >> 9) + i * cs, cs);
			bi->bi_opf = w->opf;
		} else {
			struct raidkm_row_buf *pb = &w->pbuf[i - k];

			bi = bio_alloc_bioset(rdev->bdev,
					      raidkm_row_buf_nr_vecs(pb), w->opf,
					      GFP_NOIO, &mddev->bio_set);
			raidkm_row_buf_add_to_bio(bi, pb, len);
		}
		bi->bi_iter.bi_sector = w->msect + rdev->data_offset;
		bi->bi_end_io = raidkm_row_write_endio;
		bi->bi_private = &w->mw[i];
		submit_bio_noacct(bi);
	}
	blk_finish_plug(&plug);
}
...
/*
 * Undo raidkm_row_write_prep(): the row is not written.  Called in
 * make_request context, so conf stays valid after the unfence.
 */
static void raidkm_row_write_unprep(struct r5conf *conf,
				    struct raidkm_row_wctx *w)
{
	int i;
	...
	for (i = 0; i < w->k + w->m; i++)
		if (w->mw[i].rdev) {
			rdev_dec_pending(w->mw[i].rdev, conf->mddev);
			w->mw[i].rdev = NULL;
		}
	raidkm_row_wctx_put(conf, w);
}
...
/*
 * Get the row of @raid_bio that starts at array sector @logical ready to
 * write: its members pinned, its stripes claimed, a context to do it in.
 * Nothing is written yet.  NULL = the row cannot go through the row path now
 * (counted); nothing is held.
 */
static struct raidkm_row_wctx *
raidkm_row_write_prep(struct mddev *mddev, struct bio *raid_bio,
		      sector_t logical)
{
	struct r5conf *conf = mddev->private;
	unsigned int base = (logical - raid_bio->bi_iter.bi_sector) << 9;
	struct raidkm_row_wctx *w;
	unsigned int cs, len, seq;
	sector_t msect, row;
	int n, m, k, dd_idx, pd, i;
	bool bad;

	/*
	 * The geometry and the row's place in it, read under the reshape
	 * seqcount.  A reshape starting meanwhile can make them disagree:
	 * that is a decline, not a layout bug, so warn only if the seqcount
	 * held.
	 */
	seq = read_seqcount_begin(&conf->gen_lock);
	n = READ_ONCE(conf->raid_disks);
	m = READ_ONCE(conf->m);
	k = n - m;
	cs = READ_ONCE(conf->chunk_sectors);
	len = cs << 9;
	msect = raid5_compute_sector(conf, logical, 0, &dd_idx, NULL);
	row = msect;
	bad = sector_div(row, cs) != 0;
	pd = conf->rotating ? n - 1 - (int)sector_div(row, n) : k;
	/* the row's first sector is data index 0: if the layout disagrees,
	 * this path's slot map is wrong for it and must not write */
	bad |= (dd_idx - pd - m + 2 * n) % n != 0;
	if (bad) {
		WARN_ON_ONCE(!read_seqcount_retry(&conf->gen_lock, seq));
		return NULL;
	}
	/* m > 2 encodes with the stripe path's own tables */
	if ((m != 2 || k < 2) && !conf->ec_g_tbls_gfni && !conf->ec_g_tbls_base)
		return NULL;

	if (!raidkm_row_write_bvecs_ok(raid_bio, base, k * len)) {
		atomic64_inc(&conf->row_write_declined);
		return NULL;
	}

	w = raidkm_row_wctx_get(conf, k, m);
	if (!w) {
		atomic64_inc(&conf->row_write_nobuf);
		return NULL;
	}
	w->raid_bio = raid_bio;
	w->msect = msect;
	...
	/* the members, by generator row: data index d at slot pd+m+d, parity
	 * index p at pd+p -- all healthy, all taking the whole chunk */
	for (i = 0; i < k + m; i++) {
		w->mw[i].w = w;
		w->mw[i].failed = 0;
		w->mw[i].rdev = NULL;
	...
	for (i = 0; i < k + m; i++) {
		int idx = i < k ? (pd + m + i) % n : (pd + i - k) % n;
		struct md_rdev *rdev = conf->disks[idx].rdev;

		/* Blocked: the stripe path waits for it to be unblocked
		 * (md_wait_for_blocked_rdev) before writing to it */
		if (!rdev || test_bit(Faulty, &rdev->flags) ||
		    !test_bit(In_sync, &rdev->flags) ||
		    test_bit(Blocked, &rdev->flags) ||
		    conf->disks[idx].replacement)
			goto out_declined;
		atomic_inc(&rdev->nr_pending);
		w->mw[i].rdev = rdev;
		w->mw[i].disk = idx;
	...
	/*
	 * The fence against sync passes: count ourselves, THEN look for one.
	 * raid5_sync_request() sees MD_RECOVERY_RUNNING set, THEN waits for the
	 * count to drain, so either we see its flag or it sees our count.
	 */
	atomic_inc(&conf->row_writes);
	smp_mb__after_atomic();
	w->wslot = NULL;
	w->row = raidkm_row_of(conf, msect);
	if (test_bit(MD_RECOVERY_RUNNING, &mddev->recovery) ||
	    !raidkm_row_write_lock(conf, w))
		goto out_busy;
	/*
	 * The in-flight count, THEN the quiesce: raid5_quiesce() sets
	 * conf->quiesce, THEN waits for the count, so it either sees us and
	 * waits for this row, or we see it here and give the row up -- and
	 * with it a reshape it may have begun.
	 */
	if (READ_ONCE(conf->quiesce) || conf->reshape_progress != MaxSector ||
	    read_seqcount_retry(&conf->gen_lock, seq))
		goto out_busy;
	return w;

out_busy:
	atomic64_inc(&conf->row_write_busy);
	raidkm_row_write_unprep(conf, w);
	return NULL;
out_declined:
	atomic64_inc(&conf->row_write_declined);
	for (i = 0; i < k + m; i++)
		if (w->mw[i].rdev) {
			rdev_dec_pending(w->mw[i].rdev, mddev);
			w->mw[i].rdev = NULL;
		}
	raidkm_row_wctx_put(conf, w);
	return NULL;
...
/*
 * Write a row raidkm_row_write_prep() got ready: compute its parity from the
 * caller's pages and hand the member writes to a worker.  The caller's bio
 * completes when the row does (raidkm_row_write_done).
 */
static void raidkm_row_write_go(struct mddev *mddev,
				struct raidkm_row_wctx *w)
{
	struct r5conf *conf = mddev->private;
	struct bio *raid_bio = w->raid_bio;
	...
	w->bucket = raidkm_row_bucket(conf, w->msect, 0);
	...
	for (done = 0; done < len; ) {
		...
		for (i = 0; i < k; i++) {
			...
			run = min(run, bv.bv_len);
			w->src[i] = bvec_virt(&bv);
		}
		for (i = 0; i < m; i++)
	 ...
	 * stripe (startwrite in __add_stripe_bio), for the chunk's member
	 * range.  The bit must be on disk before any member write goes out:
	 * the submit worker flushes the bitmap first, where the stripe path
	 * parks the stripe on bitmap_list until raid5d does.
	 */
	w->bitmap = !!mddev->bitmap;
	if (w->bitmap)
		raidkm_bitmap_startwrite(mddev, w->msect, w->chunk_sectors);
	md_write_inc(mddev, raid_bio);
	bio_inc_remaining(raid_bio);
	w->opf = REQ_OP_WRITE |
		 (raid_bio->bi_opf & (REQ_SYNC | REQ_FUA | REQ_META | REQ_PRIO));
	/*
	 * Not from here: this is md's make_request, and a bio submitted from
	 * inside it only goes on current->bio_list, to be sent after we
	 * return.  Until then the row stays claimed, so anything that makes
	 * this thread wait before it returns -- a stripe-cache write bouncing
	 * off a row another submitter claimed while that one waits on ours --
	 * would keep both rows' writes from ever leaving.  A worker sends them.
	 */
	queue_work(raidkm_row_wq, &w->submit);
	atomic64_inc(&conf->row_write_done);
}

#define RK_ROW_WRITE_BIO_ROWS	16	/* rows per bio the row path takes */

/*
 * Send @bi through the row path: true = taken, the row writes complete it;
 * false = nothing was written, the stripe cache takes all of it.
 *
 * All or nothing.  The bio must be made of whole rows, and every one of them
 * must be claimed before any is written.  A bio split between the two paths
 * could be handed back to md for a retry by the stripe cache
 * (STRIPE_WAIT_RESHAPE frees the clone, md_free_cloned_bio) while row writes
 * still hold it.  With 128 KiB chunks at 8+2 a bio is at most one row, so
 * this costs nothing there; with smaller rows it keeps unaligned bios off the
 * row path.
 */
static bool raidkm_row_write_bio(struct mddev *mddev, struct r5conf *conf,
				 struct bio *bi)
{
	unsigned int rowsz = conf->chunk_sectors * (conf->raid_disks - conf->m);
	struct raidkm_row_wctx *w[RK_ROW_WRITE_BIO_ROWS];
	sector_t start = bi->bi_iter.bi_sector, nrows = bio_sectors(bi);
	int i, n;

	if (sector_div(start, rowsz) || sector_div(nrows, rowsz) ||
	    !nrows || nrows > RK_ROW_WRITE_BIO_ROWS)
		return false;
	for (n = 0; n < nrows; n++) {
		w[n] = raidkm_row_write_prep(mddev, bi, bi->bi_iter.bi_sector +
					     (sector_t)n * rowsz);
		if (!w[n]) {
			for (i = 0; i < n; i++)
				raidkm_row_write_unprep(conf, w[i]);
			return false;
		}
	}
	for (i = 0; i < n; i++)
		raidkm_row_write_go(mddev, w[i]);
	return true;
}
...
static struct bio *raidkm_row_read(struct mddev *mddev, struct bio *raid_bio)
{
	...
	if (raid5_has_log(conf) || raid5_has_ppl(conf) ||
	    mddev->degraded > conf->m ||
	    conf->raid_disks > RAIDKM_MAX_STRIPE_DISKS)
		return raid_bio;
	if (conf->dcl && !raidkm_row_dcl_steady(conf))
		return raid_bio;

	/* a span with a missing chunk reads its row once, not twice */
	if (sectors < bio_sectors(raid_bio) &&
	    raidkm_row_dread_wide(mddev, raid_bio))
		return NULL;
	...
	/*
	 * The bypass is only safe on a csum array for a whole-block bio: that
	 * is the gate raid5_make_request() puts on the healthy-path bypass,
	 * because raidkm_csum_verify_abio() can only check whole blocks --
	 * a sub-block read would complete UNVERIFIED, and a straddling one
	 * would compare a window against the wrong block's CRC and fail every
	 * time.  raidkm_row_dread_one() below needs no such gate: it rounds
	 * its own span out to whole blocks and verifies that, not the bio.
	 */
	if ((!conf->csum || raidkm_csum_bio_verifiable(conf, raid_bio)) &&
	    raid5_read_one_chunk(mddev, raid_bio)) {
...
static void raid5_unplug(struct blk_plug_cb *blk_cb, bool from_schedule)
{
	...
	if (cb->list.next && !list_empty(&cb->list)) {
		while (!list_empty(&cb->list)) {
			 ...
			 * _raw_spin_unlock_irq ~14% of CPU at 16-disk width).
			 * The llist coalesces every submitter's releases into a
			 * few batched, device_lock-held drains performed by
			 * raid5d and the worker threads (release_stripe_list),
			 * which take the lock far less often and in larger
			 * batches.  do_release_stripe()'s inactive stripes land
			 * on the drainer's temp_inactive_list, so cb's own
...
static void make_discard_request(struct mddev *mddev, struct bio *bi)
{
	...
	again:
		...
		for (d = 0; d < conf->raid_disks; d++) {
			if (is_parity_disk(sh, d))
				continue;
			/* a row rebuild owns this chunk: discarding the
			 * survivors under its reads would leave the member it
			 * rebuilds holding decoded stale data.  Same bounce
			 * the normal write path takes. */
		...
		if (conf->mddev->bitmap) {
			for (d = 0; d < conf->raid_disks - conf->max_degraded;
			     d++)
				raidkm_bitmap_startwrite(mddev, sh->sector,
					RAID5_STRIPE_SECTORS(conf));
			sh->bm_seq = conf->seq_flush + 1;
			set_bit(STRIPE_BIT_DELAY, &sh->state);
		}
...
/*
 * What the stripe looked like when the write bounced off it.  Taken before the
 * caller drops its reference, because after that the stripe can be freed
 * (drop_one_stripe() from the shrinker, or a smaller stripe_cache_size) and
 * reading it would be a use-after-free.
 */
struct raidkm_overlap_snap {
	sector_t	sector;
	unsigned long	state;
	unsigned long	dev_flags;
	int		count;
	int		dd_idx;
	bool		towrite, toread, written, batch;
};

/*
 * Wait for R5_Overlap to clear, and say so when it takes long.  Every stack of
 * a writer stuck here looks the same, so the report carries what a stack
 * cannot: what the stripe it waits for was doing when this write bounced.
 */
static void raidkm_wait_overlap(struct r5conf *conf, struct r5dev *dev,
				const struct raidkm_overlap_snap *snap,
				struct bio *bi)
{
	unsigned long secs = min_t(unsigned long,
				   READ_ONCE(overlap_warn_secs), 3600);
	...
	while (wait_on_bit_timeout(&dev->flags, R5_Overlap,
				   TASK_UNINTERRUPTIBLE, secs * HZ)) {
		waited += secs;
		if (waited == secs)
			atomic64_inc(&conf->overlap_slow);
		pr_warn_ratelimited("md/raid:%s: write to sector %llu has waited %lus on R5_Overlap: when it bounced, stripe %llu state %#lx count %d dev[%d] flags %#lx towrite %d toread %d written %d batch %d; now preread_active %d delayed_list %s, parked %lld aged %lld regrant %lld\n",
				    mdname(conf->mddev),
				    (unsigned long long)bi->bi_iter.bi_sector,
				    waited,
				    (unsigned long long)snap->sector, snap->state,
				    snap->count, snap->dd_idx, snap->dev_flags,
				    snap->towrite, snap->toread, snap->written,
				    snap->batch,
				    atomic_read(&conf->preread_active_stripes),
				    list_empty_careful(&conf->delayed_list) ?
					"empty" : "NOT empty",
				    (long long)atomic64_read(&conf->delayed_parked),
				    (long long)atomic64_read(&conf->delayed_aged),
				    (long long)atomic64_read(&conf->sync_token_regrant));
	}
}
		...
		struct bio *bi, int forwrite, int previous)
{
	...
	for (dd_idx = 0; dd_idx < sh->disks; dd_idx++) {
		...
		if (stripe_bio_overlaps(sh, bi, dd_idx, forwrite)) {
			struct raidkm_overlap_snap snap = {
				.sector = sh->sector, .state = sh->state,
				.dev_flags = dev->flags,
				.count = atomic_read(&sh->count),
				.dd_idx = dd_idx,
				.towrite = dev->towrite != NULL,
				.toread = dev->toread != NULL,
				.written = dev->written != NULL,
				.batch = sh->batch_head != NULL,
			};
			...
			md_wakeup_thread(conf->mddev->thread);
			raidkm_wait_overlap(conf, dev, &snap, bi);
			return 0;
		}
	}
					      ...
					      struct bio *bi)
{
	 ...
	 */
	dd_idx++;
	/* `sh` here is a fake stripe_head filled by raid5_compute_sector to
	 * report pd_idx/qd_idx only — raid_conf is NOT set, so don't call
	 * is_parity_disk() which would deref it.  Inline the parity check. */
	if (is_raidkm(conf)) {
...
static bool raid5_make_request(struct mddev *mddev, struct bio * bi)
{
	...
	const int rw = bio_data_dir(bi);
	enum stripe_result res = STRIPE_SUCCESS;
	int s, stripe_cnt;
	...
	/* a bio of whole rows may go to the row layer, all of it -- before
	 * the stripe path's context is set up, which it does not need */
	if (rw == WRITE && raidkm_row_write_ok(mddev, conf, bi) &&
	    raidkm_row_write_bio(mddev, conf, bi)) {
		md_write_end(mddev);
		bio_endio(bi);
		return true;
	}
	...
	/*
	 * From a pool, with a stripe bitmap as long as the array's largest bio
	 * (raid5_create_ctx_pool): an array whose row is over 1 MiB takes
	 * bios of a whole row (raid5_set_limits), more stripes than the fixed
	 * RAID5_MAX_REQ_STRIPES bitmap this used to be on the stack.
	 */
	...
	if (likely(conf->reshape_progress == MaxSector)) {
		 ...
		 * the first chunk. That has the best chances of creating IOs
		 * adjacent to previous IOs in case of sequential IO and thus
		 * creates the most sequential IO pattern. We don't bother with
		 * the optimization when reshaping as the performance benefit
...
/* ============================================================================
 * COW-staged online add-parity reshape (raidkm).  See
 * md-kmec/notes/reshape-cow-design.md.  Replaces the stripe-cache reshape body
 * for the m -> m+1 case (conf->prev_m < conf->m), which corrupts the rotating
 * layout via md's raid456-assuming frontier math.
 *
 * Per band (one stripe row; k data + parity, k FIXED): read the k data blocks
 * via the OLD layout, encode the NEW m parities, then STAGE the whole band into
 * a scratch region (new layout) and COMMIT it to the row's home, journaling
 * each phase so a crash is recoverable (redo-from-old / replay-from-scratch).
 * ==========================================================================*/
#ifdef RAIDKM_FAULT_INJECT
static enum raidkm_inject_action
raidkm_reshape_inject_decide(struct r5conf *conf, long band_idx, long last_band,
			     enum raidkm_reshape_phase phase);
#endif

static void raidkm_reshape_encode_page(int k, int m,
		unsigned char *g_gfni, unsigned char *g_base,
		unsigned char **src, unsigned char **dst)
{
	/* ec_encode_data_*_gfni self-wrap kernel_fpu; base is plain C. */
	if (isal_have_avx512_gfni())
		ec_encode_data_avx512_gfni(PAGE_SIZE, k, m, g_gfni, src, dst);
	else if (isal_have_gfni())
		ec_encode_data_avx2_gfni(PAGE_SIZE, k, m, g_gfni, src, dst);
	else
		ec_encode_data_base(PAGE_SIZE, k, m, g_base, src, dst);
}

/* Park the reshape thread (debug inject) until the reshape is interrupted.
 * Only used by the RAIDKM_FAULT_INJECT hooks, so __maybe_unused in production. */
static int __maybe_unused raidkm_reshape_park(struct mddev *mddev)
{
	while (!test_bit(MD_RECOVERY_INTR, &mddev->recovery) &&
	       !kthread_should_stop())
		msleep(100);
	return -EINTR;
}
...
/*
 * Write the staged band to per-disk @base (scratch or home): unit u = j*npages
 * + po writes page band[u] to disk j at sector base + po*PAGE_SECTORS.  @limit
 * caps the units written (a torn/partial write for fault injection).
 */
static int raidkm_reshape_write_units(struct r5conf *conf, struct page **band,
				      int Nnew, int npages, sector_t home_base,
				      bool to_scratch, int limit)
{
	int u, total = Nnew * npages, nfail = 0;

	for (u = 0; u < total && u < limit; u++) {
		int j = u / npages, po = u % npages;
		...
		if (!rdev || test_bit(Faulty, &rdev->flags))
			continue;
		if (to_scratch)
			ok = raidkm_scratch_io(rdev,
				RAIDKM_RJ_SLOTS * RAIDKM_PAGE_SECTORS +
					(sector_t)po * RAIDKM_PAGE_SECTORS,
				band[u], REQ_OP_WRITE | REQ_FUA);
		else
			ok = sync_page_io(rdev,
				home_base + (sector_t)po * RAIDKM_PAGE_SECTORS,
				PAGE_SIZE, band[u], REQ_OP_WRITE | REQ_FUA, false);
		if (!ok) {
			/*
			 * A real write error mid-reshape (a healthy FUA write does
			 * not fail).  Fail the member so its now-stale slot is
			 * treated as MISSING — reads EC-reconstruct it from the
			 * survivors and recovery rebuilds it — instead of the band
			 * being journaled DONE with a silently-wrong home slot.
			 * md_error sets Faulty synchronously, so the remaining
			 * pages of this slot are skipped by the test above.
			 */
			md_error(conf->mddev, rdev);
			nfail++;
		}
...
{
	struct mddev *mddev = conf->mddev;
	int Nold = conf->previous_raid_disks, mold = conf->prev_m;
	int Nnew = conf->raid_disks, mnew = conf->m;
	int k = Nnew - mnew;
	...
	/*
	 * Read the new row's k data chunks, each from the location it occupied
	 * under the OLD geometry, into its new data slot.  Logical chunk
	 * C = row*k + d lived at old row C/kold, data index C%kold — for
	 * add-parity kold == k so every chunk is old row `row`; for add-data
	 * kold < k so the data renumbers across (possibly several) old rows.
	 * Chunks at or beyond the old array's data (add-data only) are the grown
	 * capacity: zero them, so the new parity is the parity of zeros.
	 */
	...
	for (d = 0; d < k; d++) {
		...
		if (chunk_no >= old_data_chunks) {	/* grown capacity: zero data */
			for (po = 0; po < npages; po++)
				memset(page_address(band[new_ds[d] * npages + po]),
				       0, PAGE_SIZE);
			continue;
		}
		old_row = chunk_no;
		old_didx = sector_div(old_row, kold);
		src_sector = old_row * chunk;
		raidkm_reshape_row_layout(Nold, mold, rot, old_row, old_ds, old_ps);
		rdev = conf->disks[old_ds[old_didx]].rdev;
		if (!rdev || test_bit(Faulty, &rdev->flags)) {
	...
	for (po = 0; po < npages; po++) {
		for (d = 0; d < k; d++)
			src[d] = page_address(band[new_ds[d] * npages + po]);
		for (p = 0; p < mnew; p++)
			dst[p] = page_address(band[new_ps[p] * npages + po]);
		raidkm_reshape_encode_page(k, mnew, conf->ec_g_tbls_gfni,
					   conf->ec_g_tbls_base, src, dst);
	}

	dcsum = raidkm_reshape_band_csum(band, npages, new_ds, k);

	/* ---- STAGE: journal, then write the band to scratch ---- */
	err = raidkm_reshape_jwrite(conf, ctx, RAIDKM_PH_STAGE,
				    (u64)row * k, k, dcsum, conf->reshape_progress);
	if (err) goto out;
	lim = total;
#ifdef RAIDKM_FAULT_INJECT
	...
	/* ---- COMMIT: journal (scratch durable), then write the band home ---- */
...
/*
 * Reconstruct the missing chunks of a complete CURRENT-geometry band held in
 * @band (pages laid out as slot*npages + po).  @miss[0..nmiss) are the physical
 * slots whose on-disk copy was unreadable (a failed member).  Missing DATA
 * chunks are decoded from k survivors via gf_invert_matrix (the raw-page twin
 * of ops_run_compute_km); then ALL m parities are re-encoded from the
 * now-complete data, which also fills any missing PARITY slot.  Requires
 * nmiss <= m so that at least k survivors remain.  conf->ec_a_matrix is the
 * (k + m) x k generator for the current geometry.
 */
static int raidkm_reshape_reconstruct_band(struct r5conf *conf,
		struct page **band, int npages,
		const int *ds, const int *ps,
		const int *miss, int nmiss)
{
	...
	for (p = 0; p < m; p++)   logrow[ps[p]] = k + p;
	for (i = 0; i < nmiss; i++) ismiss[miss[i]] = true;

	/* k surviving slots (data or parity) to decode from */
	...
	row1 = kmalloc_array(k, 1, GFP_KERNEL);
	dt  = kmalloc_array(k * 32, 1, GFP_KERNEL);
	if (!B || !inv || !row1 || !dt) { err = -ENOMEM; goto out; }

	/* decode matrix = generator rows of the k survivors, then invert */
	for (i = 0; i < k; i++)
		for (j = 0; j < k; j++)
			B[i * k + j] = amat[logrow[srcsl[i]] * k + j];
	if (gf_invert_matrix(B, inv, k) < 0) { err = -EIO; goto out; }	/* singular */

	/* reconstruct each missing DATA chunk: coeffs for data index d are inv[d] */
	for (i = 0; i < nmiss; i++) {
		s = miss[i];
		d = datidx[s];
		if (d < 0)
			continue;		/* parity slot: re-encoded below */
		for (j = 0; j < k; j++)
			row1[j] = inv[d * k + j];
		for (po = 0; po < npages; po++) {
			unsigned char *d1[1];
			...
			for (j = 0; j < k; j++)
				src[j] = page_address(band[srcsl[j] * npages + po]);
			d1[0] = page_address(band[s * npages + po]);
			if (isal_have_avx512_gfni()) {
				ec_init_tables_gfni(k, 1, row1, dt);
				ec_encode_data_avx512_gfni(PAGE_SIZE, k, 1, dt, src, d1);
			} else if (isal_have_gfni()) {
				ec_init_tables_gfni(k, 1, row1, dt);
				ec_encode_data_avx2_gfni(PAGE_SIZE, k, 1, dt, src, d1);
			} else {
				ec_init_tables_base(k, 1, row1, dt);
				ec_encode_data_base(PAGE_SIZE, k, 1, dt, src, d1);
			}
		}
	}
	...
	for (po = 0; po < npages; po++) {
		...
		for (p = 0; p < m; p++)
			pout[p] = page_address(band[ps[p] * npages + po]);
		raidkm_reshape_encode_page(k, m, conf->ec_g_tbls_gfni,
					   conf->ec_g_tbls_base, din, pout);
	}
out:
...
/*
 * Replay a COMMIT on assembly: the staged band in scratch (new geometry) is
 * complete and checksum-valid, but its copy to the row's home may have been
 * torn by a crash.  Re-read the band from scratch, verify @want_csum, and
 * (re)write it to home.  -EIO if the scratch band is corrupt (unreplayable).
 *
 * Degraded-tolerant: up to m members may be Faulty/unreadable (a disk lost
 * concurrently with the crash).  Their scratch chunks are EC-reconstructed
 * from the survivors; the home (re)write skips them — the normal recovery
 * rebuild restores those members afterwards.
 */
static int raidkm_reshape_replay_commit(struct r5conf *conf, sector_t row,
					u32 want_csum)
{
	int Nnew = conf->raid_disks, mnew = conf->m, k = Nnew - mnew;
	bool rot = conf->rotating;
	...
	/* Read each slot's scratch chunk.  A Faulty/unreadable member is not
	 * fatal: record it (up to m) and EC-reconstruct below from survivors. */
	for (j = 0; j < Nnew; j++) {
		struct md_rdev *rdev = conf->disks[j].rdev;
		bool ok = rdev && !test_bit(Faulty, &rdev->flags);

		for (po = 0; ok && po < npages; po++)
	...
	if (nmiss > mnew) { err = -EIO; goto out; }	/* > m members lost */
	if (nmiss) {
		err = raidkm_reshape_reconstruct_band(conf, band, npages,
						      new_ds, new_ps, miss, nmiss);
		if (err) goto out;
	}
	csum = raidkm_reshape_band_csum(band, npages, new_ds, k);
	if (csum != want_csum) { err = -EIO; goto out; }	/* scratch corrupt */
	for (j = 0; j < Nnew; j++) {
		struct md_rdev *rdev = conf->disks[j].rdev;
...
/*
 * Recover the in-flight band when assembling an interrupted add-parity reshape.
 * Read the journal (highest valid seq) and reconcile conf->reshape_progress with
 * it (the on-disk reshape_position may lag the journal):
 *   DONE   -> band committed; resume after it
 *   COMMIT -> replay scratch->home (idempotent), then resume after it
 ...
 */
/* defined later; the dcl recover rebuilds the group EC tables on a g/m change */
static int raidkm_build_ec(int m, int k, gfp_t gfp, unsigned char **a_out,
			   unsigned char **gg_out, unsigned char **gb_out);
				      ...
				      struct raidkm_reshape_journal *rj)
{
	...
	u32 old_g = le32_to_cpu(rj->dcl_old_g), old_s = le32_to_cpu(rj->dcl_old_s);
	u32 new_g = le32_to_cpu(rj->dcl_g),     new_s = le32_to_cpu(rj->dcl_s);
	u32 old_m = le32_to_cpu(rj->old_m),     new_m = le32_to_cpu(rj->new_m);
	u32 nbase = le32_to_cpu(rj->dcl_nbase);
	...
	u32 old_k = old_g - old_m, new_k = new_g - new_m;
	bool ec_change;
	sector_t resume_row;
	int err;
	 ...
	 * (not-yet-finalized) rkdcl block has to match the journal's OLD side
	...
	ec_change = (new_m != old_m || new_k != old_k);

	nge = raidkm_dcl_geom_new(newN, new_g, new_m, new_s, nbase, new_seed);
	if (!nge)
		return -ENOMEM;

	/*
	 * A group-geometry change (add-parity / add-data) alters the per-group
	 * (k,m), so the assembly-time EC tables — sized from the OLD on-disk k —
	 * no longer match the NEW map the migration writes with.  Rebuild both
	 * sets exactly as raidkm_dcl_start_reshape does for the live reshape: the
	 * NEW set for nge (used by migrate_band_dcl's encode) and the OLD set for
	 * oge (the un-migrated region).  Done before touching conf so a failure
	 * leaves it recoverable.
	 */
	if (ec_change) {
		unsigned char *na, *ngg, *ngb, *pa, *pgg, *pgb;

		if (raidkm_build_ec(new_m, new_k, GFP_KERNEL, &na, &ngg, &ngb)) {
			raidkm_dcl_geom_destroy(nge);
			return -ENOMEM;
		}
		if (raidkm_build_ec(old_m, old_k, GFP_KERNEL, &pa, &pgg, &pgb)) {
			kfree(na);
			kfree(ngg);
			kfree(ngb);
			raidkm_dcl_geom_destroy(nge);
		...
		conf->ec_a_matrix    = na;
		conf->ec_g_tbls_gfni = ngg;
		conf->ec_g_tbls_base = ngb;
		kfree(conf->prev_ec_a_matrix);
		kfree(conf->prev_ec_g_tbls_gfni);
		kfree(conf->prev_ec_g_tbls_base);
		conf->prev_ec_a_matrix    = pa;
		conf->prev_ec_g_tbls_gfni = pgg;
		conf->prev_ec_g_tbls_base = pgb;
		conf->prev_m = old_m;
...
static int raidkm_reshape_recover(struct r5conf *conf)
{
	...
	int chunk = conf->chunk_sectors;
	int k = conf->raid_disks - conf->m;
	sector_t jrow, frontier;
	u32 phase;
	int err;

	err = raidkm_reshape_jread(conf, &rj);
	if (err == -ENOENT) {
		/* No journal.  We are only called with the superblock saying a
		 * reshape is in flight, and a declustered reshape writes its
		 * first journal record (in raidkm_dcl_start_reshape) BEFORE the
		 * superblock records the reshape — so a journal-less mid-reshape
		 * dcl array is a reshape interrupted with an unreadable journal.
		 * Fail closed rather than run on a stale map: resuming without
		 * the dual map would fall through to the stock reshape path.
		 * (Keying on pool size alone would miss a spare-count change,
		 * which keeps raid_disks == dcl->N.) */
		if (conf->dcl) {
			pr_warn("md/raid:%s: dcl reshape in progress (pool %d, geometry N=%u) but no readable reshape journal — refusing\n",
	...
	/* Declustered reshape has its own recovery (the classic band math below
	 * assumes a single-stripe geometry).  A stale journal on a healthy,
	 * finalized array is ignored: at finalize the rkdcl block is rewritten to
	 * the NEW geometry, so if the loaded conf->dcl already matches the
	 * journal's NEW side there is nothing to resume.  (Keying only on N would
	 * miss a spare-count change, which keeps N and only moves g/s/seed.) */
	if (le32_to_cpu(rj.kind) == RAIDKM_RESHAPE_KIND_DCL_POOL) {
		if (conf->dcl &&
		    (u32)conf->dcl->N == le32_to_cpu(rj.new_raid_disks) &&
		    (u32)conf->dcl->g == le32_to_cpu(rj.dcl_g) &&
		    (u32)conf->dcl->s == le32_to_cpu(rj.dcl_s) &&
		    conf->dcl->seed == le64_to_cpu(rj.dcl_new_seed))
			return 0;
	...
	phase = le32_to_cpu(rj.phase);

	/*
	 * Trust the record only if it describes THIS reshape: geometry and
	 * band range must match.  A checksum-valid but foreign record (a
	 * stale journal from a previous array on the same devices) must not
	 * steer recovery — treat it like an absent journal and let the
	 * superblock reshape_position govern.
	 */
	if (le32_to_cpu(rj.old_m) != conf->prev_m ||
	    le32_to_cpu(rj.new_m) != conf->m ||
	    le32_to_cpu(rj.old_raid_disks) != conf->previous_raid_disks ||
	    le32_to_cpu(rj.new_raid_disks) != conf->raid_disks ||
	    le32_to_cpu(rj.chunk_sectors) != conf->chunk_sectors ||
	    jrow * (sector_t)chunk >= mddev->dev_sectors) {
		pr_warn("md/raidkm:%s: ignoring foreign reshape journal record (band %llu phase %u, geometry %u/%u->%u/%u chunk %u)\n",
			mdname(mddev), (unsigned long long)jrow, phase,
			le32_to_cpu(rj.old_raid_disks), le32_to_cpu(rj.old_m),
			le32_to_cpu(rj.new_raid_disks), le32_to_cpu(rj.new_m),
			le32_to_cpu(rj.chunk_sectors));
		return 0;
	}

	if (phase == RAIDKM_PH_IDLE)
		return 0;			/* freshly initialized journal */

	switch (phase) {
	case RAIDKM_PH_COMMIT:
		err = raidkm_reshape_replay_commit(conf, jrow,
						   le32_to_cpu(rj.data_csum));
		if (err) {
			pr_warn("md/raidkm:%s: reshape recover: band %llu COMMIT replay failed (%d)\n",
				mdname(mddev), (unsigned long long)jrow, err);
			return err;
		}
		fallthrough;
	case RAIDKM_PH_DONE:
	...
	spin_lock_irq(&conf->device_lock);
	conf->reshape_progress = frontier * (sector_t)chunk * k;
	conf->reshape_safe = conf->reshape_progress;
	spin_unlock_irq(&conf->device_lock);
	mddev->reshape_position = conf->reshape_progress;
	pr_info("md/raidkm:%s: reshape recover: journal band %llu phase %u -> resume at band %llu\n",
		mdname(mddev), (unsigned long long)jrow, phase,
...
/*
 * COW reshape handler — replaces reshape_request's stripe-cache body for the
 * add-parity (prev_m < m) case.  Migrates the band at @sector_nr (a per-disk
 * position), advances the frontier, and returns the per-disk sectors processed
 * (one chunk).  md_do_sync drives the loop and calls end_reshape when sector_nr
 * reaches the per-disk reshape size.
 */
static sector_t raidkm_reshape_cow(struct mddev *mddev, sector_t sector_nr,
				   int *skipped)
{
	struct r5conf *conf = mddev->private;
	int chunk = conf->chunk_sectors;
	 ...
	 * the personality to skip the already-done region.  On a resume jump
	 * straight to the frontier (conf->reshape_progress, set from the journal
	 * by raidkm_reshape_recover) instead of re-migrating committed bands —
	 * which would read their new-layout data as if it were old.
	 */
	if (sector_nr == 0 && conf->reshape_progress > 0 &&
	    conf->reshape_progress != MaxSector) {
		sector_t resume = conf->reshape_progress;

		sector_div(resume, k);		/* array sectors -> per-disk */
	...
	if (row == 0 || row == last_band)
		pr_debug("md/raidkm:%s: cow band row=%llu sector_nr=%llu dev_sectors=%llu chunk=%d k=%d last_band=%llu\n",
			 mdname(mddev), (u64)row, (u64)sector_nr,
			 (u64)mddev->dev_sectors, chunk, k, (u64)last_band);

	memset(&ctx, 0, sizeof(ctx));
	/* + RJ_SLOTS so band records always outrank the start-of-reshape
	 * journal-init IDLE records (seq 0..RJ_SLOTS-1) */
	...
	/*
	 * CLAIM the band: advance reshape_progress to its end so make_request
	 * stalls any NEW I/O to [reshape_safe, reshape_progress) — that window
	 * is exactly this band — while everything below reshape_safe proceeds
	 * immediately in the new geometry and everything above proceeds in the
	 * old.  Then drain I/O that entered before the claim (same quiesce
	 * pulse raid5_start_reshape uses), so the old-home blocks we are about
	 * to read cannot be concurrently re-written: without the claim a write
	 * racing the band migration would be silently lost when the staged
	 * (pre-write) content is committed home.
	 */
	spin_lock_irq(&conf->device_lock);
	conf->reshape_progress = (sector_nr + chunk) * k;
	spin_unlock_irq(&conf->device_lock);
	...
	if (err) {
		...
		spin_lock_irq(&conf->device_lock);
		conf->reshape_progress = (sector_t)sector_nr * k;
		spin_unlock_irq(&conf->device_lock);
		wake_up(&conf->wait_for_reshape);
		*skipped = 1;
		return 0;
	...
	/*
	 * RELEASE the band: its DONE record is journaled, so crash recovery
	 * resumes strictly after it (raidkm_reshape_recover trusts the journal
	 * over the possibly-lagging superblock reshape_position).  That makes
	 * it safe to serve the migrated region in the new geometry right away,
	 * unlike the stock engine whose reshape_safe must lag the superblock
	 * write — so I/O below the frontier never waits longer than one band.
	 */
	spin_lock_irq(&conf->device_lock);
	conf->reshape_safe = conf->reshape_progress;
	spin_unlock_irq(&conf->device_lock);
	...
	/*
	 * The band's I/O is fully synchronous (done above).  Report it as REAL
	 * progress, not "skipped": md_do_sync only accounts io_sectors for
	 * non-skipped sectors, and that accounting is what drives the
	 * sync_speed_{min,max} throttle (dm/LVM sets it via max_recovery_rate;
	 * with *skipped the reshape always runs at full tilt).  md_do_sync adds
	 * @chunk to mddev->recovery_active after we return and nothing would
	 * decrement it (hanging the reshape thread at the next barrier), so
	 * release it here — the transient negative is benign: md_do_sync is
	 * our single caller and only inspects recovery_active after its add.
	 */
 ...
 * geometry (its migrated CRC would flag it later, but the heal would then
 * decode the same rotted bytes straight back out of the new parity).  Failing
 * the band instead leaves the row in the OLD geometry, where the steady-state
 * recheck-then-heal path can still repair the block from the OLD parity; the
 * reshape resumes/re-triggers after a scrub.  An absent slot verifies nothing,
					 ...
					 sector_t dev_sector, struct page *pg)
{
	...
	mutex_lock(&s->lock);
	present = raidkm_csum_expected(s, disk, blk, &want);
	mutex_unlock(&s->lock);
	if (!present)
		return 0;
	got = raidkm_block_crc(conf, pg, 0);
	if (got != want) {
		pr_err("md/raid:%s: dcl reshape: CRC mismatch on migrate read (disk %d sector %llu, stored %08x got %08x) — failing the band; scrub/heal the array and resume\n",
		       mdname(conf->mddev), disk,
		       (unsigned long long)dev_sector, want, got);
		return -EIO;
	}
	return 0;
}
 ...
 * stale slot to the recheck-then-heal backstop, and a crash between the home
...
static void raidkm_reshape_csum_rekey(struct r5conf *conf, sector_t row,
					  ...
					  int npages)
{
	sector_t home = row * conf->chunk_sectors;
	int d, po;
	...
	for (d = 0; d < Nnew; d++) {
		...
		for (po = 0; po < npages; po++) {
			...
			raidkm_csum_store(conf, d,
					  raidkm_csum_blkidx(conf, sec),
					  raidkm_block_crc(conf,
						band[d * npages + po], 0));
		}
	}
				       ...
				       int *skipped)
{
	...
	ctx.dcl_g		= nge->g;		/* NEW group geometry */
	ctx.dcl_s		= nge->s;
	ctx.dcl_nbase		= nge->nbase;
	ctx.dcl_old_g		= conf->prev_dcl->g;	/* OLD, for a g/m/s change */
	ctx.dcl_old_s		= conf->prev_dcl->s;
...
static sector_t reshape_request(struct mddev *mddev, sector_t sector_nr, int *skipped)
{
	...
	/*
	 * The reshape maps logical sectors via the data-disk count of each
	 * geometry.  For add-parity the data count is UNCHANGED (k); only the
	 * parity count grows, so the previous geometry's data count must use
	 * prev_m (raid_disks - max_degraded would use the new m and give k-1,
	 * mis-mapping every source block).  For grow-data prev_m == m, so this
	 * is the prior behavior; new_data_disks always uses the current m.
	 */
	int data_disks = raid_disks - conf->prev_m;
	int new_data_disks = conf->raid_disks - conf->m;
	int i;
	int dd_idx;
	...
	/* raidkm disk-adding reshapes (add-parity m->m+1, or add-data k->k+1) use
	...
	/* declustered reshapes are detected by the dual map (prev_dcl set), which
	 * also covers a spare-count change where the pool size is unchanged
	 * (delta_disks == 0, previous_raid_disks == raid_disks). */
	if (is_raidkm(conf) && conf->dcl && conf->prev_dcl)
	...
	for (i = 0; i < reshape_sectors; i += RAID5_STRIPE_SECTORS(conf)) {
		...
		for (j=sh->disks; j--;) {
			sector_t s;
			/* Skip every parity slot: P (raid5), P+Q (raid6), or all
			 * m parities incl. the intermediates (raidkm m>2).  Using
			 * is_parity_disk keeps this correct for arbitrary m and
			 * is identical to the old pd_idx/qd_idx test for raid4/5/6. */
			if (is_parity_disk(sh, j))
				continue;
...
static inline bool raidkm_dcl_pop_room(struct r5conf *conf, u64 firstg,
				       u64 lastg)
{
	u64 base = (u64)atomic64_read(&conf->reb_mark) >>
		   RAID5_STRIPE_SHIFT(conf);

	/* Past the window is safe only when nothing before these addresses is
	 * still unfinished: pop_done() then takes them in order, each one AT
	 * the base, so none can overrun it.  That also keeps a row wider than
	 * the window admissible (a reshape can grow the chunk after load). */
	return lastg < base + conf->reb_win_size || firstg <= base;
}

/*
 * Population backpressure.
 *
 * raidkm_dcl_pop_done() records finished addresses in a window of
 * reb_win_size granules above the oldest unfinished one (reb_mark) and can
 * only drop anything past it — and a dropped address stops the mark for the
 * rest of the pass, so md re-scans, and on a large, fast array that repeats
 * forever.  The window was sized against md's sync flight window, but that
 * bounds how many stripes are in flight, not how far finished granules can
 * run ahead of the oldest one: that distance is sync speed times the delay
 * of the single slowest stripe, and at 600 MiB/s 64 MiB is a tenth of a
 * second.
 *
 * So bound the producer.  This thread issues or skips every population
 * address and reb_mark never moves backwards within a pass, so if it never
 * issues or skips one at or past reb_mark + window, no completion can land
 * outside the window.  When it would, it waits here instead.
 *
 * What it waits for is one slow stripe.  With rk_bio_sort that stripe's bios
 * may be parked in the ordered-submission queue, which drains at
 * PENDING_IO_MAX or when raid5d's loop exits, and with this thread paused
 * neither is guaranteed soon — so flush it and kick raid5d (delayed and
 * preread-held stripes too) on every lap.  Sleeping also flushes
 * md_do_sync's own plug.
 *
 * @first, @last: the address range the caller is about to issue or feed.
 * Returns false once the sync is interrupted (stop, freeze, error): the
 * caller returns 0, md_do_sync ends the pass, and population resumes from
 * the mark.
 */
static bool raidkm_dcl_pop_admit(struct r5conf *conf, sector_t first,
				 sector_t last)
{
	struct mddev *mddev = conf->mddev;
	u64 firstg = (u64)first >> RAID5_STRIPE_SHIFT(conf);
	u64 lastg = (u64)last >> RAID5_STRIPE_SHIFT(conf);
	unsigned long t0 = jiffies;
	bool warned = false, waited = false;

	while (!raidkm_dcl_pop_room(conf, firstg, lastg)) {
		if (!waited) {	/* rk_dcl_populate "backpressure waits" */
			atomic64_inc(&conf->reb_win_waits);
			waited = true;
		}
		if (test_bit(MD_RECOVERY_INTR, &mddev->recovery))
			return false;
		...
		md_wakeup_thread(mddev->thread);
		wait_event_timeout(conf->reb_win_wait,
				   raidkm_dcl_pop_room(conf, firstg, lastg) ||
				   test_bit(MD_RECOVERY_INTR, &mddev->recovery),
				   HZ / 10);
		if (!warned && time_after(jiffies, t0 + 30 * HZ)) {
			/* a stripe that never finishes would hang md's own
			 * checkpoint too; say which address we are stuck behind */
			pr_warn("md/raid:%s: declustered: population has waited 30s to issue sector %llu; oldest unfinished is %llu\n",
				mdname(mddev), (unsigned long long)last,
 ...
 * column's write-chain traverses the armed disk X in this row (X's own
 * data/parity slot, or another assignment's spare content X was holding),
 * issue that ONE group's stripe as a sync stripe — at most one chain
 * traverses X (vertex-disjointness), so it is still one stripe per row.
 * handle_parity_checks6 reconstructs the chain-root slot and the WRITE-side
 * chained redirect lands it on the live endpoint; completion advances the
 * prefix mark (hook at the md_done_sync site).  Content-free rows (X at an
 * unassigned spare column or on a spare-only cycle) are skipped, but their
					    ...
					    sector_t sector_nr, int *skipped)
{
	...
	sector_t row = sector_nr;
	u64 mark = (u64)atomic64_read(&conf->reb_mark);
	/* md_do_sync hands out contiguous addresses within a pass (j += what
	 ...
	 * pass interrupted after the once-failed address reconstructs is picked
	 * up again by raid5d.  Still failing: handle_failed_sync pauses again. */
	if (pass_start && READ_ONCE(conf->reb_pop_stuck)) {
	...
	/*
	 * A pass that STARTS above the mark can never move it: the oldest
	 * unfinished address lies below the start, nothing below the start is
	 * issued again, and md cannot go back.  md starts a repair at
	 * resync_min — a leftover sync_min, or curr_resync_completed after an
	 * interrupted pass, which can be past an address that failed.  Waiting
	 * would hang raidkm_dcl_pop_admit(); feeding the gap would publish spare
	 * blocks nobody wrote.  End the pass at once instead: a pass that
	 * COMPLETES, unlike an interrupted one, makes md reset resync_min to 0,
	 * and the next pass fast-forwards to the mark from there.
	 *
	 * Against the mark itself, so no unfinished address can hide below the
	 * start.  md_done_sync() runs just before raidkm_dcl_pop_done() in the
	 * completion path, so the mark can briefly trail a drained pass — that
	 * only ever costs one quick extra pass, never a hang.  Only at pass
	 * start: mid-pass, addresses above a failure are normal (the sync thread
	 * runs up to a window ahead of the failing stripe).
	 */
	if (pass_start && sector_nr > mark) {
		pr_info_ratelimited("md/raid:%s: declustered: population pass starts at sector %llu, past its mark %llu; ending it so the next pass restarts from the mark\n",
				    mdname(mddev), (unsigned long long)sector_nr,
	...
	if (root < 0) {
		...
		/* the whole row is fed to the mark below: admit it as a range */
		if (!raidkm_dcl_pop_admit(conf, sector_nr, sector_nr + sk - 1))
			return 0;
	...
	raid5_release_stripe(sh);
	raidkm_dcl_maybe_checkpoint(conf);
	ret = RAID5_STRIPE_SECTORS(conf);
out:
	conf->reb_next = (u64)sector_nr + ret;	/* where md's next call lands */
	return ret;
}
 ...
 * context (blocking OK). */
/* Scrub a vacated reb[] entry.  STATE FIRST: the lock-free readers that
 * pre-check state (raid5d's COPYING nudge) must never pair a still-COPYING
 * state with an already-scrubbed disk of -1 (conf->disks[-1] is an OOB
 * read); a reader that instead matches the old disk and then reads state
 * NONE simply stops redirecting — the desired post-retire behaviour. */
static void rkdcl_reb_scrub(struct rkdcl_reb *r)
{
	WRITE_ONCE(r->state, RKDCL_ASSIGN_NONE);
	smp_wmb();		/* state visible before the ids vanish */
	r->disk  = -1;
	r->spare = -1;
}
...
{
	struct r5conf *conf = mddev->private;
	int saved_nreb, saved_pop;
	int i, err;

	mutex_lock(&conf->dcl_retire_mutex);
	saved_nreb = conf->nreb;
	saved_pop = conf->reb_pop;
	if (!saved_nreb) {
		mutex_unlock(&conf->dcl_retire_mutex);
		return 0;
	}
	raid5_quiesce(mddev, 1);
	/* shrink-first: entries above nreb are untouched, so the failure
	 * path can restore by just re-widening the count.  mddev->lock
	 * keeps the sysfs show from printing a torn view. */
...
/* Retire exactly ONE assignment — the copied one — keeping every other
 * assignment (and its populated spare content) intact.  Safe ONLY when
 * nothing can ever trust un-copied bytes on the slot's device:
 *
 *   - the copy COMPLETED: R holds the slot's FULL role content, hosted
 *     chains included, because the copy sources every row via
 *     raidkm_dcl_chain_root (another assignment's chain stopping at the
 *     now-live R finds its content there); or
 *   - the replacement is GONE: a chain stopping at a dead endpoint
 *     decodes via parity (consistent with logical content throughout).
 *
 * A PARTIAL-copy abort with R still present must retire-all instead: the
 * follow-up decode RECOVER rebuilds only X's own group columns, never the
 * spare content X hosts for other assignments, so a surviving assignment's
 * chain would read garbage from R once its recovery_offset sweeps past.
 *
 * Same discipline as retire_all: full quiesce (no lock-free reb[] reader
 * in flight), journal-first strict quorum (a stale lower-gen survivor
 * elected later would resurrect the retired assignment and redirect the
 * now-live slot to a stale spare), restore inside the quiesce on journal
 * failure.  The array is compacted under the quiesce; the vacated tail
 * entry is scrubbed so a later arm's publication window never exposes it. */
static int raidkm_dcl_retire_one(struct mddev *mddev, int disk,
				 const char *why)
{
	struct r5conf *conf = mddev->private;
	struct rkdcl_reb saved;
	int saved_pop, remain;
	int idx, i, err;

	/* Retire BY DISK and revalidate under the mutex: the callers'
	 * trigger checks (sync thread, rescue worker) are lock-free and can
	 * both fire for the same completion/loss — the loser must find the
	 * entry already gone and no-op, never compact a second time or hit
	 * the entry that shifted into a stale index. */
	mutex_lock(&conf->dcl_retire_mutex);
	for (idx = 0; idx < conf->nreb; idx++)
		if (conf->reb[idx].disk == disk &&
		    conf->reb[idx].state == RKDCL_ASSIGN_COPYING)
			break;
	if (idx == conf->nreb) {
		mutex_unlock(&conf->dcl_retire_mutex);
		return 0;		/* already retired (lost the race) */
	}
	saved = conf->reb[idx];
	saved_pop = conf->reb_pop;
	raid5_quiesce(mddev, 1);
	/* compact under the quiesce + mddev->lock (stripe-path readers are
	 * drained; the sysfs show and raid5d's lock-free pre-checks see
	 * either the old or the new consistent view, never a torn one) */
	spin_lock(&mddev->lock);
	for (i = idx; i < conf->nreb - 1; i++)
		conf->reb[i] = conf->reb[i + 1];
	conf->nreb--;
	rkdcl_reb_scrub(&conf->reb[conf->nreb]);
	WRITE_ONCE(conf->reb_pop, -1);
	spin_unlock(&mddev->lock);
	err = raidkm_dcl_journal_write(conf, true);
	spin_lock(&mddev->lock);
	if (err) {
		for (i = conf->nreb; i > idx; i--)
			conf->reb[i] = conf->reb[i - 1];
		conf->reb[idx] = saved;
		conf->nreb++;
		WRITE_ONCE(conf->reb_pop, saved_pop);
	}
	remain = conf->nreb;	/* captured before the quiesce drops: an
				 * auto-arm can bump nreb the moment I/O
				 * resumes and the log (which the gates
				 * parse) must match the journaled state */
	spin_unlock(&mddev->lock);
	raid5_quiesce(mddev, 0);
	mutex_unlock(&conf->dcl_retire_mutex);
	if (!err)
		pr_info("md/raid:%s: declustered: spare assignment retired for disk %d (%s; %d assignment(s) remain)\n",
			mdname(mddev), saved.disk, why, remain);
	else
		pr_warn("md/raid:%s: declustered: retire of disk %d (%s) journal failed (%d); will retry\n",
			mdname(mddev), saved.disk, why, err);
	return err;
}
...
static void raidkm_dcl_rescue_worker(struct work_struct *work)
{
	...
	if (!mddev_trylock(mddev))
		return;
	/* single snapshot + bounds: a concurrent retire stores reb_pop=-1
	 * and compacts — a stale index must never be dereferenced (bound by
	 * the allocation size s; retire_one revalidates BY DISK anyway) */
	pop = READ_ONCE(conf->reb_pop);
	if (pop >= 0 && pop < (int)conf->dcl->s &&
	    READ_ONCE(conf->reb[pop].state) == RKDCL_ASSIGN_COPYING) {
		int X = READ_ONCE(conf->reb[pop].disk);
		struct md_rdev *rdev = (X >= 0 && X < conf->raid_disks) ?
			conf->disks[X].rdev : NULL;

		/* both shapes are retire-ONE-safe (see raidkm_dcl_retire_one):
		 * an absent replacement is a dead endpoint (decode), a
		 * complete copy holds the full role content */
		if (X < 0 || X >= conf->raid_disks)
			;		/* torn read mid-retire: nothing to do */
		else if (!rdev || test_bit(Faulty, &rdev->flags))
			raidkm_dcl_retire_one(mddev, X,
 ...
 * the per-stripe decode with a straight copy of X's already-populated content
 * from the per-row chain endpoint (raidkm_dcl_copy_src — the rotating spare
 * column when X's is the only assignment) into R.  Correctness model B (per-band
 * quiesce + offset-split, no mirror): the band is copied under a raid5_quiesce
 * so no user I/O races the mark advance; the journaled mark is then the
 ...
 * SOURCE disk's key — the per-row chain endpoint the bytes were read from
 * (raidkm_dcl_copy_src; the rotating spare in the single-assignment case)
 * — to the replacement's (slot X) key alongside the bytes.  MIGRATE, never recompute: recomputing from the copied bytes would
...
/* Multi-assignment copy: the disk a copy row is sourced from.  R must
 * become a byte-exact stand-in for X's whole ROLE — its own group column
 * AND any spare content it hosts for OTHER assignments' chains (the
 * COPYING redirect already routes those chains to R below the mark, and
 * retire_one relies on them finding their content there).  So the source
 * is the live endpoint of the chain rooted at chain_root(row, X):
 * X's own column when it is a group column, the hosting assignment's
 * content otherwise.  Rows the band processes are >= the mark, so the
 * COPYING hop at X always continues spare-ward — the walk can never
 * resolve to R itself.  Returns -1 for a content-free row (X holds an
 * unassigned spare column / spare-only cycle: R holds nothing there and
 * nothing ever reads it).  Single-assignment reduces to the old
 * spare_disk(row) exactly.
 *
 * Returns -1 for a content-free row (skip), -EIO if the walk resolved to
 * X itself — impossible while the band-rows >= mark contract holds, and
 * copying R onto itself would silently bless garbage, so the caller must
 * treat it as a band error, never a success. */
static int raidkm_dcl_copy_src(struct r5conf *conf, u64 row, int X)
{
	int root = raidkm_dcl_chain_root(conf, row, X);
	int src;

	if (root < 0)
		return -1;
	src = raidkm_dcl_redirect(conf,
				  dcl_disk(conf->dcl, row, (u32)root),
				  row, false);
	if (WARN_ON_ONCE(src == X))
		return -EIO;
	return src;
}
...
static void raidkm_dcl_copy_worker(struct work_struct *work)
{
	...
	while (!atomic_read(&b->error)) {
		...
		if (row >= b->row_end)
			break;
		sd = raidkm_dcl_copy_src(conf, row, b->X);
		if (sd == -1)
			continue;	/* content-free row for X: R holds
					 * nothing here (never read) */
		if (sd < 0) {		/* -EIO: walk resolved to X (contract
					 * breach) — abort, never self-copy */
					...
					sector_t sector_nr, int *skipped)
{
	 ...
	 * records at an INTR.
	 *
	 * The retire mutex is held for the whole band: it freezes reb[]
	 * for the workers' lock-free chain walks (raid5_quiesce drains
	 * stripes, not workqueue sync_page_io) and blocks the rescue
	 * worker's retire, whose own quiesce(1)/quiesce(0) pair would
	 * otherwise un-quiesce the array from under the band (conf->quiesce
	 * is a state machine, not a refcount).
	 */
	mutex_lock(&conf->dcl_retire_mutex);
	/* revalidate under the mutex: the dispatch check in sync_request
	 * was lock-free and a retire can land in between (reb_pop then
	 * reads -1 → reb[-1] wild read without this) */
	pop = conf->reb_pop;
	if (pop < 0 || pop >= (int)conf->dcl->s ||
	...
	X = conf->reb[pop].disk;
	mark = (u64)atomic64_read(&conf->reb_mark);
	...
	if (!rdst || test_bit(Faulty, &rdst->flags)) {
		/* Replacement lost mid-copy: retire THIS assignment (§5-safe
		 * — X decodes via parity; below-mark spare may be stale but
		 * is never read once NONE; a dead endpoint is retire-ONE-safe
		 * so other assignments keep their populated spares) instead
		 * of livelocking on INTR re-nudges.  Journal failure keeps
		 * COPYING; raid5d's rescue retries. */
		mutex_unlock(&conf->dcl_retire_mutex);
		raidkm_dcl_retire_one(mddev, X, "replacement lost mid-copy");
		set_bit(MD_RECOVERY_INTR, &mddev->recovery);
		*skipped = 1;
		return 0;
	}
	...
	if (!nwk) {
		 ...
		 * rather than livelocking on retries.  Retire-ALL, not one:
		 * R survives with a partial copy, and the decode RECOVER
		 * never rebuilds hosted spare content (see retire_one). */
		mutex_unlock(&conf->dcl_retire_mutex);
	...
	if (!atomic_read(&band.error) &&
		...
		int sd = raidkm_dcl_copy_src(conf, row, X);
		unsigned int tail_bytes =
			(band_end - row * conf->chunk_sectors) << 9;

		if (sd < -1)		/* -EIO contract breach: never
					 * self-copy (see copy_src) */
	...
	if (atomic_read(&band.error)) {
		 ...
		 * RECOVER never rebuilds hosted spare content (retire_one). */
				   ...
				   sector_t max_sector, int *skipped)
{
	...
	unsigned int row_win;
	int tslot = -1;

	/*
	 * No sync pass touches a stripe while a full-row write is in flight:
	 * the row write bypasses the stripe cache, so a resync or check
	 * reading the row under it would see some members old and some new,
	 * and a resync would then "repair" the parity from that mix.  Row
	 * writes decline once MD_RECOVERY_RUNNING is set; here the pass waits
	 * out the ones already issued (raidkm_row_write_prep: its count, then
	 * the flag; here the flag was set before the thread ran, then the
	 * count).  Free when none are in flight.
	 */
	smp_mb();
	wait_event(conf->row_write_wait, !atomic_read(&conf->row_writes));

	/*
	 * raid_km redesign: m > 2 resync/scrub goes through the synchronous
	 * ISA-L verify+repair path in handle_parity_checks6 / ops_run_check_pq
	 * (re-encode each parity from data, compare, repair).  A *degraded*
	 * m > 2 array is handled too: the fill path reconstructs any missing
	 * data slot via the gf_invert_matrix decode (Phase 6) before the
	 * verify runs, and ops_run_check_pq skips parity on failed disks.
	 */
	...
	if (sector_nr >= max_sector) {
		 ...
		 * from it. */
		/* single snapshot + bounds on the in-flight index: a
		 * concurrent rescue retire stores reb_pop = -1 (the reb[-1]
		 * wild-read class); retire_one revalidates by disk anyway */
		cpop = -1;
		cst = RKDCL_ASSIGN_NONE;
		if (conf->dcl) {
			cpop = READ_ONCE(conf->reb_pop);
			if (cpop >= 0 && cpop < (int)conf->dcl->s)
				cst = READ_ONCE(conf->reb[cpop].state);
			else
				cpop = -1;
		}
		if (cst == RKDCL_ASSIGN_COPYING) {
			...
			u64 mark = (u64)atomic64_read(&conf->reb_mark);
			int rd = READ_ONCE(conf->reb[cpop].disk);
		...
		} else if (cpop >= 0 && cst == RKDCL_ASSIGN_POPULATING &&
		    test_bit(MD_RECOVERY_REQUESTED, &mddev->recovery)) {
			u64 mark = (u64)atomic64_read(&conf->reb_mark);

			/* the pass is over: the next populate call starts one */
			conf->reb_next = U64_MAX;
			/* A pass that ENDED CLEANLY hit no unreconstructable
			 * address — that INTRs it, via md_done_sync(ok=0), which
			 * lands before this call even from the final drain — so
			 * any pause is from an earlier pass: lift it and let
			 * raid5d retry.  This is also what makes a repair count
			 * when its loop never ran (resync_min at the device end). */
			if (!test_bit(MD_RECOVERY_INTR, &mddev->recovery)) {
				WRITE_ONCE(conf->reb_pop_stuck, false);
				WRITE_ONCE(conf->reb_fail_sector, U64_MAX);
			}
			...
			if (!test_bit(MD_RECOVERY_INTR, &mddev->recovery) &&
				...
				struct rkdcl_reb *r = &conf->reb[cpop];

				r->state = RKDCL_ASSIGN_POPULATED;
				conf->reb_pop = -1;
				pr_info("md/raid:%s: declustered: population of disk %d into spare col %d COMPLETE — redundancy restored (%d assignment(s) active)\n",
					mdname(mddev), r->disk, r->spare,
					conf->nreb);
			}
			raidkm_dcl_journal_write(conf, false);
		}
		 ...
		 * helpers (bitmap_start_sync / cond_end_sync / end_sync /
		 * close_sync) dereference mddev->bitmap *without* a NULL guard
		 * (verified by disassembling the running bitmap_start_sync:
		 * it loads mddev->bitmap and calls _raw_spin_lock_irq on it
		 * with no NULL check).  A raidkm array created with
		 * --bitmap=none has mddev->bitmap == NULL, so any of these
		 * calls panics.  Stock raid5/6 reaches them only with a bitmap
		 * present; gate them on the bitmap actually existing. */
	...
	/* §13 guard: a STOCK recover must never run while assignments are
	 * live unless it is the armed copy above.  Reachable when an
	 * ASSEMBLE carries a recovering replacement whose COPYING arm
	 * journal a crash beat (mdadm's --add wrote R's slot-assigning SB
	 * before the arm; the hot-add shapes are handled in
	 * raid5_add_disk): under a redirect, stock rebuild writes to an
	 * ASSIGNED slot land on the spare (R would be blessed In_sync
	 * without ever being written), and hosted spare content on an
	 * unassigned recovering slot is never rebuilt.  Retire everything
	 * (the all-or-nothing rule — every slot then decodes) and let the
	 * restarted recovery run as the validated decode leg.  Journal
	 * failure: claim no progress; the next pass retries. */
	if (conf->dcl && conf->nreb &&
	    test_bit(MD_RECOVERY_RECOVER, &mddev->recovery) &&
	    !test_bit(MD_RECOVERY_REQUESTED, &mddev->recovery) &&
	    !test_bit(MD_RECOVERY_SYNC, &mddev->recovery)) {
		raidkm_dcl_retire_all(mddev,
				      "unarmed recovery under live assignments");
		set_bit(MD_RECOVERY_INTR, &mddev->recovery);
		*skipped = 1;
		return 0;
	}
	 ...
	 * POPULATING only — reb_pop also indexes a COPYING entry for whole
	 * copy sessions, and a user check/repair must then fall through to
	 * the normal (redirect-aware) machinery, NOT drive a population
	 * pass that advances the copy's mark without writing R. */
	if (conf->dcl && conf->reb_pop >= 0 &&
	    conf->reb[conf->reb_pop].state == RKDCL_ASSIGN_POPULATING &&
	    test_bit(MD_RECOVERY_REQUESTED, &mddev->recovery))
	...
	/* RHEL 10.2 port fix #9 (cont.): only consult the bitmap when one
	 * exists.  Without a bitmap, bitmap_start_sync would conceptually
	 * "always resync", so the skip-block optimisation simply does not
	 * apply (matches the NULL-bitmap behaviour of the upstream helper). */
	if (mddev->bitmap &&
	    !test_bit(MD_RECOVERY_REQUESTED, &mddev->recovery) &&
	    !conf->fullsync &&
	 ...
	 * workers rebuilding chunks the stripe cache is about to take whole
	 * windows of.  (A call that merely is not on the chunk grid keeps the
	 * window: that is the stripe cache finishing ONE declined chunk.)
	 */
	if (!row_win)
		raidkm_row_win_drain(conf, conf->row_rb);
	if (row_target && !(sector_nr % conf->chunk_sectors) &&
	    sector_nr + conf->chunk_sectors <= max_sector) {
		unsigned long band_started;
		sector_t did;

		raidkm_row_rb_pace_wait(mddev);
		band_started = jiffies;
	...
	for (submitted = 1;
		 ...
		 * preread_active_stripes is nonzero whenever stripes are queued
		 * for I/O (reads, and the read phase of RMW writes).  When it is
		 * zero (rebuild only, Phase 2) the window fills to the normal
		 * 75% limit with no penalty. */
		 ...
		 * stripes.  (Without a bitmap every region is synced, as before;
		 * the next sync_request call skips a clean run via the bitmap
...
static void raid5d(struct md_thread *thread)
{
	...
	if (conf->dcl && conf->reb_pop >= 0 &&
	    conf->reb[conf->reb_pop].state == RKDCL_ASSIGN_POPULATING &&
	    md_is_rdwr(mddev) && !READ_ONCE(conf->reb_pop_stuck) &&
	    !test_bit(MD_RECOVERY_RUNNING, &mddev->recovery) &&
	 ...
	 * two burst failures (want occupied for the whole pre-arm window,
	 * which spans entire recoveries/populations) and (b) disks left
	 * missing after retire-all — raid5_error never re-fires for an
	 * already-Faulty member.  Re-derive the want from the array state
	 * instead of trusting the one-shot handoff: any missing/Faulty
	 * member not covered by an active assignment gets parked.  Same
	 * lock as the error handler's park (device_lock). */
	if (conf->dcl && conf->dcl_auto && md_is_rdwr(mddev) &&
	    READ_ONCE(conf->reb_want) < 0 && conf->reb_pop < 0 &&
	    conf->nreb < (int)conf->dcl->s &&
	...
	/* Declustered auto-arm service: a failed slot was parked in
	 * reb_want (by the error handler or the rescan above; neither can
	 * journal from its context).  Arm it here through the same path as
	 * the sysfs store.  trylock only — raid5d must never sleep on
	 * mddev_lock (md_check_recovery precedent); a failed trylock, a
	 * running recovery, or a running population (arm returns -EBUSY)
	 * retries on the next pass — a second failure's population starts
...
int
raid_km_set_cache_size(struct mddev *mddev, int size)
{
	int result = 0;
	struct r5conf *conf = mddev->private;
...
EXPORT_SYMBOL(raid_km_set_cache_size);
...
raid5_store_stripe_cache_size(struct mddev *mddev, const char *page, size_t len)
{
	...
	if (!conf)
		...
		err = raid_km_set_cache_size(mddev, new);
	mddev_unlock(mddev);
...
raidkm_show_dcl_populate(struct mddev *mddev, char *page)
{
	...
	if (conf && conf->dcl) {
		...
		for (i = 0; i < conf->nreb && ret < (ssize_t)PAGE_SIZE - 64;
		     i++) {
			struct rkdcl_reb *r = &conf->reb[i];

			if (r->state == RKDCL_ASSIGN_POPULATING)
				ret += sprintf(page + ret,
					"populating %d -> spare %d mark %llu/%llu\n",
					r->disk, r->spare,
					(unsigned long long)atomic64_read(&conf->reb_mark),
					(unsigned long long)mddev->dev_sectors);
			else if (r->state == RKDCL_ASSIGN_COPYING)
				ret += sprintf(page + ret,
					"copying %d <- spare %d mark %llu/%llu\n",
					r->disk, r->spare,
					(unsigned long long)atomic64_read(&conf->reb_mark),
					(unsigned long long)mddev->dev_sectors);
			else
		...
		/* how often population had to wait for its slowest stripe
		 * (raidkm_dcl_pop_admit) — a trailing line, so every reader
		 * matching the state prefix or "mark N/" is unaffected */
		if (conf->nreb)
			ret += sprintf(page + ret, "backpressure waits %llu\n",
				(unsigned long long)atomic64_read(&conf->reb_win_waits));
		/* rk_dcl_row_rebuild: rows the row engine wrote whole, and
		 * rows it handed back to the stripe path.  Also trailing. */
		if (conf->nreb && READ_ONCE(conf->dcl_row_rebuild))
			ret += sprintf(page + ret,
				"row rows %llu declined %llu\n",
				(unsigned long long)atomic64_read(&conf->dcl_row_pop_rows),
				(unsigned long long)atomic64_read(&conf->dcl_row_pop_declined));
		pf = READ_ONCE(conf->reb_fail_sector);
		if (conf->nreb && READ_ONCE(conf->reb_pop_stuck)) {
			if (pf != U64_MAX)
				ret += sprintf(page + ret,
					"paused: cannot reconstruct sector %llu\n",
					(unsigned long long)pf);
			else
				ret += sprintf(page + ret, "paused\n");
		}
		if (!conf->nreb)
...
static int raidkm_dcl_arm(struct mddev *mddev, unsigned int x)
{
	...
	if (x >= (unsigned int)mddev->raid_disks)
		return -EINVAL;
	if (conf->reb_pop >= 0 ||	/* populations are sequential */
	    test_bit(MD_RECOVERY_RUNNING, &mddev->recovery))
		return -EBUSY;
	if (conf->nreb >= (int)conf->dcl->s)
		return -ENOSPC;		/* every spare column consumed */
	for (i = 0; i < conf->nreb; i++)
		if (conf->reb[i].disk == (int)x)
			return -EEXIST;	/* already assigned */
	...
	conf->reb_win_base = 0;
	bitmap_zero(conf->reb_win_bits, conf->reb_win_size);
	atomic64_set(&conf->reb_win_waits, 0);
	WRITE_ONCE(conf->reb_fail_sector, U64_MAX);
	WRITE_ONCE(conf->reb_pop_stuck, false);
	conf->reb_next = U64_MAX;
	spin_unlock_irqrestore(&conf->reb_win_lock, flags);
	atomic64_set(&conf->reb_mark, 0);
	/* Publication order matters: the redirect readers run lock-free on
	 * the I/O path and gate on nreb — the entry fields (and reb_pop)
	 * must be globally visible BEFORE the count admits them, or a
	 * reader chains through a half-written entry (a zeroed entry
	 * matches healthy disk 0; a stale retired entry redirects a
	 * healthy replaced disk).  smp_store_release pairs with the
	 * smp_load_acquire in raidkm_dcl_redirect/chain_root. */
	conf->reb[conf->nreb].disk  = x;
	conf->reb[conf->nreb].spare = j;
	conf->reb[conf->nreb].state = RKDCL_ASSIGN_POPULATING;
	WRITE_ONCE(conf->reb_pop, conf->nreb);
	smp_store_release(&conf->nreb, conf->nreb + 1);
	err = raidkm_dcl_journal_write(conf, true);
	if (err) {
		/* Retract count-first (readers seeing the old count may
		 * still finish a walk over the valid POPULATING entry —
		 * benign: reads decode, writes are §1-idempotent). */
		smp_store_release(&conf->nreb, conf->nreb - 1);
		WRITE_ONCE(conf->reb_pop, -1);
		return err;
	}
	pr_info("md/raid:%s: declustered: population ARMED: disk %u -> spare col %d (%d assignment(s) active)\n",
		mdname(mddev), x, j, conf->nreb);
	set_bit(MD_RECOVERY_REQUESTED, &mddev->recovery);
	set_bit(MD_RECOVERY_SYNC, &mddev->recovery);
	set_bit(MD_RECOVERY_NEEDED, &mddev->recovery);
	md_wakeup_thread(mddev->thread);
...
raidkm_store_dcl_reshape(struct mddev *mddev, const char *page, size_t len)
{
	...
	if (len >= PAGE_SIZE)
		return -EINVAL;
	/* "<newN>:<seed_hex>[:<newlayout_hex>]" — the optional new layout word
	 * carries a group-geometry change (add-parity/add-data/spare-count);
	 * omit it for a pure pool expansion (g/m/s unchanged). */
	nf = sscanf(page, "%u:%llx:%x", &newN, &newseed, &newlay);
	if (nf < 2)
		return -EINVAL;
	...
	/* the layout word must keep the declustered bit: a non-dcl word would
	 * misroute the (delta_disks == 0) spare-count case down the classic
	 * in-place-reshape path in raid5_run / check_reshape.  It must also
	 * keep the native-checksum bit: a reshape only moves/re-keys CRCs, it
	 * cannot enable or disable the feature — a word missing the bit would
	 * silently drop checksumming when finish_reshape persists it. */
	if (!raidkm_layout_is_dcl((int)newlay) ||
	    raidkm_layout_has_csum((int)newlay) !=
...
/*
 * worker_thread_cnt: the same setting expressed as TOTAL worker threads,
 * which is the natural mental model for users ("I want N parallel workers").
 * Reads return worker_cnt_per_group * group_cnt; writes divide by the number
 * of groups (= num_possible_nodes(), what alloc_thread_groups() picks),
 * rounding up so the array gets AT LEAST the requested total.  This is a
 * raidkm-specific alias for group_thread_cnt; both are R/W and refer to the
 * same underlying state.
 */
static ssize_t
raid5_show_worker_thread_cnt(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	spin_lock(&mddev->lock);
	...
	if (conf)
		ret = sprintf(page, "%d\n",
			      conf->worker_cnt_per_group * conf->group_cnt);
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raid5_store_worker_thread_cnt(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned int new_total, new_per_group, nodes;
	int err;
	struct r5worker_group *new_groups, *old_groups;
	int group_cnt;
	...
	if (len >= PAGE_SIZE)
		return -EINVAL;
	if (kstrtouint(page, 10, &new_total))
		return -EINVAL;
	if (new_total > 8192)
		return -EINVAL;
	...
	/* Defensive: a running kernel always has node 0 in the possible map,
	 * but a broken invariant upstream shouldn't OOPS us with a div-by-0. */
	if (WARN_ON_ONCE(nodes == 0))
		nodes = 1;
	if (new_total == 0)
		new_per_group = 0;
	else
		/* round up so total >= requested */
		new_per_group = (new_total + nodes - 1) / nodes;
...
static struct md_sysfs_entry
raid5_worker_thread_cnt = __ATTR(worker_thread_cnt, S_IRUGO | S_IWUSR,
				 raid5_show_worker_thread_cnt,
				 raid5_store_worker_thread_cnt);

#ifdef RAIDKM_FAULT_INJECT
/*
 * raidkm_reshape_inject — debug knob for the COW-staged reshape crash tests
 * (tools/raidkm-test-reshape-crash.sh).  Format: "<band>:<phase>:<action>",
 * or "off"/empty to disarm.  See raid_km_reshape.h for the contract.  The
 * reshape thread consults raidkm_reshape_inject_decide() at each phase boundary
 * and parks (and, for "torn", torn-writes the next step) when the point matches.
 */
static const char *raidkm_phase_name(enum raidkm_reshape_phase p)
{
	switch (p) {
	case RAIDKM_PH_STAGE:	return "STAGE";
	case RAIDKM_PH_COMMIT:	return "COMMIT";
	case RAIDKM_PH_DONE:	return "DONE";
	case RAIDKM_PH_FINALIZE: return "FINALIZE";
	default:		return "IDLE";
	}
}

static int raidkm_parse_phase(const char *s, enum raidkm_reshape_phase *out)
{
	if (sysfs_streq(s, "STAGE"))	*out = RAIDKM_PH_STAGE;
	else if (sysfs_streq(s, "COMMIT"))	*out = RAIDKM_PH_COMMIT;
	else if (sysfs_streq(s, "DONE"))	*out = RAIDKM_PH_DONE;
	else if (sysfs_streq(s, "FINALIZE"))	*out = RAIDKM_PH_FINALIZE;
	else return -EINVAL;
	return 0;
}

static int raidkm_parse_band(const char *s, long *out)
{
	if (sysfs_streq(s, "first"))	{ *out = RAIDKM_INJ_BAND_FIRST; return 0; }
	if (sysfs_streq(s, "last"))	{ *out = RAIDKM_INJ_BAND_LAST;  return 0; }
	if (sysfs_streq(s, "mid"))	{ *out = RAIDKM_INJ_BAND_MID;   return 0; }
	return kstrtol(s, 10, out) ? -EINVAL : 0;
}

static ssize_t
raidkm_reshape_inject_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	struct raidkm_reshape_inject *inj;
	int ret;

	spin_lock(&mddev->lock);
	conf = mddev->private;
	if (!conf || !is_raidkm(conf)) {
		ret = sprintf(page, "off\n");
	} else {
		inj = &conf->reshape_inject;
		if (inj->parked)
			ret = sprintf(page, "parked@%ld:%s\n", inj->parked_band,
				      raidkm_phase_name(inj->parked_phase));
		else if (inj->action != RAIDKM_INJ_OFF)
			ret = sprintf(page, "armed %ld:%s:%s\n", inj->band,
				      raidkm_phase_name(inj->phase),
				      inj->action == RAIDKM_INJ_TORN ? "torn" : "hang");
		else
			ret = sprintf(page, "off\n");
	}
	spin_unlock(&mddev->lock);
...
static ssize_t
raidkm_reshape_inject_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	char buf[64], *p, *tok;
	long band;
	...
	if (*p == '\0' || sysfs_streq(p, "off")) {
		band = RAIDKM_INJ_BAND_FIRST;
		phase = RAIDKM_PH_IDLE;
		action = RAIDKM_INJ_OFF;
	} else {
		tok = strsep(&p, ":");
		if (!tok || raidkm_parse_band(tok, &band))
			return -EINVAL;
		tok = strsep(&p, ":");
		if (!tok || raidkm_parse_phase(tok, &phase))
			return -EINVAL;
		tok = strsep(&p, ":");
		if (!tok)
			return -EINVAL;
		if (sysfs_streq(tok, "hang"))		action = RAIDKM_INJ_HANG;
		else if (sysfs_streq(tok, "torn"))	action = RAIDKM_INJ_TORN;
		else return -EINVAL;
	}
	...
	if (!conf || !is_raidkm(conf)) {
		...
		spin_lock_irq(&conf->device_lock);
		conf->reshape_inject.band = band;
		conf->reshape_inject.phase = phase;
		conf->reshape_inject.action = action;
		conf->reshape_inject.parked = false;
		spin_unlock_irq(&conf->device_lock);
	}
	mddev_unlock(mddev);
	return err ?: len;
...
static struct md_sysfs_entry
raidkm_reshape_inject = __ATTR(raidkm_reshape_inject, S_IRUGO | S_IWUSR,
			       raidkm_reshape_inject_show,
			       raidkm_reshape_inject_store);

/*
 * Consulted by the reshape thread after it has durably written @phase's journal
 * record for the band at ordinal @band_idx (with @last_band the final ordinal).
 * Returns the action to take: OFF (proceed), HANG (park here), or TORN (the
 * caller torn-writes the next step, then parks).  Marks the parked state so the
 * sysfs read-back reports "parked@<band>:<phase>".  Caller holds no conf lock.
 */
static enum raidkm_inject_action __maybe_unused
raidkm_reshape_inject_decide(struct r5conf *conf, long band_idx, long last_band,
			     enum raidkm_reshape_phase phase)
{
	struct raidkm_reshape_inject *inj = &conf->reshape_inject;
	long want;
	enum raidkm_inject_action action;

	if (inj->action == RAIDKM_INJ_OFF || inj->phase != phase)
		return RAIDKM_INJ_OFF;

	switch (inj->band) {
	case RAIDKM_INJ_BAND_LAST:	want = last_band;	break;
	case RAIDKM_INJ_BAND_MID:	want = last_band / 2;	break;
	default:			want = inj->band;	break;
	}
	if (band_idx != want)
		return RAIDKM_INJ_OFF;
	...
	spin_lock_irq(&conf->device_lock);
	inj->parked = true;
	inj->parked_band = band_idx;
	inj->parked_phase = phase;
	spin_unlock_irq(&conf->device_lock);
	pr_warn("md/raidkm:%s: reshape fault-inject %s at band %ld phase %s\n",
		mdname(conf->mddev),
		action == RAIDKM_INJ_TORN ? "torn" : "hang",
		band_idx, raidkm_phase_name(phase));
	return action;
}

/*
 * raidkm_reshape_jtest — DEBUG self-test of the journal/scratch primitives
 * (raid_km-reshape.c).  Writing a value runs jwrite(COMMIT, seq) + jread and
 * logs PASS/FAIL; reading shows the current on-disk record.  DESTRUCTIVE: it
 * writes to the scratch zone (presently the data-region tail), so run it only
 * on a throwaway array.
 */
static ssize_t
raidkm_reshape_jtest_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf = mddev->private;
	struct raidkm_reshape_journal rj;
	int err;
	...
	err = raidkm_reshape_jread(conf, &rj);
	if (err)
		return sprintf(page, "no-record (%d)\n", err);
	return sprintf(page,
		       "seq=%llu phase=%s band=%llu chunks=%u dcsum=0x%08x pos=%llu\n",
		       le64_to_cpu(rj.seq), raidkm_phase_name(le32_to_cpu(rj.phase)),
		       le64_to_cpu(rj.band_start_chunk), le32_to_cpu(rj.band_chunks),
		       le32_to_cpu(rj.data_csum), le64_to_cpu(rj.reshape_position));
}

static ssize_t
raidkm_reshape_jtest_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	struct raidkm_reshape_ctx ctx = {};
	struct raidkm_reshape_journal rj;
	...
	ctx.old_m		= conf->prev_m;
	ctx.new_m		= conf->m;
	ctx.old_raid_disks	= conf->raid_disks;
	ctx.new_raid_disks	= conf->raid_disks + 1;
	ctx.chunk_sectors	= conf->chunk_sectors;

	err = raidkm_reshape_jwrite(conf, &ctx, RAIDKM_PH_COMMIT,
				    42, 12, 0xdeadbeefu, 4096);
	if (!err)
		err = raidkm_reshape_jread(conf, &rj);
	mddev_unlock(mddev);

	if (err) {
		pr_warn("md/raidkm:%s: jtest I/O failed (%d)\n", mdname(mddev), err);
		return err;
	}
	ok = le64_to_cpu(rj.seq) == seq &&
	     le32_to_cpu(rj.phase) == RAIDKM_PH_COMMIT &&
	     le64_to_cpu(rj.band_start_chunk) == 42 &&
	     le32_to_cpu(rj.band_chunks) == 12 &&
	     le32_to_cpu(rj.data_csum) == 0xdeadbeefu &&
	     le64_to_cpu(rj.reshape_position) == 4096;
	pr_warn("md/raidkm:%s: jtest seq=%llu %s (read seq=%llu phase=%s band=%llu csum=0x%08x)\n",
		mdname(mddev), seq, ok ? "PASS" : "FAIL",
		le64_to_cpu(rj.seq), raidkm_phase_name(le32_to_cpu(rj.phase)),
		le64_to_cpu(rj.band_start_chunk), le32_to_cpu(rj.data_csum));
	return ok ? len : -EIO;
}

static struct md_sysfs_entry
raidkm_reshape_jtest = __ATTR(raidkm_reshape_jtest, S_IRUGO | S_IWUSR,
			      raidkm_reshape_jtest_show,
			      raidkm_reshape_jtest_store);

/*
 * raidkm_reshape_maptest — DEBUG check of the band/row geometry engine
 * (raidkm_reshape_row_layout).  Reading validates, for both the current geometry
 * and the would-be add-parity target (m+1, raid_disks+1), that every row's data
 * + parity slots form a permutation of 0..N-1 and that rotating parity hits every
 * disk; reports PASS/FAIL plus a sample row (old vs new).
 */
static bool raidkm_geom_ok(int raid_disks, int m, bool rotating)
{
	int ds[RAIDKM_MAX_STRIPE_DISKS], ps[RAIDKM_MAX_STRIPE_DISKS];
	int seen[RAIDKM_MAX_STRIPE_DISKS], pd_hist[RAIDKM_MAX_STRIPE_DISKS];
	int k = raid_disks - m, row, i, covered = 0;

	if (raid_disks > RAIDKM_MAX_STRIPE_DISKS || k < 1 || m < 1)
		return false;
	memset(pd_hist, 0, sizeof(pd_hist));
	for (row = 0; row < raid_disks; row++) {
		raidkm_reshape_row_layout(raid_disks, m, rotating, row, ds, ps);
		memset(seen, 0, sizeof(seen));
		for (i = 0; i < k; i++)
			if (ds[i] < 0 || ds[i] >= raid_disks || seen[ds[i]]++)
				return false;
		for (i = 0; i < m; i++)
			if (ps[i] < 0 || ps[i] >= raid_disks || seen[ps[i]]++)
				return false;
		for (i = 0; i < raid_disks; i++)
			if (seen[i] != 1)		/* every slot covered once */
				return false;
		pd_hist[ps[0]] = 1;			/* pd_idx == parity slot 0 */
	}
	if (rotating) {
		for (i = 0; i < raid_disks; i++)
			covered += pd_hist[i];
		if (covered != raid_disks)		/* rotation hits every disk */
			return false;
	}
	return true;
}
...
static ssize_t
raidkm_reshape_maptest_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf = mddev->private;
	int N, m, k, i, len = 0;
	int ds[RAIDKM_MAX_STRIPE_DISKS], ps[RAIDKM_MAX_STRIPE_DISKS];
	bool rot, old_ok, new_ok;
	...
	old_ok = raidkm_geom_ok(N, m, rot);
	new_ok = raidkm_geom_ok(N + 1, m + 1, rot);

	len += sprintf(page + len, "current   N=%d m=%d %s: %s\n", N, m,
		       rot ? "rotating" : "parity-N", old_ok ? "PASS" : "FAIL");
	len += sprintf(page + len, "addparity N=%d m=%d %s: %s\n", N + 1, m + 1,
		       rot ? "rotating" : "parity-N", new_ok ? "PASS" : "FAIL");

	raidkm_reshape_row_layout(N, m, rot, 1, ds, ps);
	len += sprintf(page + len, "row1 old  data[");
	for (i = 0; i < k; i++)
		len += sprintf(page + len, "%d ", ds[i]);
	len += sprintf(page + len, "] parity[");
	for (i = 0; i < m; i++)
		len += sprintf(page + len, "%d ", ps[i]);
	len += sprintf(page + len, "]\n");

	raidkm_reshape_row_layout(N + 1, m + 1, rot, 1, ds, ps);
	len += sprintf(page + len, "row1 new  data[");
	for (i = 0; i < k; i++)
		len += sprintf(page + len, "%d ", ds[i]);
	len += sprintf(page + len, "] parity[");
	for (i = 0; i < m + 1; i++)
...
static struct md_sysfs_entry
raidkm_reshape_maptest = __ATTR(raidkm_reshape_maptest, S_IRUGO,
				raidkm_reshape_maptest_show, NULL);

/*
 * raidkm_reshape_start — DEBUG trigger for an online add-parity (m -> m+1)
 * reshape, so the COW migration can be driven/tested without mdadm changes.
 * Prerequisite: the +1 parity disk has already been attached as a spare
 * (mdadm --add).  Sets the new geometry and invokes check_reshape +
 * start_reshape via the personality, which sets prev_m/m, builds the new EC
 * tables, promotes the spare, and arms MD_RECOVERY_RESHAPE.
 */
static ssize_t
raidkm_reshape_start_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	int saved_layout, err;
	...
	/*
	 * Scratch-headroom validation (data_offset gap on every member) is
	 * done by raidkm_check_reshape below.
	 */
	saved_layout = mddev->layout;
	mddev->new_level = mddev->level;
	mddev->new_layout = (conf->m + 1) |
		(conf->rotating ? RAIDKM_LAYOUT_ROTATING : 0);
	mddev->new_chunk_sectors = mddev->chunk_sectors;
	mddev->delta_disks = 1;
	mddev->reshape_backwards = 0;
...
static struct md_sysfs_entry
raidkm_reshape_start = __ATTR(raidkm_reshape_start, S_IWUSR,
...
static ssize_t
raidkm_row_write_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->row_write));
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raidkm_row_write_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned long new;
	int err;
	...
	if (!conf) {
		...
		WRITE_ONCE(conf->row_write, !!new);
		/* off: give back the idle contexts; rows in flight give
		 * theirs back as they finish (raidkm_row_wctx_put) */
		if (!new)
			raidkm_row_wctx_free_idle(conf);
	}
	mddev_unlock(mddev);
	return err ?: len;
}
...
static struct md_sysfs_entry
raidkm_row_write_entry = __ATTR(rk_row_write, S_IRUGO | S_IWUSR,
				raidkm_row_write_show, raidkm_row_write_store);

static ssize_t
raidkm_row_write_depth_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->row_write_depth));
	spin_unlock(&mddev->lock);
	return ret;
}

/* rows the row path keeps in flight; each holds m chunk-sized parity buffers */
static ssize_t
raidkm_row_write_depth_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned long new;
	int err;
	...
	if (len >= PAGE_SIZE)
		return -EINVAL;
	if (kstrtoul(page, 10, &new) || !new || new > RK_ROW_WRITE_DEPTH_MAX)
		return -EINVAL;
	err = mddev_lock(mddev);
	if (err)
		return err;
	...
	if (!conf) {
		...
		WRITE_ONCE(conf->row_write_depth, new);
		/* a smaller pool shrinks as contexts come back */
		raidkm_row_wctx_free_idle(conf);
	}
	mddev_unlock(mddev);
	return err ?: len;
...
static struct md_sysfs_entry
raidkm_row_write_depth_entry = __ATTR(rk_row_write_depth, S_IRUGO | S_IWUSR,
				      raidkm_row_write_depth_show,
				      raidkm_row_write_depth_store);

static ssize_t
raidkm_row_rebuild_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->row_rebuild));
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raidkm_row_rebuild_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned long new;
	int err;
	...
	if (!conf)
		...
		/* read once per sync step; a change lands on the next chunk */
		WRITE_ONCE(conf->row_rebuild, !!new);
	mddev_unlock(mddev);
	return err ?: len;
}

static struct md_sysfs_entry
raidkm_row_rebuild_entry = __ATTR(rk_row_rebuild, S_IRUGO | S_IWUSR,
				  raidkm_row_rebuild_show,
...
raidkm_row_rebuild_window_show(struct mddev *mddev, char *page)
{
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->row_rb_window));
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raidkm_row_rebuild_window_store(struct mddev *mddev, const char *page,
				size_t len)
{
	struct r5conf *conf;
	unsigned long new;
	...
	if (!conf)
		...
		/* live: the sync thread drains the window it has and starts
		 * the next one at this size (0: back to synchronous bands) */
		WRITE_ONCE(conf->row_rb_window, (int)new);
	mddev_unlock(mddev);
	return err ?: len;
}

static struct md_sysfs_entry
raidkm_row_rebuild_window_entry = __ATTR(rk_row_rebuild_window,
					 S_IRUGO | S_IWUSR,
					 raidkm_row_rebuild_window_show,
					 raidkm_row_rebuild_window_store);

static ssize_t
raidkm_dcl_row_rebuild_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->dcl_row_rebuild));
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raidkm_dcl_row_rebuild_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned long new;
	int err;
	...
	if (!conf)
		...
		/* read once per population address; a change lands on the
		 * next row, and each row is complete before the mark moves */
		WRITE_ONCE(conf->dcl_row_rebuild, !!new);
	mddev_unlock(mddev);
	return err ?: len;
}

static struct md_sysfs_entry
raidkm_dcl_row_rebuild_entry = __ATTR(rk_dcl_row_rebuild, S_IRUGO | S_IWUSR,
				      raidkm_dcl_row_rebuild_show,
				      raidkm_dcl_row_rebuild_store);
...
raidkm_row_stats_show(struct mddev *mddev, char *page)
{
	...
	if (conf)
		ret = sprintf(page,
			      ...
			      (long long)atomic64_read(&conf->row_dread_csum_bad),
			      ...
			      (long long)div_u64(atomic64_read(&conf->row_rebuild_head_wait_ns),
						 NSEC_PER_MSEC),
			      (long long)atomic64_read(&conf->row_rebuild_retries),
			      (long long)atomic64_read(&conf->row_rebuild_head_timeouts),
			      (long long)atomic64_read(&conf->delayed_parked),
			      (long long)atomic64_read(&conf->delayed_aged),
			      (long long)atomic64_read(&conf->sync_token_regrant),
			      (long long)atomic64_read(&conf->overlap_slow),
			      (long long)atomic64_read(&conf->row_write_done),
			      (long long)atomic64_read(&conf->row_write_busy),
			      (long long)atomic64_read(&conf->row_write_nobuf),
			      (long long)atomic64_read(&conf->row_write_declined),
			      (long long)atomic64_read(&conf->row_write_errors));
	spin_unlock(&mddev->lock);
	return ret;
}
...
static ssize_t
raidkm_bio_sort_show(struct mddev *mddev, char *page)
{
	struct r5conf *conf;
	int ret = 0;
	...
	conf = mddev->private;
	if (conf)
		ret = sprintf(page, "%d\n", READ_ONCE(conf->batch_bio_dispatch));
	spin_unlock(&mddev->lock);
	return ret;
}

static ssize_t
raidkm_bio_sort_store(struct mddev *mddev, const char *page, size_t len)
{
	struct r5conf *conf;
	unsigned long v;
	int err;

	if (kstrtoul(page, 10, &v) || v > RK_BIO_SORT_SYNC)
		return -EINVAL;
	err = mddev_lock(mddev);
	if (err)
		return err;
	...
	if (!conf)
		...
		/* A stripe in flight may have sampled the old value; both
		 * paths issue the same bios, so a mixed window is harmless. */
		WRITE_ONCE(conf->batch_bio_dispatch, v);
	mddev_unlock(mddev);
	return err ?: len;
}

static struct md_sysfs_entry
raidkm_bio_sort_entry = __ATTR(rk_bio_sort, S_IRUGO | S_IWUSR,
			       raidkm_bio_sort_show, raidkm_bio_sort_store);
...
static struct attribute *raid5_attrs[] =  {
	...
	&raidkm_row_write_entry.attr,
	&raidkm_row_write_depth_entry.attr,
	...
	&ppl_write_hint.attr,
#ifdef RAIDKM_FAULT_INJECT
	&raidkm_reshape_inject.attr,
	&raidkm_reshape_jtest.attr,
	&raidkm_reshape_maptest.attr,
	&raidkm_reshape_start.attr,
#endif
	NULL,
};
...
/*
 * raid_km redesign: stock raid456 reserves the fixed
 * CPUHP_MD_RAID5_PREPARE hotplug slot.  Two modules can't share
 * the same hotplug state, so we ask the kernel for a dynamic
 * slot (CPUHP_BP_PREPARE_DYN) and remember the assigned number.
 * Set in raid5_init below, consumed by raid5_alloc_percpu and
 * free_scratch_buffer.
 */
static enum cpuhp_state raid_km_cpuhp_state;
...
{
	if (conf->effective_level == 6 && !percpu->spare_page) {
...
static void raid5_free_percpu(struct r5conf *conf)
{
	...
	cpuhp_state_remove_instance(raid_km_cpuhp_state, &conf->node);
	free_percpu(conf->percpu);
}

/*
 * raid_km redesign: build the (k+m) x k EC generator matrix and the m*k*32
 * GFNI- and base-format encode tables for the given m and k into freshly
 * kmalloc'd buffers.  Factored out of setup_conf so create, reshape-start
 * (new k) and mid-reshape assembly (previous k) share identical logic.  Matrix
 * type by m mirrors the original: Vandermonde (raid6-equivalent at m=2, MDS at
 * small k+m) for m <= RAIDKM_VANDERMONDE_MAX_M, Cauchy (always MDS) for larger
 * m.  m is invariant across a reshape, so only the dimensions change.  On
 * failure nothing is allocated (all-or-nothing).
 */
static int raidkm_build_ec(int m, int k, gfp_t gfp,
			   unsigned char **a_out,
			   unsigned char **gg_out,
			   unsigned char **gb_out)
{
	int total = k + m;
	unsigned char *a, *gg, *gb;

	a  = kmalloc_array(total, k, gfp);
	gg = kmalloc_array(m * k, 32, gfp);
	gb = kmalloc_array(m * k, 32, gfp);
	if (!a || !gg || !gb) {
		kfree(a);
		kfree(gg);
		kfree(gb);
	...
	if (m <= RAIDKM_VANDERMONDE_MAX_M)
		gf_gen_rs_matrix(a, total, k);
	else
		gf_gen_cauchy1_matrix(a, total, k);
	/* skip the leading k x k identity; encode rows start at row k */
	ec_init_tables_gfni(k, m, a + k * k, gg);
	ec_init_tables_base(k, m, a + k * k, gb);
	*a_out = a;
	*gg_out = gg;
	*gb_out = gb;
	return 0;
...
static void free_conf(struct r5conf *conf)
{
	...
	/* an abort in setup_conf() can come here before the pool exists */
	if (conf->row_wctx_free.next) {
		raidkm_row_wctx_free_idle(conf);
		WARN_ON_ONCE(conf->row_wctx_nr);	/* a row still in flight */
	}
	kvfree(conf->row_wlock);
	...
	/* Before kfree(conf->disks): destroying the csum workqueues EXECUTES any
...
static int raid5_alloc_percpu(struct r5conf *conf)
{
	...
	err = cpuhp_state_add_instance(raid_km_cpuhp_state, &conf->node);
	if (!err) {
...
static struct r5conf *setup_conf(struct mddev *mddev)
{
	...
	if (mddev->new_level != 5
	    && mddev->new_level != 4
	    && mddev->new_level != 6
	    && mddev->new_level != RAID_KM_LEVEL) {
		pr_warn("md/raid:%s: raid level not set to 4/5/6/%d (%d)\n",
			mdname(mddev), RAID_KM_LEVEL, mddev->new_level);
		return ERR_PTR(-EIO);
	}
	/*
	 * raidkm: for level 71, mddev->new_layout is NOT an ALGORITHM_*
	 * selector.  Its low byte carries the parity count m and bit
	 * RAIDKM_LAYOUT_ROTATING selects the parity placement (clear =
	 * PARITY_N, set = rotating; see raid_km.h).  All m in
	 * [2, RAIDKM_MAX_M] dispatch through ISA-L's ec_encode_data;
	 * users who want raid6's specific P+Q math should pick level 6.
	 * Reject an m out of range, or any unknown layout bit, before the
	 * rest of setup so the error message is clear.
	 */
	...
	if (is_raid6_math(mddev->new_level) && mddev->raid_disks < 4) {
	...
	conf->row_write = default_row_write;
	conf->row_wlock = kvcalloc(RAIDKM_ROW_WLOCK_SLOTS,
				   sizeof(*conf->row_wlock), GFP_KERNEL);
	if (!conf->row_wlock)
		goto abort;
	conf->row_write_depth = RK_ROW_WRITE_DEPTH;
	atomic_set(&conf->row_writes, 0);
	init_waitqueue_head(&conf->row_write_wait);
	spin_lock_init(&conf->row_wctx_lock);
	...
	conf->row_rb_pace = 0;
	conf->dcl_row_rebuild = default_dcl_row_rebuild;
	 ...
	 * write throughput at cnt=2 because the single raid5d kthread otherwise
	 * caps at ~75% of one core. Auto-default targets TOTAL worker threads
	 * ~= nproc/2 regardless of NUMA topology: alloc_thread_groups() creates
	 * num_possible_nodes() groups each with wcnt workers, so we divide
	 * nproc/2 by node count for the per-group value here. Floor of 2 per
	 * group preserves the win on small hosts; users can override via the
	 * default_group_thread_cnt module parameter (per group) or per-array
	 * sysfs: worker_thread_cnt (total, recommended) or group_thread_cnt
	 * (per group, stock-mdraid compatible).
	 */
	{
		int wcnt = default_group_thread_cnt;
		unsigned int nodes = num_possible_nodes();
		/* Defensive: a running kernel always has node 0 in the possible
		 * map, but a broken invariant shouldn't OOPS us with a div-by-0. */
		if (WARN_ON_ONCE(nodes == 0))
			nodes = 1;
		if (wcnt < 0)
			wcnt = max(num_online_cpus() / (2u * nodes), 2u);
		if (!alloc_thread_groups(conf, wcnt, &group_cnt, &new_group)) {
			conf->group_cnt = group_cnt;
			conf->worker_cnt_per_group = wcnt;
	...
	/*
	 * Collecting a handled stripe's member bios and submitting them in
	 * stripe-sector order costs a lock and a list_sort per flush; upstream
	 * pays that only on rotational members, where it saves seeks, and turns
	 * it off as soon as one member is non-rotational.
	 *
	 * On flash the trade is not seek time, it is REQUEST SIZE: with worker
	 * groups, several workers submit 4 KiB member I/O for different stripes
	 * at once, the block layer sees each member's sectors out of order and
	 * merges almost nothing, and a rebuild reaches the drive in ~6 KiB
	 * requests -- below the indirection unit of a large-IU SSD, which then
	 * rewrites a whole unit per request.  Single-threaded (group_thread_cnt
	 * 0) the same rebuild merges to ~125 KiB, which is what identifies
	 * ORDER, not the 4 KiB stripe unit, as the cause.
	 *
	 * So keep upstream's default and expose the choice: sysfs rk_bio_sort.
	 */
	conf->batch_bio_dispatch = RK_BIO_SORT_WRITES;
	rdev_for_each(rdev, mddev) {
		if (test_bit(Journal, &rdev->flags))
			continue;
		if (bdev_nonrot(rdev->bdev)) {
			conf->batch_bio_dispatch = RK_BIO_SORT_OFF;
			break;
		}
	}
	if (default_bio_sort >= 0)
		conf->batch_bio_dispatch = min(default_bio_sort,
					       RK_BIO_SORT_SYNC);
	...
	/*
	 * raid_km redesign: effective_level must be set BEFORE
	 * raid5_alloc_percpu, because the CPUHP callback chain calls
	 * alloc_scratch_buffer which reads conf->effective_level to
	 * decide whether to allocate the per-CPU spare_page (needed
	 * for raid6-style P+Q syndrome validation in
	 * ops_run_check_pq → async_syndrome_val).  Without this,
	 * spare_page stays NULL and async_pq.c hits BUG_ON during
	 * the first resync check.  See struct r5conf for the field
	 * comment.
	 */
	conf->effective_level = is_raid6_math(conf->level) ? 6 : conf->level;
	...
	conf->level = mddev->new_level;
	/* See struct r5conf comment: effective_level is what internal
	 * "raid6 math" checks should look at. */
	conf->effective_level = is_raid6_math(conf->level) ? 6 : conf->level;
	if (conf->level == RAID_KM_LEVEL) {
		...
		/* Native checksum: disk-backed (SB layout bit) and/or in-core
		 * (native_csum module param).  Allocate the demand-paged region
		 * cache + its workqueue; csum_disk additionally persists via the
		 * reserved tail region (writeback on stop). */
		conf->csum_disk = raidkm_layout_has_csum(mddev->new_layout);
		if (native_csum || conf->csum_disk) {
			conf->csum = raidkm_csum_cache_alloc(conf);
			if (!conf->csum) {
				/* Fail the run rather than silently running an
				 * integrity-labelled array unprotected: writes
				 * would leave the on-disk region STALE while
				 * the SB layout bit still claims csum, and the
				 * NEXT assemble would false-mismatch (and
				 * heal-rewrite) every block written now. */
				pr_err("md/raid:%s: native csum alloc failed\n",
				       mdname(mddev));
				goto abort;
			}
		}
		...
		if (raidkm_layout_is_dcl(mddev->new_layout)) {
			...
			mutex_init(&conf->dcl_retire_mutex);
			/*
			 * Write-intent bitmap: SUPPORTED (internal).  The
			 * stripe write tracking (raidkm_bitmap_startwrite /
			 * _endwrite on sh->sector) and the sync skip/gate
			 * calls in raid5_sync_request both use the per-device
			 * stripe-sector domain, which is exactly the domain
			 * the declustered row iteration uses — one row's
			 * ngroups group-stripes share sh->sector, and the
			 * bitmap's per-chunk counter absorbs the multiple
			 * writers.  Spare population / copy-from-spare stay
			 * mark-journaled via the rkdcl block, independent of
			 * the bitmap.  File-backed bitmaps are untested with
			 * the dcl path — keep refusing those.
			 */
			if (mddev->bitmap_info.file) {
				pr_err("md/raid:%s: declustered + file-backed bitmap not supported\n",
				       mdname(mddev));
				goto abort;
			}
	...
	/*
	 * prev_m tracks the not-yet-reshaped geometry's parity count; it only
	 * differs from m during an add-parity reshape (wired in start_reshape).
	 * At create/assemble there is no m-change in flight, so hold it == m.
	 */
	conf->prev_m = conf->m;
	if (conf->effective_level == 6) {
		conf->max_degraded = conf->m;
		if (is_raidkm(conf))
			/* raidkm incremental-parity path: m > 2 uses ISA-L's
			 * ec_encode_data_update_*_gfni; m == 2 uses the stock
			 * raid6_call (async_gen_syndrome) RMW dance, which is
			 * byte-identical (see ops_run_prexor6 /
			 * ops_run_reconstruct6 SYNDROME_SRC_WRITTEN branches).
			 * Always advertise RMW either way so small writes skip
			 * the full-stripe RCW encode. */
			conf->rmw_level = PARITY_ENABLE_RMW;
		else if (raid6_call.xor_syndrome)
			conf->rmw_level = PARITY_ENABLE_RMW;
		else
	...
	/*
	 * raidkm I/O path: ISA-L ec_encode_data_*_gfni for all m
	 * (level 6 still uses raid6_call P+Q math; level 71 unifies
	 * on generic k+m EC regardless of m).  Build the (k + m) x k
	 * generator matrix and the m * k * 32 encoding tables once
	...
	if (is_raidkm(conf)) {
		...
		if (raidkm_build_ec(conf->m, k, GFP_KERNEL,
				    &conf->ec_a_matrix, &conf->ec_g_tbls_gfni,
				    &conf->ec_g_tbls_base)) {
			ret = -ENOMEM;
			goto abort;
		}
		/*
		 * Assembling an array that is mid-reshape (reshape_position set,
		 * a real disk-count change): the not-yet-reshaped region still
		 * uses the previous geometry, so build its tables too.  They are
		 * selected per stripe by region (raidkm_a_matrix/raidkm_g_*) and
		 * freed when the reshape finishes (raid5_finish_reshape).  For an
		 * add-parity reshape the previous geometry's parity count differs
		 * from conf->m and is carried by mddev->layout (the pre-reshape
		 * layout); for grow-data it equals conf->m.  prev_m must reflect
		 * that so raidkm_sh_m() picks the right m for before-region stripes.
		 */
		if (mddev->reshape_position != MaxSector &&
		    conf->previous_raid_disks != conf->raid_disks) {
			int prev_m = raidkm_layout_m(mddev->layout);
			int pk = conf->previous_raid_disks - prev_m;

			conf->prev_m = prev_m;
			if (raidkm_build_ec(prev_m, pk, GFP_KERNEL,
					    &conf->prev_ec_a_matrix,
					    &conf->prev_ec_g_tbls_gfni,
					    &conf->prev_ec_g_tbls_base)) {
				ret = -ENOMEM;
				goto abort;
			}
...
/*
 * The per-request context of raid5_make_request(), from a pool: its stripe
 * bitmap has one bit per stripe of the largest bio the array takes, which
 * raid5_set_limits() raises to a whole row when a row is over
 * RAID5_MAX_REQ_STRIPES stripes (as upstream 4ffe28ed0d7c "md/raid5: make
 * sure max_sectors is not less than io_opt" does, with this pool).  The limit
 * is set once, before this runs, and a reshape only moves io_opt.  dm-raid
 * has no queue of ours: it keeps the old fixed size.
 */
#define RAID5_CTX_POOL_MIN	16

static int raid5_create_ctx_pool(struct r5conf *conf)
{
	struct stripe_request_ctx *ctx;
	unsigned int stripes = RAID5_MAX_REQ_STRIPES;

	if (!mddev_is_dm(conf->mddev))
		stripes = max_t(unsigned int, stripes,
				queue_max_hw_sectors(conf->mddev->gendisk->queue) >>
				RAID5_STRIPE_SHIFT(conf));
	/* + 1: an unaligned bio spans one more stripe */
	conf->ctx_size = struct_size(ctx, sectors_to_do,
				     BITS_TO_LONGS(stripes + 1));
	conf->ctx_pool = mempool_create_kmalloc_pool(RAID5_CTX_POOL_MIN,
						     conf->ctx_size);
...
static int raid5_set_limits(struct mddev *mddev)
{
	...
	/*
	 * md-kmec default: enable zero-copy writes (skip_copy).  For a
	 * full-page-aligned write we alias the stripe-cache page directly to
	 * the incoming bio page instead of memcpy'ing it in via biodrain,
	 * which is the dominant non-EC cost on full-stripe writes (~+36% on a
	 * wide CPU-bound array).  The only requirement is stable pages, which
	 * is free for the O_DIRECT OST workload md-kmec targets.  Can be
	 * overridden per-array via the skip_copy sysfs attribute.
	 */
	conf->skip_copy = 1;
	lim.features |= BLK_FEAT_STABLE_WRITES;
...
static int raid5_run(struct mddev *mddev)
{
	...
	if (mddev->reshape_position != MaxSector) {
		...
		int old_disks;
		int max_degraded = (is_raid6_math(mddev->level) ? 2 : 1);
		/* Parity counts of the new and old geometries.  For raidkm m is
		 * packed in the layout low byte and an add-parity reshape changes
		 * it, so derive each side's count from its layout; the generic
		 * raid4/5/6 fallback (1 or 2) is the same on both sides. */
		int new_md = max_degraded, old_md = max_degraded;
		unsigned int chunk_sectors;
		int new_data_disks;

		if (mddev->level == RAID_KM_LEVEL) {
			new_md = raidkm_layout_m(mddev->new_layout);
			old_md = raidkm_layout_m(mddev->layout);
		}
		...
		sector_div(here_old, chunk_sectors * (old_disks - old_md));
		...
		if (mddev->delta_disks == 0 &&
			 ...
			 * denser dcl packing a forward migration never overwrites
			 * a not-yet-vacated source row.  Either way the per-band
			 * COW stages to scratch before overwriting home, and
			 * recovery is journal-driven (raidkm_reshape_recover) —
			 * including a resume at reshape_position 0, which the
			 * stock check would reject as "too early".  This is the
			 * normal dm/LVM activation path: the reshape is stamped
			 * into the superblocks by one table load and actually
			 * runs on the next. */
			pr_debug("md/raid:%s: raidkm COW reshape will continue\n",
				 mdname(mddev));
		} else if (mddev->reshape_backwards
	...
	/* Declustered Phase 3/4: an interrupted population OR copy was
	 * restored from the rkdcl journal.  A POPULATING pass self-triggers a
	 * REQUESTED sync (action_store precedent); a COPYING pass rides md's
	 * own RECOVER for the still-recovering replacement, so it only needs
	 * MD_RECOVERY_NEEDED — the copy_request branch intercepts it. */
	if (conf->dcl && conf->reb_pop >= 0 &&
	    conf->reb[conf->reb_pop].state == RKDCL_ASSIGN_COPYING) {
		pr_info("md/raid:%s: declustered: resuming copy of disk %d from mark %llu\n",
			mdname(mddev), conf->reb[conf->reb_pop].disk,
			(unsigned long long)atomic64_read(&conf->reb_mark));
		set_bit(MD_RECOVERY_NEEDED, &mddev->recovery);
	} else if (conf->dcl && conf->reb_pop >= 0) {
...
static void raid5_free(struct mddev *mddev, void *priv)
{
	...
	/* Native checksum: drain the workqueue + flush dirty region pages on a
	 * clean stop, before the conf and its rdevs go away. */
	raidkm_csum_writeback(conf);
	free_conf(conf);
...
static int raid5_add_disk(struct mddev *mddev, struct md_rdev *rdev)
{
	...
	/*
	 * Declustered Phase 3/3b rebalance v1 (population design note §5): a
	 * replacement may not join mid-population, and joining an array with
	 * assignments first RETIRES ALL of them — under a full quiesce,
	 ...
	 * replacement exactly like the Phase-2-validated rebuild leg.
	 *
	 * ALL-OR-NOTHING is a correctness rule, not laziness: with chained
	 * redirects, dropping only the replaced disk's assignment while
	 * another assignment's chain traverses it would leave that other
	 * assignment's spare content pointing at the BLANK replacement
	 * (stale-spare reads) — and stock RECOVER only rebuilds the
	 * replacement's own slots, not spare-column content it hosts for
	 * others.  Dropping every assignment is always safe (§5: redirect
	 * off => decodable); auto-arm (or the operator) re-populates the
	 * still-missing disks afterwards.  The spare columns are freed;
	 * their stale content is never mapped.  Copy-aware partial retire
	 * is the Phase-4 fast path.
	 */
	/*
	 * Declustered Phase 4 copy-from-spare (declustered-rebalance-copy-
	 * design.md §13): if the replacement can fill the slot of a
	 * POPULATED assignment, don't retire — admit it recovering and arm
	 * COPYING (below, after the admit loop) so md's RECOVER copies X's
	 * already-populated content instead of decoding.  MULTI-ASSIGNMENT
	 * aware: the copy sources every row from the chain endpoint
	 * (raidkm_dcl_copy_src), so R receives X's full role — its own
	 * group columns AND spare content X hosts for OTHER assignments'
	 * chains — and completion retires ONLY the copied assignment
	 * (raidkm_dcl_retire_one), leaving the others' populated spares
	 * intact.  Every OTHER assignment must itself be POPULATED (a
	 * mid-flight pass is excluded by reb_pop, and copy_src resolves
	 * chains through steady POPULATED hops only).  Ineligible shapes
	 * fall back to the validated retire-all leg.
	 *
	 * Native checksum composes with the copy: each copied block's stored
	 * CRC is MIGRATED from the source (chain endpoint) disk's key to R's
	 * alongside the bytes (raidkm_dcl_csum_migrate in the copy workers),
	 * so R's content stays verified end-to-end — a torn or rotted copy
	 * is caught by the migrated CRC on first read and healed by decode.
	 */
	copy_eligible = false;
	if (conf->dcl && conf->reb_pop < 0 && conf->nreb &&
	    /* the copy buffers are chunk-order contiguous pages; a chunk
	     * too large to buffer takes the decode leg */
	    get_order((unsigned int)conf->chunk_sectors << 9) <=
		MAX_PAGE_ORDER) {
		int i;
		bool all_pop = true, fillable = false;

		for (i = 0; i < conf->nreb; i++) {
			if (conf->reb[i].state != RKDCL_ASSIGN_POPULATED)
				all_pop = false;
			else if (conf->disks[conf->reb[i].disk].rdev == NULL &&
				 (rdev->raid_disk < 0 ||
				  rdev->raid_disk == conf->reb[i].disk))
				fillable = true;
		}
		copy_eligible = all_pop && fillable;
	}
...
out:
	 ...
	 * the replacement fills a POPULATED assignment's slot.  Resolve now
	 * that the admit picked a slot:
	 *
	 *  - landed at an assignment's X: arm COPYING for THAT assignment
	 *    under a full quiesce (a lock-free reader must never pair the
	 *    NEW state with the STALE mark (dev_sectors from the completed
	 *    population): covered=true would route a write to blank R and
	 *    lose it from the spare).
	 *  - landed elsewhere (a non-assigned empty slot won), or the
	 *    COPYING journal failed: the deferred retire-all is REQUIRED
	 *    before any stock RECOVER runs — under POPULATED the redirect
	 *    sends an assigned slot's rebuild writes to the spare (R
	 *    reaches In_sync without ever being written), and stock RECOVER
	 *    never rebuilds hosted spare content for surviving assignments.
	...
	if (!err && copy_eligible) {
		...
		for (i = 0; i < conf->nreb; i++)
			if (conf->reb[i].disk == rdev->raid_disk &&
			    conf->reb[i].state == RKDCL_ASSIGN_POPULATED) {
				idx = i;
				break;
			}
		if (idx >= 0) {
			...
			raid5_quiesce(mddev, 1);
			m0 = (u64)atomic64_read(&conf->reb_mark);
			jm0 = conf->reb_journal_mark;
			atomic64_set(&conf->reb_mark, 0);
			...
			if (jerr) {
				 ...
				 * never reads the mark today, but a
				 * mark-0-under-POPULATED remnant is a trap
				 * for any future mark-aware consumer */
				conf->reb[idx].state = RKDCL_ASSIGN_POPULATED;
				WRITE_ONCE(conf->reb_pop, -1);
				atomic64_set(&conf->reb_mark, m0);
				conf->reb_journal_mark = jm0;
			}
			raid5_quiesce(mddev, 0);
			if (!jerr)
				pr_info("md/raid:%s: declustered: copy-from-spare rebalance armed for disk %d (spare col %d) -> replacement (%d assignment(s) active)\n",
					mdname(mddev), conf->reb[idx].disk,
					conf->reb[idx].spare, conf->nreb);
		}
		if (idx < 0 || jerr) {
			jerr = raidkm_dcl_retire_all(mddev, idx >= 0 ?
...
static int raid5_resize(struct mddev *mddev, sector_t sectors)
{
	...
	/*
	 * Declustered: the map is per-row (hashed), so a member-size GROW is
	 * just more rows at the tail of every disk — same N/g/m/s/seed, no
	 * permutation change.  What does move is the rkdcl metadata block,
	 * which sits AT dev_sectors on every member: write it at the NEW
	 * offset first (the old block stays valid at the old offset), persist
	 * the superblock's new dev_sectors, and only then expose the new
	 * capacity — every crash window reads a valid block at the offset its
	 * superblock points to.  The new rows hold garbage, so a resync from
	 * the old size parity-initializes them (recovery_cp below).
	 */
	if (conf->dcl) {
		sector_t old_ds = mddev->dev_sectors;

		sectors &= ~((sector_t)conf->chunk_sectors - 1);
		if (sectors < old_ds)
			return -EINVAL;		/* shrink: deferred */
		if (conf->prev_dcl || mddev->reshape_position != MaxSector)
			return -EBUSY;		/* reshape in flight */
		if (conf->nreb)
			return -EBUSY;		/* live spare assignment: the
						 * rkdcl POPULATED mark sentinel
						 * is dev_sectors itself */
		if (mddev->degraded)
			return -EINVAL;		/* v1: healthy pool only */
		if (conf->csum)
			return -EOPNOTSUPP;	/* CRC region sits past
						 * dev_sectors; relocation is a
						 * follow-up */
		newsize = raid5_size(mddev, sectors, mddev->raid_disks);
		if (mddev->external_size && mddev->array_sectors > newsize)
			return -EINVAL;
		if (mddev->bitmap) {
		...
		if (sectors != old_ds) {
			...
			if (ret) {
				mddev->dev_sectors = old_ds;
				pr_warn("md/raid:%s: declustered: resize aborted — could not write the rkdcl block at the new size\n",
					mdname(mddev));
				return ret;
			}
			set_bit(MD_SB_CHANGE_DEVS, &mddev->sb_flags);
		...
		if (sectors > old_ds && mddev->recovery_cp > old_ds) {
			mddev->recovery_cp = old_ds;
			/* the new rows were never written, so their bitmap
			 * bits are clean and the scoped-skip in sync_request
			 * would pass over exactly the region that needs its
			 * parity initialized — force the sweep (it starts at
			 * recovery_cp, so only the new rows are visited;
			 * cleared again when the sync completes) */
			conf->fullsync = 1;
			set_bit(MD_RECOVERY_NEEDED, &mddev->recovery);
		}
...
static int raidkm_dcl_start_reshape(struct mddev *mddev)
{
	...
	if (mddev->delta_disks == 0 &&
	    (mddev->new_layout < 0 || mddev->new_layout == mddev->layout))
		return -EINVAL;

	/*
	 * The NEW declustered geometry comes from the staged new_layout word
	 * (g/m/s) plus the new pool size.  Pool expansion leaves the layout word
	 * unchanged (g/m/s fixed, only N and the seed move); the group-geometry
	 * reshapes — add-parity (m+1), add-data (k+1), spare-count (s+/-1) —
	 * change it.  Deriving from new_layout keeps a single path for all of
	 * them and is identical to the old behaviour when new_layout == layout.
	 */
	{
		int lay = (mddev->new_layout >= 0) ? mddev->new_layout
						   : mddev->layout;
		new_g = RAIDKM_LAYOUT_DCL_G(lay);
		new_m = raidkm_layout_m(lay);
		new_s = RAIDKM_LAYOUT_DCL_S(lay);
	}
	new_k = new_g - new_m;
	ec_change = (new_m != oge->m || new_k != oge->k);
	if (new_g < new_m + 1 || new_s < 1 || newN < new_g + new_s ||
	    (newN - new_s) % new_g)			/* C1 for the NEW geometry */
		return -EINVAL;
	 ...
	 * (add-parity / add-data).  Sleepable + before touching conf, so a
	 * failure leaves the array at its old geometry.  Installed (and the old
	 * set demoted to prev_ec_*) under device_lock below.
	 */
	if (ec_change &&
	    raidkm_build_ec(new_m, new_k, GFP_KERNEL, &new_a, &new_gg, &new_gb))
		return -ENOMEM;

	/* build the NEW map (nbase inherited; N, g/m/s and seed per new_layout) */
	nge = raidkm_dcl_geom_new(newN, new_g, new_m, new_s, oge->nbase,
	 ...
	 * (mddev->layout = conf->algorithm).  Unchanged for pool expansion. */
	conf->algorithm = (mddev->new_layout >= 0) ? mddev->new_layout
						   : mddev->layout;
	if (ec_change) {
		/* the un-migrated region keeps decoding with the old (k,m); the
		 * migrated region uses the new — demote current tables to prev_*
		 * and install the freshly built set.  max_degraded tracks the
		 * larger m so stripe buffers fit either region. */
		conf->prev_m = oge->m;
		conf->m = new_m;
		conf->max_degraded = max(new_m, (int)oge->m);
		conf->prev_ec_a_matrix    = conf->ec_a_matrix;
		conf->prev_ec_g_tbls_gfni = conf->ec_g_tbls_gfni;
		conf->prev_ec_g_tbls_base = conf->ec_g_tbls_base;
		conf->ec_a_matrix    = new_a;
		conf->ec_g_tbls_gfni = new_gg;
		conf->ec_g_tbls_base = new_gb;
	}
	conf->generation++;
	smp_mb();
	if (backward) {
	...
	set_bit(MD_RECOVERY_NEEDED, &mddev->recovery);
	conf->reshape_checkpoint = jiffies;
	pr_info("md/raid:%s: declustered reshape started: N %d->%d g %u->%u m %u->%u s %u->%u (ngroups %u->%u), new seed 0x%llx\n",
		mdname(mddev), conf->previous_raid_disks, newN,
		oge->g, nge->g, oge->m, nge->m, oge->s, nge->s,
...
static int raid5_start_reshape(struct mddev *mddev)
{
	...
	/*
	 * raidkm: the data-disk count k is about to change, so build the EC
	 * matrix/tables for the new geometry now — while sleepable and holding
	 * no lock — so an allocation failure aborts cleanly with the array still
	 * at its old geometry.  They are installed (and the old set demoted to
	 * prev_ec_*) under device_lock together with the geometry change below.
	 */
	if (is_raidkm(conf)) {
		/*
		 * The new geometry's parity count comes from new_layout: for an
		 * add-parity reshape it is conf->m + 1, for a grow-data reshape it
		 * equals conf->m.  Build the new EC tables for (new_m, new_k); for
		 * add-parity new_k == k (data count unchanged), for grow-data new_m
		 * == m so this matches the previous fixed-m behavior.
		 */
		int new_m = raidkm_layout_m(mddev->new_layout);
		int nk = (conf->raid_disks + mddev->delta_disks) - new_m;

		if (raidkm_build_ec(new_m, nk, GFP_KERNEL,
				    &new_a, &new_gg, &new_gb))
			return -ENOMEM;
	}
	...
	/*
	 * raidkm: invalidate the COW reshape journal on every member
	 * (including the just-added one) before the reshape arms.  The
	 * scratch zone below data_offset is never zeroed by userspace, so a
	 * stale journal from a previous array on the same devices could
	 * otherwise win jread() on recovery and fake a completed migration.
	 */
	if (is_raidkm(conf)) {
		struct raidkm_reshape_ctx ctx = {
			.jseq		= 0,
		...
		if (raidkm_reshape_journal_init(conf, &ctx))
			pr_warn("md/raidkm:%s: failed to initialize the reshape journal on any member\n",
				mdname(mddev));
	}
...
static void raid5_finish_reshape(struct mddev *mddev)
{
	...
	if (!test_bit(MD_RECOVERY_INTR, &mddev->recovery)) {
		...
		if (is_raidkm(conf) && conf->prev_dcl) {
			...
			md_set_array_sectors(mddev, raid5_size(mddev, 0, 0));
			/*
			 * md-core only refreshes the gendisk capacity after a
			 * reshape when delta_disks > 0 (md.c).  A spare-count
			 * reshape keeps N fixed (delta_disks == 0) yet changes
			 * ngroups, so the array size moves with no disk added or
			 * removed — expose it here ourselves.  set_capacity() is
			 * safe under the reconfig_mutex held across finish_reshape
			 * (unlike set_capacity_and_notify(), which md-core calls
			 * only after dropping the lock); a re-assemble would set it
			 * anyway, this just makes the new size live immediately.
			 */
			if (!mddev_is_dm(mddev) && mddev->gendisk)
				set_capacity(mddev->gendisk, mddev->array_sectors);
			set_bit(MD_SB_CHANGE_DEVS, &mddev->sb_flags);
			pr_info("md/raid:%s: declustered geometry reshape finished: %u groups/row, array now %llu sectors\n",
		 ...
		 * this block is skipped and prev_ec_* stay live for the resume;
		 * they are freed at array stop in free_conf.)
		 */
		if (is_raidkm(conf)) {
			kfree(conf->prev_ec_a_matrix);
			kfree(conf->prev_ec_g_tbls_gfni);
			kfree(conf->prev_ec_g_tbls_base);
			conf->prev_ec_a_matrix = NULL;
...
static void raid5_quiesce(struct mddev *mddev, int quiesce)
{
	...
	if (quiesce) {
		...
		/* row writes too: their completion work still uses its
		 * context and conf after the stripes are released
		 * (raidkm_row_write_done) */
		wait_event_cmd(conf->wait_for_quiescent,
				    atomic_read(&conf->active_stripes) == 0 &&
				    atomic_read(&conf->active_aligned_reads) == 0 &&
				    atomic_read(&conf->row_writes) == 0,
...
/*
 * raid_km redesign: reshape gate for level 71.  raidkm supports reshape for
 * disk-count (capacity / "add data disk") and chunk-size changes, plus the
 * single-parity add (m -> m+1) online reshape.  Constraints on a layout change
 * (mddev->layout packs m in the low byte and the PARITY_N/rotating bit):
 *   - the parity-placement bit is immutable (flipping it would remap every
 *     block under a geometry the reshape engine does not assume);
 *   - m may only INCREASE by exactly one per reshape (adding one parity disk).
 *     The before-region keeps m parities and the after-region m+1, with both
 *     EC table sets live (prev_ec_* / ec_*, selected per stripe by region);
 *     max_degraded tracks the larger m so the stripe cache is sized for both.
 *     A larger jump, an m decrease, or a placement flip is rejected.
...
static int raidkm_check_reshape(struct mddev *mddev)
{
	...
	if (mddev->new_layout >= 0 && mddev->new_layout != mddev->layout) {
		/*
		 * Declustered geometry reshapes (add-parity m+1, add-data k+1,
		 * spare-count s+/-1) change g/m/s in the layout word; the number of
		 * disks added is ngroups (one new column per group), not 1, and the
		 * DCL start path (raidkm_dcl_start_reshape) validates C1 and rebuilds
		 * the group EC.  Accept the layout-only stage (delta_disks == 0) like
...
scratch:
	 ...
	 * and tables are rebuilt for the new k in raid5_start_reshape (the old-k
	 * set is kept as prev_ec_* and selected per stripe by k for the duration
	 * of the reshape; see raidkm_a_matrix()/raidkm_g_*()), so reconstruction
	 * stays correct across the new geometry.
	 */
	if (new_chunk > 0) {
		if (!is_power_of_2(new_chunk))
			return -EINVAL;
...
static void *raid6_takeover(struct mddev *mddev)
{
	 ...
	 *
	 * raid_km redesign note: takeover from stock raid5/6 is out of
	 * scope until the on-disk format and reshape paths are settled.
	 * The check below intentionally compares against ourselves so
	 * any current attempt to use takeover returns -EINVAL.
	 */
	int new_layout;

	if (mddev->pers != &raid_km_personality)
		return ERR_PTR(-EINVAL);
	if (mddev->degraded > 1)
		return ERR_PTR(-EINVAL);
...
/*
 * raid_km redesign: a single personality at level 71.  Initially
 * uses the same raid5_* handlers as stock raid4/5/6 — the k+m
 * generalization (ISA-L encode/decode, m-way recovery, etc.) is
 * staged in over subsequent commits.  For this first commit the
 * behavior on m=2 should be identical to stock raid6.
 */
static struct md_personality raid_km_personality =
{
	...
	.resize		= raid5_resize,
	.size		= raid5_size,
	.check_reshape	= raidkm_check_reshape,
...
static int __init raid5_init(void)
{
	...
	raid5_wq = alloc_workqueue("raidkmwq",
	...
	raid_km_cpuhp_state = ret;
	register_md_personality(&raid_km_personality);
	return 0;
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("raidkm — k+m Reed-Solomon md personality (fork of raid5.c)");
MODULE_ALIAS("md-personality-71");
MODULE_ALIAS("md-raidkm");
MODULE_ALIAS("md-level-71");
