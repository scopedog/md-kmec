		...
		if (!sh) {
			...
			if (!sh) {
				...
				pr_debug("md/raid:%s: Increasing stripe cache size to %d to recovery data on journal.\n",
					mdname(mddev),
					new_size);
				ret = raid_km_set_cache_size(mddev, new_size);
 ...
 *
 */
int raid_km_c_journal_mode_set(struct mddev *mddev, int mode)
{
	struct r5conf *conf;
...
EXPORT_SYMBOL(raid_km_c_journal_mode_set);
				      ...
				      const char *page, size_t length)
{
	...
	if (ret)
		return ret;
	ret = raid_km_c_journal_mode_set(mddev, mode);
